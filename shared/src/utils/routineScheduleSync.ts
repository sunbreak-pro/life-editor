...
/**
 * 1:1 behaviour-preserving port of
 * frontend/src/utils/routineScheduleSync.ts. NOT modified — the only
 * delta vs. the Tauri original is the import paths (shared `dateKey` /
 * `generateId` / `routineFrequency`). The `shouldCreateRoutineItem`
 * reject order (isDeleted → isArchived/!isVisible → group OR → own freq)
 * is the Issue 017 guard (d): a soft-deleted routine never re-spawns a
 * schedule_item, so the generator cannot resurrect cascade-deleted
 * children (S4-0 D-4). Do not reorder these checks.
 *
 ...
 * Tauri original and none is added here (改変禁止). The runaway-creation
 * defence is upstream — `shouldCreateRoutineItem` returns false for
 * deleted/archived/invisible routines and "group" with no groups, and
 * `shouldRoutineRunOnDate` returns false for unknown frequency. Callers
 * that pass an unbounded range are themselves capped
 * (`backfillMissedRoutineItems` clamps to 90 days);
 * `ensureRoutineItemsForDateRange` is uncapped by existing contract
 * (S4-0) and relies on the `(routine_id, date)` partial UNIQUE for
 * idempotency under rapid re-invocation.
 */
