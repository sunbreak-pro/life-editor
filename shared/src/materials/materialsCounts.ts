 ...
 *
 * The Materials providers (TodoTree / Notes / Dailies / WikiTags) are mounted
 ...
 *
 *   - todos: live todos that still need doing. Excludes soft-deleted rows,
 *     DONE rows, retired 'folder' rows (#225), and payload-less orphans.
 *     A todo with no status yet counts as unfinished.
 ...
 * all three tabs lived when this was written; `todos` feeds Schedule's Todo tab
 * since #411, so the keys are surfaces rather than one section's tab union.
 */
export interface MaterialsCounts {
  /** Incomplete todos (a todo that still needs doing = a meaningful number). */
  todos: number;
  notes: number;
  daily: number;
}
