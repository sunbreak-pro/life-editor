 ...
 *
 * The three statuses side by side is `TodoStatusChoices`, and it is the right
 ...
 * `useTodoTreeCRUD.toggleTodoStatus` has always used on the Todos side.
 *
 * The icons and the order come from `todoStatusVisuals`, so a status is drawn
 * the same here as on the Kanban board and the mobile todo list — the point of
...
export interface TodoStatusCycleButtonProps {
  status: TodoStatus;
  ...
  label: string;
  /**
   * Colour of the icon once the todo has left NOT_STARTED. Defaults to the app
...
export function TodoStatusCycleButton({
  status,
  onChange,
  labels,
  label,
  ...
  className,
}: TodoStatusCycleButtonProps) {
  const Icon = STATUS_ICON[status];
  return (
    <button
      type="button"
      onClick={() => onChange(nextTodoStatus(status))}
