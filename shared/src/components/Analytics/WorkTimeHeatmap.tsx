...
function getHeatmapColor(minutes: number, maxMinutes: number): string {
  if (minutes === 0 || maxMinutes === 0)
    return "var(--color-lumen-hover, #f3f4f6)";
      ...
      <div className="relative overflow-x-auto">
        ...
        >
          ...
          {Array.from({ length: 24 }, (_, h) => (
            <div
              key={`h-${h}`}
              className="text-[9px] text-lumen-text-secondary text-center leading-5"
            >
              {h}
            </div>
          ))}
          ...
          {DAY_KEYS.map((dayKey, dayIndex) => (
            <Fragment key={`row-${dayKey}`}>
              <div className="text-[10px] text-lumen-text-secondary flex items-center pr-1">
                {labels.days[dayKey]}
              </div>
        ...
        {tooltip && (
          <div
            className="fixed z-50 px-2 py-1 text-xs rounded-md bg-lumen-bg border border-lumen-border shadow-sm text-lumen-text pointer-events-none"
            style={{
              left: tooltip.x,
              top: tooltip.y,
