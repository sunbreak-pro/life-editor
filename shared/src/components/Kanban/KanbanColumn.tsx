 ...
 * badge) and a scrollable card stack. The accent color (status / tag hue)
 * rides on the `--kanban-col-accent` inline CSS var so the band, dot and count
 * badge all pick it up (user-data driven color — §6 permits inline CSS vars
 * for user colors).
 *
 * Status columns swap the dot for a lucide status icon; tag columns keep the
 * rounded dot.
 */
...
export function KanbanColumn({
  ...
  return (
    ...
    >
      ...
      <div className="relative border-b border-lumen-border bg-lumen-bg-secondary px-3.5 pb-2.5 pt-3.5">
        <span
          aria-hidden
