 ...
 * select intents. ink-* tokens, opaque panel (§6.4 / §5).
 */
export function BacklinkView({
  ...
  return (
    <aside className="flex flex-col h-full w-64 shrink-0 border-l border-ink-border bg-ink-bg">
      <div className="px-3 py-2.5 border-b border-ink-border">
        <div className="text-[11px] uppercase tracking-[0.14em] font-medium text-ink-text-secondary">
          {labels.title}
        </div>
        {targetLabel && (
          <div className="mt-0.5 text-[12px] truncate text-ink-text">
            {targetLabel}
          </div>
        )}
      </div>
      ...
      {entries.length === 0 ? (
        <div className="flex-1 flex items-center justify-center px-4 text-center text-[12px] text-ink-text-secondary">
          {labels.empty}
        </div>
      ) : (
          ...
          {entries.map((entry) => (
            <li key={entry.id}>
              <button
                ...
                className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-ink-hover text-left"
              >
                <ArrowLeft
                  size={11}
                ...
                <FileText
                  ...
                  className="text-ink-text-secondary shrink-0"
                />
                <span className="text-[12px] truncate text-ink-text">
                  {entry.label}
                </span>
              </button>
            </li>
