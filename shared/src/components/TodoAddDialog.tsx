/*
 * TodoAddDialog (W-UX) — the "add a todo" overlay for the Kanban. Pure
 ...
 * §frontend gotcha).
 *
 * life-tags S1: folders no longer group todos, so the todo/folder type toggle
 * and the "file under folder" select were retired — this dialog now only
 * captures a todo title (organizing is done via tags).
 */
...
export type TodoAddType = "task";

export interface TodoAddDialogLabels {
  /** Dialog title. */
  title: string;
  titleLabel: string;
...
export interface TodoAddDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (input: {
    type: TodoAddType;
...
export function TodoAddDialog({
  ...
  onSubmit,
  labels,
}: TodoAddDialogProps): React.JSX.Element {
