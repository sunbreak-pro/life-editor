...
export function useCalendarsAPI(options: UseCalendarsAPIOptions) {
  ...
  // Initial load + every syncVersion bump (mirrors routines/notes), through
  // the shared load effect (#672). Same three states as the hand-written
  // version it replaces, plus #296's error un-latch, which this hook was
  ...
  const { isLoading, error } = useDomainLoad({
    ...
    apply: setCalendars,
    fallbackMessage: "Failed to load calendars",
  });
