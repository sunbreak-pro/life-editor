...
/*
 * #569 — the Schedule section's todo-chip gestures are undoable.
 *
 * They all write through `updateNode`, which persists SILENTLY (no undo
 * command) because the Todos board saves titles and bodies through the same
 * call as the user types. So the fix is per-call opt-in, and these tests pin
 * both halves of it: the Schedule gestures push, and the Todos-side call next
 * to them still does not.
 *
 * The undo itself is asserted on the tree the provider hands its consumers —
 * the calendar chips are a pure derivation of that array (todosToCalendarChips
...
/** Never been near the calendar — what the tray's "add from todos" offers. */
 ...
 * (web/src/schedule/CalendarTab.tsx — handleTodoChipMove / Resize /
 * DropAllDay / handleTodoAddCandidate), plus the Todos-board title save that
...
function ChipProbe() {
  ...
  return (
    <>
      ...
      {/* Todos board title save — the path that must stay silent. */}
...
describe("Schedule todo-chip writes are undoable (#569)", () => {
  it("place: undo returns the chip to the all-day lane", async () => {
    const { ds, lastSynced } = makeTodoDS();
    await mount(ds);
  ...
  it("add to today: undo takes the todo back off the calendar entirely", async () => {
    const { ds, lastSynced } = makeTodoDS();
    await mount(ds);
    ...
    await click("undo");
    // Not "all-day on today with no time" — unscheduled. The todo has to leave
  ...
  // Regression guard for the Todos side (#569 DoD): the board saves a title on
  ...
  it("a plain updateNode (Todos board) still pushes nothing", async () => {
    const { ds, synced } = makeTodoDS();
    await mount(ds);
