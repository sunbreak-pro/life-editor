...
/*
 * #507 — the todo body's "[[" wiring. The graph half is what has logic worth
 * pinning: a resolved link becomes an item_links edge from the todo, so a link
 * written in a todo reaches Connect and the target's backlinks the same way a
 * link written in a note does.
 *
 * The three guards below each stand for a way the edge write can go wrong, and
 * they are the same ones useNoteLinking carries — this hook exists so the todo
...
describe("useTodoLinking edge mirroring", () => {
  it("writes an inline-origin edge from the todo to the link target", () => {
    link("task-1", "note-9");
