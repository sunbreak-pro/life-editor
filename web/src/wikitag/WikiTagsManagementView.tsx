  ...
  return (
    ...
    >
      <input
        ...
        className={`min-w-[8rem] flex-1 rounded-md border border-notion-border bg-notion-bg px-2 py-1 text-sm text-notion-text ${FOCUS_RING}`}
      />
      <button
        type="submit"
        className={`inline-flex items-center gap-0.5 rounded-md border border-notion-border bg-notion-bg px-2 py-1 text-xs text-notion-text hover:bg-notion-hover ${FOCUS_RING}`}
        aria-label="Create"
      >
...
function TagRow({
  ...
  return (
    <li className="flex items-center justify-between gap-2 rounded-md border border-notion-border bg-notion-bg-secondary px-2 py-1">
      {editing ? (
        <form
        ...
        >
          <input
            ...
            className={`flex-1 rounded-md border border-notion-border bg-notion-bg px-2 py-0.5 text-sm text-notion-text ${FOCUS_RING}`}
          />
        </form>
      ) : (
      ...
      <div className="flex gap-1">
        <button
          ...
          className={`text-notion-text-secondary hover:text-notion-text ${FOCUS_RING} rounded p-1`}
        >
        ...
        <button
          ...
          className={`text-notion-text-secondary hover:text-notion-danger ${FOCUS_RING} rounded p-1`}
        >
...
function GroupCard({
  ...
  return (
    <article className="space-y-2 rounded-md border border-notion-border bg-notion-bg-secondary p-2">
      <header className="flex items-center justify-between gap-2">
        {editing ? (
          ...
          <h3 className="flex-1 text-sm font-semibold text-notion-text">
            {group.name}
          </h3>
        )}
        <button
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {memberTagIds.map((tagId) => {
          ...
          return (
            <span
              key={tagId}
              className="inline-flex items-center gap-1 rounded-md border border-notion-border bg-notion-bg px-1.5 py-0.5 text-xs text-notion-text"
            >
              ...
              <button
                ...
                className={`text-notion-text-secondary hover:text-notion-danger ${FOCUS_RING} rounded`}
              >
      ...
      <div className="space-y-1">
        <input
          ...
          className={`w-full rounded-md border border-notion-border bg-notion-bg px-2 py-1 text-xs text-notion-text ${FOCUS_RING}`}
        />
            ...
            filteredCandidates.map((tag) => (
              <li key={tag.id}>
                <button
                  ...
                  className={`flex w-full items-center gap-1.5 rounded-md px-2 py-1 text-left text-xs text-notion-text hover:bg-notion-hover ${FOCUS_RING}`}
                >
                  {tag.color && (
                    <span
                      aria-hidden
...
export function WikiTagsManagementView() {
  ...
  if (wiki.loading) {
    return <p className="text-sm text-notion-text-secondary">Loading…</p>;
  }
