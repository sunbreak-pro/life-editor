 ...
 *   - opaque surface (ink-bg, no transparency on the panel — only the
  ...
  return (
    ...
    >
      <div
        ...
        className="w-full max-w-sm rounded-lg border border-ink-border bg-ink-bg p-5 shadow-xl"
      >
        <h2
          id={headingId}
          className="mb-4 text-base font-semibold text-ink-text"
        >
          {title}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <label className="block text-sm text-ink-text-secondary">
            ...
            <input
              ...
              className={`mt-1 w-full rounded-md border border-ink-border bg-ink-bg px-2 py-1.5 text-sm text-ink-text ${FOCUS_RING}`}
            />
          </label>
          ...
          {error && (
            <p id={errId} role="alert" className="text-sm text-ink-danger">
              {error}
            </p>
          )}
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button
              ...
              onClick={onClose}
              disabled={busy}
              className={`rounded-md border border-ink-border px-3 py-1.5 text-sm text-ink-text hover:bg-ink-hover disabled:opacity-40 ${FOCUS_RING}`}
            >
              {labels.cancel}
            </button>
            <button
              ...
              disabled={busy}
              aria-busy={busy}
              className={`rounded-md bg-ink-accent px-3 py-1.5 text-sm text-ink-on-accent hover:opacity-90 disabled:opacity-40 ${FOCUS_RING}`}
            >
              {labels.submit}
            </button>
          </div>
