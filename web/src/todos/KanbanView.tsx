 ...
 * editing. The host resolves each todo's tags from the WikiTags context
 ...
 * <RightSidebarPortal>, hosting the shared <TodoDetailPanel> + the web TipTap
 ...
 * MobileTodoList bottom sheet (#470 — mobile-scope.md #6 Phase 2), with the
 ...
 *
 * Layout: the full-width DnD board is the Todos view (the old list mode was
 * retired 2026-07-18 — Board only). Cards + @dnd-kit + card-click → the
 * selected todo's detail in the rightSidebar. The grouping axis (viewMode:
...
export function KanbanView({
  ...
  // renderTodoDetail, so wide and the mobile sheet get it from one place.
   ...
   * inline in `renderTodoDetail` because that helper runs during render, and a
  ...
  // Add-todo dialog (W-UX). The board had no create entry point; this small
  // centered overlay creates a todo, then opens it straight into the detail
  // modal.
  //
  // The shell's global:new-task intent (pendingNewTodo) opens this dialog on
  // the wide board (TodoAddDialog auto-focuses its title input and creates the
  // todo on submit via the TodoTree provider) — the app's own create-and-focus
  ...
  // the Todos tab → the guarded during-render tracker below. Both derive state
  ...
  // while rendering" pattern. Narrow relies on the MobileTodoList quick-add.
  ...
  const columns = useMemo<KanbanColumnModel[]>(() => {
    switch (viewMode) {
      case "status":
        return buildStatusColumns(tree.nodes, labels, tagsByTodo);
      case "tag":
        return buildTagColumns(tree.nodes, tags, tagsByTodo, labels);
    }
  }, [viewMode, tree.nodes, labels, tags, tagsByTodo]);
  ...
  const statusColumns = useMemo<KanbanColumnModel[]>(
    () => buildStatusColumns(tree.nodes, labels, tagsByTodo),
    [tree.nodes, labels, tagsByTodo],
  );
  ...
  // K3 (target-IA) — the selected todo's detail now lives in the shared
  // rightSidebar panel (Desktop) instead of a centered modal. narrow shows the
  // same panel inside the MobileTodoList sheet (#470), so the portal is
  // wide-only.
  const selected = tree.selectedTodo;

  /*
   * One panel, two surfaces. Everything except the status control is identical,
   * so building it once keeps a field added to the todo detail from reaching
   * only one width.
   *
   ...
   */
  const renderTodoDetail = (todo: TodoNode, statusControl?: ReactNode) => (
    ...
    <div className="flex flex-col gap-3">
      ...
      <TodoBodyDraft key={todo.id} onSave={saveTodoDetail}>
        {(draft) => (
          <TodoDetailPanel
            todoId={todo.id}
            title={todo.title}
            ...
            contentEditor={
              <RichTextEditor
                ...
                onResolvedLinkInserted={(targetId) =>
                  handleResolvedLinkInserted(todo.id, targetId)
                }
              />
            }
          />
      ...
      {dataService && (
        ...
        <button
          type="button"
          onClick={() => handleConvertFromDetail(todo)}
  ...
  // Desktop: the selected todo's detail, pushed into the rightSidebar on
  ...
  const todoDetail = isWide && selected ? renderTodoDetail(selected) : null;
  ...
  const mobileTodoDetail = sheetTodo
    ? renderTodoDetail(
        sheetTodo,
        <TodoStatusChoices
          value={sheetTodo.status ?? "NOT_STARTED"}
          onChange={(status) => tree.setTodoStatus(sheetTodo.id, status)}
          labels={labels}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      {isWide && selected && (
        <RightSidebarPortal>{todoDetail}</RightSidebarPortal>
      )}
