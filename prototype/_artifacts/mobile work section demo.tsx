...
const INITIAL_PRESETS: Preset[] = [
  { id: 'classic', name: 'Classic', workMin: 25, breakMin: 5, longBreakMin: 15, sessionsBeforeLongBreak: 4 },
  { id: 'long-focus', name: 'Long Focus', workMin: 50, breakMin: 10, longBreakMin: 20, sessionsBeforeLongBreak: 3 },
  { id: 'short-burst', name: 'Short Burst', workMin: 15, breakMin: 3, longBreakMin: 10, sessionsBeforeLongBreak: 4 },
];
...
const MOCK_TASKS: Task[] = [
  ...
  // personal (7)
  { id: 't14', title: '読書(Clean Architecture)', folder: 'personal', done: false },
...
// Root
// =============================================
export default function MobileWorkSectionDemo() {
  ...
  const [presetSwitchPending, setPresetSwitchPending] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sessionType, activePreset.id, activePreset.workMin, activePreset.breakMin, activePreset.longBreakMin]);
  ...
  function handleSessionEnd(manual: boolean) {
    ...
    const startedAt =
      sessionStartedAtRef.current ?? new Date(Date.now() - durationSec * 1000).toISOString();
  ...
  function tryActivatePreset(presetId: string) {
    ...
    if (isRunning || remainingSec !== getPlannedSec(activePreset, sessionType)) {
    ...
    >
      ...
      >
        ...
        <div className="text-center">
          ...
          <div className="text-[11px] mt-1 uppercase tracking-widest" style={{ color: C.overlay0 }}>
            フォーカス
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto pb-24">
        ...
        {subTab === 'history' && (
          <HistoryTab sessions={completedSessions} />
        )}
      ...
      {completionModalOpen && completedSessionMeta && (
        <SessionCompletionModal
          ...
          nextIsLongBreak={completedSessionCount % activePreset.sessionsBeforeLongBreak === 0}
  ...
  return (
    <div className="flex flex-col px-5">
      ...
      >
        ...
        <div className="flex-1 text-left text-sm truncate" style={{ color: currentTask ? C.text : C.overlay0 }}>
          {currentTask ? currentTask.title : 'タスク未選択 (フリーセッション)'}
        </div>
        {currentTask && (
          <span
            className="text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded"
            style={{ background: FOLDER_COLOR[currentTask.folder] + '33', color: FOLDER_COLOR[currentTask.folder] }}
          >
      ...
      <div className="flex flex-col items-center mt-2">
        ...
        >
          <svg width={260} height={260} className="-rotate-90">
            <circle cx={130} cy={130} r={R} fill="none" stroke={C.surface0} strokeWidth={10} />
            <circle
              cx={130}
              cy={130}
              r={R}
              ...
              style={{ transition: 'stroke-dashoffset 1s linear, stroke 300ms ease-out' }}
            />
          </svg>
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            {autoStartCountdown !== null ? (
              <>
                <div className="text-7xl font-light tabular-nums" style={{ color: arcColor }}>
                  {autoStartCountdown === 0 ? 'GO' : autoStartCountdown}
                </div>
                <div className="text-[10px] uppercase tracking-widest mt-2" style={{ color: C.overlay0 }}>
                  AUTO START
                </div>
              </>
            ) : (
              <>
                ...
                <div className="text-[11px] uppercase tracking-widest mt-2" style={{ color: C.overlay0 }}>
                  {isRunning
                    ? 'IN PROGRESS'
                    : remainingSec === 0
                    ? 'COMPLETED'
        ...
        <div className="flex items-center gap-3 mt-6">
          <button
            ...
            style={{ borderColor: C.surface1, color: C.subtext1, background: 'transparent' }}
            aria-label="リセット"
          >
            <RotateCcw size={18} />
          </button>
...
function HistoryTab({ sessions }: { sessions: CompletedSession[] }) {
  ...
  const recent = sessions.filter((s) => new Date(s.completedAt) >= sevenDaysAgo);
  ...
  return (
    <div className="px-5 space-y-6">
      {/* Summary cards */}
      <div>
        <div className="text-[10px] uppercase tracking-widest font-semibold mb-2" style={{ color: C.overlay0 }}>
          直近 7 日間
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-xl p-4" style={{ background: C.surface0 }}>
            <div className="text-[10px] uppercase tracking-wider" style={{ color: C.overlay0 }}>
              累計時間
            </div>
            <div className="text-2xl font-bold mt-1 tabular-nums" style={{ color: C.green }}>
              {formatHM(totalSec)}
            </div>
          </div>
          <div className="rounded-xl p-4" style={{ background: C.surface0 }}>
            ...
            <div className="text-2xl font-bold mt-1 tabular-nums" style={{ color: C.mauve }}>
              {workCount}
            </div>
          </div>
        </div>
...
function SessionListRow({ session }: { session: CompletedSession }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] tabular-nums w-10 flex-shrink-0" style={{ color: C.subtext0 }}>
      ...
      <span className="flex-1 text-xs truncate" style={{ color: C.text }}>
        {session.taskTitle ?? <span style={{ color: C.overlay0 }}>フリーセッション</span>}
      </span>
  ...
  return (
    <div className="px-5 space-y-6">
      ...
      <div>
        ...
        {/* Sliders */}
        <div className="rounded-xl p-4 mt-3 space-y-4" style={{ background: C.surface0 }}>
          <SliderRow
            label="作業時間"
...
function TaskPickerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: C.base }}>
      {/* Header */}
      <div
        className="flex items-center justify-between px-5 pt-6 pb-3"
        style={{ background: C.base }}
        ...
        <div className="space-y-3">
          {folders.map((fk) => {
            ...
            return (
              <div key={fk}>
                <button
                  onClick={() => setExpanded((prev) => ({ ...prev, [fk]: !prev[fk] }))}
...
function SessionCompletionModal({
  ...
  meta: { sessionType: SessionType; taskTitle: string | null; durationSec: number };
      ...
      >
        ...
        {meta.taskTitle && (
          ...
          >
            ...
            <div className="text-sm font-medium mt-0.5" style={{ color: C.text }}>
              {meta.taskTitle}
            </div>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            style={{ background: 'transparent', border: `1px solid ${C.surface1}`, color: C.subtext1 }}
          >
            <RotateCcw size={16} />
            もう一度同じセッション
          </button>
    ...
    >
      ...
      >
        ...
        <input
          ...
          style={{ background: C.surface0, color: C.text, border: `1px solid ${C.surface1}` }}
        />
