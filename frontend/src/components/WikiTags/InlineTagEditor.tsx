...
export function InlineTagEditor({
  ...
  return createPortal(
    <div
      ref={popupRef}
      className="fixed z-[9999] bg-notion-bg border border-notion-border rounded-lg shadow-lg w-56 overflow-hidden"
      style={{ top: position.top, left: position.left }}
    >
      {entityTags.length > 0 && (
        <div className="p-2 border-b border-notion-border flex flex-wrap gap-1">
          {entityTags.map((tag) => (
            <span
              key={tag.id}
