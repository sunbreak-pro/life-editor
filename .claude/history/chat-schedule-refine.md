...
### 2026-09-22 - schedule 9 本の PR のコンフリクトを解消し、main の赤に追随した

#### 概要

提出済みの 9 本が全部 CONFLICTING に転落していたので、main を取り込んで解消しました。衝突は 9 本とも `shared/src/components/Analytics/chartTheme.ts` の 1 ファイルだけで、原因は復旧の二重取りです。さらに main 自体が別の型エラーで赤かったため、既存の修理 PR #1945 の 1 commit を 9 本に載せました。9 本とも MERGEABLE + CI 緑で、merge はユーザーです。

#### 変更点

- **衝突の解消**: 提出時に 9 本へ配った「`chartTheme.ts` の末尾に `estimateLabelWidth` / `fitAxisLabel` を戻す」commit と、同じ復旧を入れた main 側の PR #1942 が、同じ末尾を両側から足していました。main 版は各ブランチ版と同一ブロックを含み `evenDateTicks` も持つ上位互換だったため、`git checkout origin/main -- shared/src/components/Analytics/chartTheme.ts` で main 側を丸ごと採りました。解消後の各ブランチの差分は、それぞれの Issue のファイルだけです
- **main の赤への追随**: `StreakDisplayLabels.days`（文字列）が `formatDays`（関数）へ変わり、`shared/tests/briefingHeadingSpacing.test.tsx` だけが `days: "days"` のままで `typecheck:tests` が TS2353 で落ちていました。修理 PR #1945（1 行）が既に open だったので、その commit `45f7617c` を 9 本に cherry-pick しました
- **取り込む main のずれ**: 1 本目を処理した後に、他 worktree の fetch で `origin/main` が `4b89dd9c` → `32465e76` へ進みました。最初の 1 本だけ古い main を取り込んでいて CI が落ちたので、取り込み直しました
- **検証**: `shared` の build / typecheck:tests と `web` の typecheck:tests をローカルで緑にし、GitHub 側は 9 本とも `typecheck + test + build` と `docs-lint` の両方が SUCCESS です
- **PR**: #1904 / #1908 / #1914 / #1919 / #1925 / #1927 / #1929 / #1933 / #1935（すべて open・MERGEABLE・CI 緑）。tracker の PR #1938 も main を取り込み済み
...
指示された 4 件を指定順に 1 Issue = 1 ブランチ = 1 PR で出した。4 本とも `origin/main` から独立に切り、**1 本ずつローカルで CI `verify` の全ステップを緑にしてから** PR を開いた（#1642 は docs を触るので `docs-lint` も = 15/15）。セッション終了時点で #1813 / #1816 / #1817 は merge 済み、#1818 が open。

| Issue     | PR    | 内容                                                       |
| --------- | ----- | ---------------------------------------------------------- |
| #1801     | #1813 | 繰り返しの「この回以降」を Undo に載せ、ダイアログで明示   |
| #1802     | #1816 | 祝日の非表示を設定として保存し、Settings に行を足す        |
| #1800     | #1817 | タグの Undo 配線を 5 Context と同じ場所へ + ambient の守り |
| #1642 W16 | #1818 | 複製がリマインダーを写し、落ちたら報告する（K-07 / K-10）  |

#### 変更点

- **#1801 は undo を opt-in にした**: 同じ `detachRoutine` を呼ぶ入口が 2 つあり、エディタの「繰り返し = なし」は**開いている回を pin してそれを繰り返しに戻す**自前の逆操作を既に持っている。既定 ON にするとその経路が 2 回 push する。スコープダイアログ側は何も pin しないので、逆操作は「ゴミ箱の回を戻す → 繰り返しを戻す」の順（`deleteRoutine` の undo と同じ順序。繰り返しを先に戻すと生成器が live な行を見つけられず新しい id を打つ）
- **裁定の条件はコードではなく文言だった**（D-20260919-sched-2 = B）。`RepeatScopeDialog` の `labels` に optional な `thisAndFutureNote` を足し、「この予定と今後の予定」の下にだけ出して `aria-describedby` で読み上げに繋いだ。**ダイアログ全体に出すと他 2 つのスコープについて嘘になる**（あちらはきれいに戻る）。削除モード限定にしたのも同じ理由で、編集側の「この回以降」はタグを動かさない
- **#1802 は「他 2 フィルタと何が違うか」を書くのが本体**だった。保存しない理由（復元すると空いて見えるだけの枠ができる）が祝日には届かない — 隠すのが利用者の作れない行だから。この段落をフックのヘッダーに足すのが DoD の項目そのもの
- **#1800 は調査で前提が崩れた**: やること 1・2 は #1667（PR #1697・2026-09-19）で着地済みだった。残っていたのは「5 Context と形を揃える」だけで、**そこを触って初めて ambient 経路に守りが 1 本も無い**と分かった（既存 4 ケースは全部 `undoRedo` prop を明示注入していて、ambient の拾い方を壊しても全緑）。`undoRedoDomainWiring.test.tsx` に 6 本目のドメインとして 2 ケース足した
- **#1642 は W14 / W15 が chat-main の手番なので、§1-K の残りから 1 単位を切った**。K-07（複製がリマインダーを落とす）と K-10（落ちた複製が幽霊行を残す）は同じ関数の 2 つの穴で、他 3 本が触るファイルと重ならない。**K-06（タグの引き継ぎ）は P-008 で記録にとどめた** — タグの書き込み経路は本計画の Scope 外で、#1800 がその層を Undo に載せた以上、複製が n 個の付与を撃つと「1 押下 = 1 エントリ」が割れる

#### 検証

- 4 本とも CI `verify` 14 ステップ緑。`npm ci` の 4 ステップだけ省いた（lockfile が `origin/main` から無変更・`node_modules` 在中）
- **4 本とも「壊して赤になること」を実測した**（修正を `origin/main` の版に戻して再実行）。#1801 = 3 種の mutation で 6/7・1/9・2/29 / #1802 = 2 種で 1/4・2/5 / #1800 = 1 種で 2/17 / #1642 = 2 種で 3/26・1/26
- **#1800 の 1 周目は `shared — typecheck:tests` だけが赤**だった（テストで `userId` という存在しない prop を渡していた）。`build` も `vitest` も素通りする独立ゲートで、CLAUDE.md §7.1 が名指ししているとおりの穴

#### 次

- merge はユーザー。**実ブラウザ確認は merge 後 chat-main**。#1801 = 「この予定と今後の予定」で削除 → Ctrl+Z → 繰り返しが戻り**タグは戻らない** / #1802 = 隠す → リロード → 隠れたまま / #1800 = 4 面それぞれでタグ付与 → Ctrl+Z / #1642 = リマインダーを変えた予定を複製
- §1-K の未着手は K-03 / K-05 / K-09 / K-11 / K-13 / K-16 の 6 件（計画書の表に記載済み）。Issue 化は chat-main の手番
...
#1747（クリックパネルの時刻変更でツアー 3/10 が進まない）を 1 行で直し、#1642 の計画書を W14（実ブラウザ検証）の前に同じ 7 軸で読み直して追補した。2 本とも `origin/main` から独立に切った別ブランチ。

#### 変更点

- **#1747 の原因は「書き込みは通るが副作用だけ落ちる」形**: #1664 が足したバブルの `onRetime` が素の `handleUpdate` を呼び、ツアーへ `TOUR_ACTIONS.scheduleEventTimeChanged` を投げるラッパ `handleUpdateReported` を通っていなかった。予定は正しく動くので画面上は成功に見え、Edit details 経由の Save だけがステップを進める。スコープダイアログ・undo 登録・楽観更新は同じハンドラ由来なので、直したのは通知だけ
- **テストは「今日の配線」ではなく「次に足される経路」を狙う形にした**: `onRetime` がラッパを通ることを名指しするだけでは、同型の 3 本目が足されたときに何も落ちない。**素の `handleUpdate(` の出現数が 1（ラッパ自身の委譲）であること**を固定した — `handleUpdateReported(` は続く文字が `R` なので同じ正規表現に当たらず、数えられる
- **`CalendarTab` はソーステキスト assert が唯一の手**（`rules/frontend.md` §テスト環境の制約 / `D-20260812-refactor-2`）: Provider 一式 + 実レイアウトが要るので web の test は 1 本も render していない。加えてバブルは**配置済みブロックのクリック**から開くため、jsdom にレイアウトが無い以上「座標に依存しない形」でも実経路は駆動できない
- **偽陽性の否定を必ず取る**: `git show origin/main:<file>` を temp に出し、同じ正規表現を node で当てて `false` / 2 件になることを確認した。ソーステキスト assert は書いた瞬間に緑になりやすいので、これが無いと「通っているのか、当たっていないのか」が分からない
- **#1642 は新規計画書を作らなかった**: /goal の指示は `2026-09-19-schedule-refactor.md` の新規作成だったが、工程 1 は 2026-09-16 に PR #1653 で merge 済み（`git log -- <plan>` で判明）で、工程 2 も W0〜W12 が着地していた。2 本目を作ると正本が割れるので**既存の `2026-09-16-schedule-refactor.md` への追補に切り替えた**。判断は PR 本文の冒頭に明示して差し戻せるようにした
- **§1-K を新設（16 件）**。W0〜W12 と 7 本の機能 PR が着地したあとの現状を同じ 7 軸で読み直し、既存 ID と重ならないものだけを並べた。**意図された差 3 件は載せずに落とした**（ノート併用時の配置 Undo・Todo 削除の確認ポリシー差・日跨ぎドラッグのスコープ省略）— どれもコード側のコメントが理由を書いており、棚卸し表に載せると「直すべきもの」に見える
- **B-14 = undo の二重トースト**（shared-fix レーンの #1681 工程 1 の棚卸しが出典）: `useItemConversion.ts` と `useCreatePanelNotes.ts` の undo クロージャが catch で失敗トーストを出したあと**正常終了する**ため、`UndoRedoManager.apply` は成功と見て `UndoRedoHost` が「元に戻しました」を重ねる。しかも失敗したコマンドが redo 側へ移る。**B-10 の裏返しで、W5 が Provider 側だけを直したのですり抜けた**。当該 4 箇所はこちらで読んで確認済み
- **AC の行数条件が未達だったので自己免除せずキューへ**（P-008 / D-20260919-sched-6）: `CalendarTab.tsx` は「1,239 行を超えない」に対し 1,375 行。ただし `git log --numstat` を merge 単位で引くと、**W8/W9/W10/W12 をまとめた PR #1684 は -1 行**で、+137 行は #1638 / #1639 / #1640 / #1641 / #1626 / #1664 / #1678 の 7 本が足したもの。**リファクタの成果は機能追加に食われて見えなくなる** — 行数の AC を書くなら「どの PR 群での増減か」を併記しないと、達成したのに未達に見える
- **§5-2 で open Issue の実態を引き直した**: 2026-09-16 に「後に回す・W15 まで着手しない」とした 4 件（#1639 / #1640 / #1641 / #1626）は全部 2026-09-19 に merge されていた。**衝突は 1 件も起きていない**（どれも W8 / W11 の merge 後に着地）。未着手の open は #1747 / #1663 / #1678 の 3 件
- **確度の区別を守った**: K-01 / K-06 / K-11 / K-12 と B-14 はメインが該当行を直接読んで裏を取り、残りは並列調査の報告をそのまま採った（`rules/docs-consistency.md` §5）。**実ブラウザ / 実 DB での再現は 1 件も無い**ことを表の外に明記した — W14 の担当分
- **検証**: #1752 は CI `verify` 全ステップ + `docs-lint` = 15/15 緑（ラッパを `( npm run X | tail )` にせず、出力を変数へ取って `code=$?` を見る形）。#1756 は docs のみで `git diff --stat` にコードが出ないため `docs-lint` 緑まで
