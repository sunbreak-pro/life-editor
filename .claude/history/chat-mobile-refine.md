...
### 2026-09-01 - #1400 / #1402 モバイルドロワーの幅ピン留めとスワイプ判定拡張（PR #1416 / #1420 open）

#### 概要

`shared-fix [mobile-refine]` の open 2 件を、それぞれ `origin/main` から切った独立ブランチで実装して PR まで出した。どちらも `shared/src/components/MobileDrawer.tsx` 起点。CI `verify` ジョブのステップを上から全部ローカルで緑にし、`docs-lint` も通してから push（merge は P-001 でユーザー）。

#### 変更点

- **#1400 → PR #1416（open）**: 幅が `w-80` = 20rem で、外観設定が root の font-size を書き換える（`ThemeContext`）ため文字サイズに追従していた（小 280px / 中 360px / 大 440px = 端末より広い）。中プリセット由来の px（`20 * fontSizeToPx(DEFAULT_MOBILE_FONT_SIZE_STEP)` = 360px）をインライン style で固定した。**issue 外の追加が 1 つ = `max-w-[85vw]`** — 360px 固定は 360px 幅の端末を端まで覆って scrim のタップ余地が消えるため（小プリセットが偶然与えていた逃げ道）。PR 本文で明示した
- **#1402 → PR #1420（open）**: 開く側 = `useEdgeSwipeOpen` のエッジ帯を 24 → 44px。Android のジェスチャーナビが外側 20〜24px を自分の戻る操作に取るので、指が実際に置ける幅がほとんど無かった。閉じる側 = `useSwipeToDismiss` のハンドラを scrim にも spread し、パネルの外から始めた左スワイプでも閉じるようにした（scrim は元からタップで閉じるので、奪うものが無い）
- **テスト**: 幅 3 本（`mobileDrawer`）/ エッジ帯の内外 2 本（`edgeSwipeOpen`）/ scrim 発 4 本（`swipeToDismiss`）

#### 手順・知見

- **1 つのフックを 2 要素へ spread しても dismiss は二重発火しない**。`useSwipeToDismiss` の release は `startRef` を読んでから `reset()` するので、パネル発の press がパネル → scrim の順に両方を通っても、scrim 側は既に null を見る。既存のパネル発テストがそのままこの経路を踏むため、追加の「1 回だけ」テストは書かなかった
- **jsdom はレイアウトも stylesheet も持たない**ので rem 追従そのものは測れない。幅を Tailwind クラスではなくインライン style にしたのは `style.width` で読み戻せるようにするため（`bottomSheetSafeArea` のクラス断言と同じ発想）
- 2 本とも `MobileDrawer.tsx` を触るが hunk が離れている（幅 = className / style 行、判定 = ヘッダーコメントと scrim の属性）ので、どちらが先に merge されても解けるはず
