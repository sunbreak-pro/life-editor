...
2026-09-21 の画面別探索検証で起票された Work の 4 件を、それぞれ `origin/main` から切ったブランチで直して PR を開いた。4 本とも open（merge = 人手 P-001）。#1857 / #1858 / #1793 は指示どおり触っていない。

#### 変更点

- **#1853（sev:blocking）→ PR #1879**: 一時停止 → 再開 → 完走したセッションが記録されなかった。pause が行を閉じ、再開は行を開かず、完了時は閉じる行が無いまま return していた。**走っている区間ごとに 1 行**へ変更（pause でその区間の秒数を書いて閉じ、再開で新しい行を開き、目標に届いた行を `completed = true` で閉じる）。1 行を開いたままにする案は、停止中にタブを閉じると時間が残らない点と、行の開始〜終了の幅（フリーセッションの予定の元）に停止時間が混ざる点で不採用。`isCountedSession` は不変、Reset は記録を取り消さない（#1475）をテストで固定
- **#1853 の完了モーダル**: `workDurationMinutes`（名目の長さ）ではなく、新設の `lastLoggedWorkSeconds`（完了した WORK のうち集計に数えられる行の合計秒）を読む。+5 分で延ばした回も実績どおりに出る
- **#1853 の副作用**: 作業名なしの WORK を一時停止すると、数えられる区間ごとにフリーセッションの予定が 1 件できる（#1665 の「1 行 1 予定」を、これまで欠けていた行にも当てた結果）
- **#1854 → PR #1881**: 完了後の idle な休憩で Skip が無効だった。`PomodoroTimer` の Skip から `disabled={isIdle}` を外した（Reset は据え置き）。idle の `setPhase` は開いている行が無ければ何も書かないことを Provider のテストで固定
- **#1855 → PR #1885**: Desktop の選択チップに `min-w-0 max-w-full` を足した。チップが flex の子で、最小幅が中身の幅になっていたのが原因
- **#1856 → PR #1888**: `WorkTagSelector` に `sheet` prop を足し、Mobile のタグ行だけが渡す。ポップオーバーの代わりに BottomSheet で開き、トリガー・入力欄・行に 44px の下限が付く。Desktop は不変（テストで固定）。i18n に `work.freeSession.tagSheetClose` を追加
- **検証**: 4 ブランチそれぞれで CI `verify` のステップ列をローカル全通し（shared / web / desktop / mcp-server）、すべて exit 0。`npm ci` は回し直していない
- **未検証**: 実ブラウザ確認は chat-main の担当（§7.4）。#1855 の 1440px での rect 実測と #1856 の 390px での実測は DoD に残っている。#1853 は DB に 2 行できて 2 行目が `completed = true` かの確認
