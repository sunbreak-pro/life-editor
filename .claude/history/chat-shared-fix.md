...
### 2026-09-22 - [shared-fix] 完全削除後の Undo と、残りの塗り disabled ボタン

#### 概要

/goal で渡された 2 件を 1 件 1 ブランチで PR まで出しました。**PR #1910（#1876）** と **PR #1913（#1803）** で、どちらも open です（merge は P-001 でこうだいさんの手番）。

#### #1876 — 完全削除したアイテムの Undo が無言で空振りする

undo 履歴のコマンドは行の id を掴んだ関数です。#1727 以降、id で行を名指すコマンドはセクションを移っても残るので、Settings のゴミ箱で完全削除した行のコマンドが履歴に残り続けていました。押すとコマンドは走り、更新が 1 件にも当たらないまま成功して、画面は動かないのに「元に戻しました」のトーストだけが出ます。Redo が有効になるのが唯一の反応でした。リスト全体を書き戻す種類のコマンドはもっと悪くて、消したはずの行を DB に戻します。

直したのは `web/src/trash/TrashScreen.tsx` の 1 ファイルで、完全削除が通ったら undo 履歴を捨てます。1 行ずつの削除と、1 行でも実際に消えた一括削除が対象です。全部が失敗した一括削除は何も消えていないので履歴を残します。

- **捨てるのは履歴全部**にしました。コマンドが名乗るのは押した領域（todoTree / note など）だけで、どの行を触るかは持っていないため、消えた行の分だけを選べません。Todos 側は前から同じ答えを出していて（`useTodoTreeDeletion.permanentDelete` が履歴を消します）、ゴミ箱の確認ダイアログも先に「この操作は取り消せません」と言っています
- **context の値は ref 経由で読みます**。値の同一性は履歴が動くたびに変わるので、ハンドラの依存配列に入れると push のたびに作り直しになります。読むのはクリックの await が解けた後だけです
- テストは `web/tests/trashScreenActions.test.tsx` に 4 本足しました。1 行の完全削除で両方向が消えること、復元では消えないこと、一括削除で 1 行でも消えれば消えること、全滅した一括削除では消えないこと

#### #1803 — 残りの塗り disabled ボタンを 1 つの書き方へ

起票が 2026-09-05 なので、7 箇所すべてを `origin/main` で測り直してから着手しました。7 箇所とも古いままで、直っていたのは Issue が予想したとおり `AudioMixer` と `PomodoroSettings` だけでした（`AudioMixer` に残っていた行はボタンではなく音量のスライダーです）。

寄せたのは `Button` の `danger`、予定の編集ペインの保存、作成パネルの送信、タグフィルタの「グループを保存」、Kanban の追加ダイアログの送信、Todo 詳細の保存、ノートのパスワードダイアログの送信です。`DISABLED_FILLED_BTN` は 7 つ目が `web/` にあるので shared のバレルから export しました。

- **`danger` を動かせた条件は #1804**（PR #1811）です。`danger` の無効は 3 画面で「処理中」の意味を兼ねていて、灰色に沈めると「止まっている」と読めます。#1811 がその 3 画面にスピナーと「…中」のラベルを入れたので、処理中であることが塗りに依存しなくなりました
- **枠線ボタンは触っていません**。塗りが無いので薄くするだけで意味が通り、沈めると無かった枠を描くことになります。判断した結果として触っていないことが読めるよう、寄せたボタンの隣にある 4 箇所（作成パネルの「追加して編集」、タグフィルタの「適用」、ポモドーロのプリセット保存、パスワードダイアログのキャンセル）にコメントを置きました。作成パネルは 2 つのボタンが 1 つの文字列を共有していたので、塗り用と枠線用に分けました
- **`Button.tsx` のコメントを実態に合わせました**。「`primary` だけが `DISABLED_FILLED_BTN` にいる」「`danger` は保留」の 2 つとも古くなっていました
- テストは `buttonDisabledFill.test.tsx` の「`danger` は従来どおり」という 1 本を反対の主張に差し替え、5 つの suite に 1 本ずつ足しました。2 本では隣の枠線ボタンが薄くなるままであることも確かめています

#### 検証

CI の `verify` ジョブと `docs-lint` をローカルで上から順に回しました。**main がこの 2 本と無関係に赤い**ため、結果は 2 つに分かれます。

緑: `shared` の lint、`web` の lint / typecheck:tests / test、`desktop` の typecheck / test、`mcp-server` の 3 ステップ、`docs-lint`。

赤: `shared` の build / typecheck:tests / test と `web` build と `desktop` build の 5 つです。どれも報告するのは `chartTheme.ts` の `fitAxisLabel` と `TodosTab.tsx` の `sessionsWithinRange` の 2 記号だけで、これは merge の解決で落ちたものです。main 自身の CI（3d742107）も同じ理由で落ちていて、直す PR が #1905 / #1906 / #1907 と 3 本出ています。自分の変更が足したものが無いことは、`shared` の build と typecheck:tests がこの 2 記号以外を 1 件も出さないこと、`shared` の test がブランチの有無にかかわらず同じ 6 ファイル / 17 件で落ちることで確かめました。

#1876 のブランチでは、直す PR の 1 本を作業ツリーに取り込んで残りが緑かを確かめようとしましたが、システムがメモリ不足でそのバックグラウンド実行を止めました。**止められたスクリプトは後片付け（`git merge --abort`）まで進まないので、MERGE_HEAD と他人の変更が staged のまま worktree に残ります**。そのまま commit していたら無関係の 2 ファイルが PR に載っていました。背景タスクが killed で終わったら、まず `git rev-parse -q --verify MERGE_HEAD` と `git status --porcelain` を打つことです。

#### main の破損を 2 本に取り込んだ（同日・こうだいさんの指示）

「赤くなっているため修正して」と言われたので、`fix/shared-todostab-missing-import` の 2 ファイルを PR #1910 と #1913 に 1 コミットずつ載せました。取り込む前に 3 本の修理 PR を突き合わせ、**#1905 と #1906 は tree が完全一致、#1907 はブロックの並び順だけが違う**ことと、`chartTheme.ts` の中身が #1893 が出した版（15f6319f）と同一であることを確かめています。採ったのは main に対して純粋な追加になる版です。

取り込み後、両ブランチで CI の 15 ステップ + `docs-lint` をローカルで上から回して全部 exit 0、GitHub 側の CI も緑になりました。どれか 1 本が先に入れば、この 1 コミットは squash で差分ゼロになります。

tracker の PR #1917 には載せていません。記録のブランチが実装と merge 順から独立していることがその価値なので、赤のまま置いて本文に理由を書きました。#1910 か #1913 が入れば緑になります。
