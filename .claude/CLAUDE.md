...
- **worktree 横断の共有修正タスク（2026-07-10〜）**: 正本 = GitHub Issues の label **`shared-fix`**（宛先 = タイトル prefix `[<worktree-slug>]` / `[all]`）。各 worktree チャットはセッション開始時と作業の区切りに `gh issue list -R sunbreak-pro/life-editor --label shared-fix --state open` で自分宛を確認する（運用詳細 → [`comm/README.md`](./comm/README.md) §Shared-fix ルート）
