...
> ⚠️ **Active Migration (2026-05-04 開始)** — `refactor/web-first-v2` ブランチで Tauri 2 + Cloudflare D1 + portable-pty → **Electron + Capacitor + Web + Supabase** への大規模移行が進行中。SSOT は [`docs/vision/plans/2026-05-04-cross-platform-migration.md`](./docs/vision/plans/2026-05-04-cross-platform-migration.md)。本ファイル §1-7 は現行 Tauri 構成を記述しており、Phase 5 完了時に全面改訂される。現在は Phase 0（学習スパイク `~/dev/learning/applications/_spikes/web-first-spike-1/`）。
...
- **関連ドキュメント**: `MEMORY.md`(タスク) / `HISTORY.md`(履歴) / [`docs/vision/plans/2026-05-04-cross-platform-migration.md`](./docs/vision/plans/2026-05-04-cross-platform-migration.md)(移行 SSOT) / `docs/vision/`(設計原則) / `docs/vision/plans/`(実装プラン) / `docs/requirements/`(Tier 1-3) / `docs/known-issues/`(Root Cause [INDEX](./docs/known-issues/INDEX.md)) / `docs/code-explanation/` / `archive/`
...
### Desktop vs Mobile 設計思想

- **Desktop**: **クリエイティブ重視**。フル機能で「作る・編集する」を最大化
- **Mobile**: **コンパクト重視**。機能を絞り「消費・素早い記録」に最適化。Desktop の縮小コピーではなく、Mobile 専用にレイアウト・導線を再設計する
- **Mobile 必須セクション（この 4 つのみ）**:
  - **Schedule**: 予定（イベント） / タスク / ルーティン
  - **Work**: 標準ミュージックのみ（カスタム音源追加は Mobile では非対応のまま）
  - **Notes**: デイリー / ノート
  - **Settings**
- **Mobile でも快適にすべき共通機能**: スラッシュコマンド・タグ付け（Desktop と同等の UX を目標。コンパクト UI 内でも 1〜2 タップで到達できるようにする）
...
### 3.3 Section Routing（7 SectionId、可視 6 + ターミナル状態 1）

React Router なし。`App.tsx::activeSection` で切替: `schedule` / `materials` / `connect` / `work` / `analytics` / `settings` / `terminal`。`terminal` は描画上 `return null` のプレースホルダで、実体の `TerminalPanel` は全画面共通の下部パネル（VSCode 方式）として独立 mount される。

### 3.4 サブシステム

- **Terminal**: `portable-pty` (Rust) + `xterm.js`、Ctrl+`` ` `` 開閉、Catppuccin Mocha
- **Audio Mixer**: 5 種環境音（rain / wind / ocean / birds / fire）+ カスタム。`AudioContext` は `suspended` → ユーザー操作後 `resume()` 必須
...
- 正本: `src-tauri/src/db/migrations/`（WAL）/ 現行 `LATEST_USER_VERSION = 70`、約 40 テーブル。Fresh DB は `full_schema.rs` で V60 相当を一括作成 → V61+ を逐次適用
...
- **🚀 [`docs/vision/plans/2026-05-04-cross-platform-migration.md`](./docs/vision/plans/2026-05-04-cross-platform-migration.md)** — **現行 SSOT**。Tauri 2 → Electron + Capacitor + Web + Supabase 大規模移行（Phase 0-5、3-4.5 ヶ月）。下記 Tauri/Cloud 系プランの大半を上書きする
- [`docs/vision/realtime-sync.md`](./docs/vision/realtime-sync.md) — Phase 1: foreground 可変 polling + 変更イベント駆動 push（Supabase Realtime 採用で再評価予定）
- [`docs/vision/desktop-followup.md`](./docs/vision/desktop-followup.md) — Desktop 残課題（Phase 5 で再仕分け）

旧 Tauri 前提プランは移行 Phase 5 で archive 予定（参考のみ）: [`docs/vision/mobile-porting.md`](./docs/vision/mobile-porting.md) / [`docs/vision/mobile-data-parity.md`](./docs/vision/mobile-data-parity.md) / [`docs/vision/ios-everywhere-sync.md`](./docs/vision/ios-everywhere-sync.md) / [`docs/requirements/ios-additions.md`](./docs/requirements/ios-additions.md) / [`docs/vision/plans/2026-04-26-windows-android-port.md`](./docs/vision/plans/2026-04-26-windows-android-port.md)
...
2. **実装プラン**: `.claude/docs/vision/plans/YYYY-MM-DD-<slug>.md` に作成 → Vision と相互リンク
3. **完了**: プランを `.claude/archive/` 移動（vision/plans/ から取り出す）、実装規約は本ファイルに統合、判断理由は `vision/coding-principles.md` 等に残す
...
`.claude/comm/` 経由で複数 Claude チャット間の非同期通信を行う（Phase 1: Outbox のみ）。
