...
`isTauriMobile()` が `true` の場合にモバイルモードで起動（Tauri + iOS デバイス判定）。デスクトップと同じ React コードベースだが Provider セットが異なる:
...
- **ルーティング**: React Router なし。`App.tsx` の `activeSection` で画面切替。`SectionId`: `"schedule"` / `"materials"` / `"connect"` / `"work"` / `"analytics"` / `"settings"`
- **TaskNode**: フラット配列 + `parentId` で階層表現。`type: 'folder' | 'task'`。フォルダの階層制限なし
- **ソフトデリート**: `is_deleted` + `deleted_at` カラム → TrashView から復元可能（Tasks/Notes/Memos/Routines/Databases/Templates）。CustomSounds もソフトデリート対応だがファイルベース（SQLite カラムではなく IPC 経由で管理）
...
- **ID**: String型。TaskNode は `"<type>-<timestamp+counter>"` 形式（例: `task-1710201234566`）、他エンティティは `generateId(prefix)` で `"<prefix>-<uuid>"` 形式（例: `note-xxxxxxxx-xxxx-...`）
