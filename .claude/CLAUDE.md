...
| ツール実行ハング（応答停止）       | `~/.claude/rules/bash-tool-stability.md`（本体 SSE バグ起因・ローカルはシロ。ESC 復帰 → jsonl 系統判定 → 混雑帯回避。重い Bash は background/subagent に逃がす。切り分けは memory `bash-tool-hang-diagnosis`） |
