...
- **ソフトキーボード 2 件（#607 / #608・PR #621 merged 2026-08-10）— deploy 後にスマホで 4 点**: ① Note の本文をタップして入力パネルが閉じないか（**閉じる場合は「板ごと消えて一覧に戻る」のか「板は残って本文だけ灰色の棒になる」のか**を見る — 後者なら今回特定した原因で確定、前者なら別の原因がまだ生きている）② キーボードを出すとタブバーが消え、閉じると戻るか ③ **タブバー非表示のとき safe-area 下端の余白も一緒に消える**ので、ホームインジケータ帯に本文が乗らないか ④ 「その他」シートを開いたままキーボードが上がるとシートごと消える — 許容できるか。**iOS は未検証**（実測は Android Chrome のみ）
