...
- 次: **PR #1020 の CI 確認と、PR #990（decisions 昇格）の merge 可否提示**。続けて chat-main 持ちの #1013（tracker guard の正規表現）→ #994（モバイル実ブラウザ計測 6 項目・#992 の着手条件）→ #1006 / #1015（docs 2 本）。**Epic #627 は子 4 本が全 close で close 判断可能**、**Epic #716 は #1014 の着地と狭幅の実機目視（ユーザー手番）だけ**。溜まっている merge 後の実ブラウザ検証（#892 / #675 / #948 / #985）も chat-main 手番。#677 / #898 は凍結
...
- [chat-main] **#675 の実ブラウザ回帰検証 6 項目 PASS → CLOSED + #870 起票** ✅（2026-08-15）— 週表示 / 月表示 / ドラッグ移動 / リサイズ / 繰り返しのスコープ選択 / Todo の追加と削除がすべて OK。console error 0・Supabase 938 リクエスト全成功。**スコープ 3 種（only / following / all）が別々に効くことを実測**したので「黙って全件書き換わる」退行はない。**前セッションの「`CalendarTab.tsx` が増えている」は誤報だった** — 分割前の基準を Issue 起票時点の数字にしていたためで、分割 PR #839 の直前直後で測れば 2,716 → 2,557 行（159 行減）。**行数の増減は対象コミットの直前直後で測る**。DoD「引数が実質半減」は達成扱いにしたが内訳をコメントに残した（自前 12 個 + 横流し 17 個 = ホストが渡す総数は 29 で減っていない）。**#870** = 時刻変更と繰り返し ON を同じ Save で行うとルーチンのテンプレート時刻が変更前になる（`useRepeatMutations.ts:321` の `const seed = selected;`・分割前にも同一行があり #675 の退行ではない）
- [chat-main] **#831 の stacked merge 事故を検出 → 復旧 #865 の着地を確認 + D-20260813-briefing-1 昇格（#860 起票）** ✅（2026-08-14）— #831 の 3 PR は**すべて MERGED 表示なのに main へ届いたのは PR-A だけ**だった。原因は base 張り替えレース（#861 merge 01:44:14Z → #862 merge 01:44:24Z の **10 秒差**で、GitHub が base を main に張り替える前に押された）。検出は 3 角度 = `git merge-base --is-ancestor <mergeCommit> origin/main` が #862/#863 で NOT / main に `list_tasks`・`typeTask` が残存 / `-mcp-docs` ブランチとの diff が 284 files。**`TaskNode` の grep は `setTaskNodes` に当たって誤検出する** — 着地判定は「その PR でしか生まれない成果物」（MCP ツール名・i18n キー・リネーム後ファイル名）で行う。復旧はブランチが健在だったので base=main の PR 1 本（#865）にまとめるだけで済み、書き直しゼロ。据え置き 3 点（`TodoNodeType = "task"` / `role: "task"` 4 箇所 / `tasks_payload`）も実測で無事。判断キュー側は D-20260813-briefing-1 = A を台帳へ昇格し、実装 Issue **#860**（`section:analytics` = briefing-refine）を起票
