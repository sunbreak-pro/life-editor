...
- 次: 配った 8 件（#1336 / #1342 / #1343 / #1345 / #1211 / #1300 / #1301 / #1337）+ **新規 14 件（#1362〜#1375）** の PR 回収 → merge 後に UI 変更を実ブラウザ検証。**新規 14 件はまだ `/goal` を配っていない**（issue-prompter で materials / schedule / briefing / tags / analytics / shared-fix の 6 レーン分を組み立てる。#1367 と #1373 は `AgendaList.tsx` を共有するので同一レーンで順に、#1368 は共有部品 `TodoStatusCheckbox.tsx` なので単独で回す）。**chat-main 手番は新規 14 件の配布と claude-launcher の docs 追随**（`plans/2026-08-29-claude-launcher-desktop.md` の Status が IN PROGRESS のまま・CLAUDE.md §5 も「#1211 で再設計中」のままで、実装は今朝 `07a1da97` で着地済み）。#1335 = 夜間ルーチンの Task Scheduler 登録は 2026-08-30 に「今はやらず Issue 化して可視化」とユーザー裁定済みなので、着手にはその裁定を覆す判断が要る。**Epic #1121 / #716 の残りはどちらもユーザー手番** — #1121 = 通しツアーの完走 1 回（10 ステップ中 9 つが実操作 = 予定 / Todo / ノートを実際に作るので、狭幅の実機目視と同時にやるのが二度手間にならない。「つながり / 集中 / 分析」の disabled 3 つを初回スコープ外として許容するか決めれば close 判定できる）/ #716 = 狭幅の実機目視 + `mobile-scope.md` の突き合わせ（chat-main 手番）。#677 / #898 は凍結
