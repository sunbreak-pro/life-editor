...
- 次: **main の CI（`CI — live stack`）が緑になったのを確認してから #860（briefing-refine）と #675（refactor-core）のプロンプトを投入する** — 復旧 PR #865 時点の run は後続 push でキャンセルされており、3,470 箇所の改名直後なので一度は緑を見る（前例 = PR #829「各 PR 単独では緑・組み合わせで赤」）。**#627 の子 Issue 4 本（Schedule / Tasks / Work / Tags）の起票は chat-main の手番**（裁定 `D-20260811-main-1` = A 済み・#831 の改名と衝突するため後回しにしていた分）。#716 の残りは狭幅の実機目視（ユーザー手番）、#677 は凍結
