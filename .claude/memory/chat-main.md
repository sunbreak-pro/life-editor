...
- 次: やること残り = ③ r4 計画書の COMPLETED 化 + 乖離レビュー 3 行 + archive（一時 worktree で PR。AC「open Issue が Epic + 凍結のみ」は r4 スコープの Issue で判定し #1283〜 の新ラウンドは対象外と明記）④ repo 外 `$HOME/dev/Claude/hooks-lib/regen-index.sh` へ RETIRED スキップ分岐を追随（このマシンと Mac で各 1 回 = PR #1295 の残）⑤ #1135 方向 (b)（regen 時に gh で PR 実状態を引く案）の扱いを決め、見送りなら (a) 完了として close 判定をユーザーへ提示。**Epic #1121 は #1192 / #1193 / #1201 / #1194 全着地 → 実機確認後に close 判定**、**Epic #716 は狭幅の実機目視（ユーザー手番）のみ**。#677 / #898 は凍結
