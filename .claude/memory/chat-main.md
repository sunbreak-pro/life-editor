...
- 次: **#868（#860 の実装）の merge を待って巡回**。**#627 の子 Issue 4 本（Schedule / Tasks / Work / Tags）の起票は chat-main の手番**（裁定 `D-20260811-main-1` = A 済み・#831 の改名と衝突するため後回しにしていた分）。**#870 は schedule-refine レーンへ渡す**（`section:schedule`・単独で完結する小さい修正）。#716 の残りは狭幅の実機目視（ユーザー手番）、#677 は凍結
