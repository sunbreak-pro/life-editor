...
- 次: **PR #1112 の merge 可否提示**（docs のみ・docs-lint 緑）。続けて chat-main 持ちの #1006 / #1015（docs 2 本）と #1013（tracker guard の正規表現）。**#1114 は shared-fix レーン、#1115 も shared-fix、#1116 は work-refine** へ配る（`issue-prompter` で `/goal` を組む）。**Epic #627 は子 4 本が全 close で close 判断可能**、**Epic #716 は #1014 の着地と狭幅の実機目視（ユーザー手番）だけ**。溜まっている merge 後の実ブラウザ検証（#892 / #948 / #985）も chat-main 手番。#677 / #898 は凍結
