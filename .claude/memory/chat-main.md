...
- 次: 配った 3 件（#1336 / #1342 / #1343）の PR 回収。**Epic #1121 はツアーの実機動作を確認済み**（概要モーダル → セクション選択 → 吹き出し → スキップまで通る・「つながり / 集中 / 分析」は準備中で disabled）なので、この disabled 3 つを初回スコープ外として許容するか決めれば close 判定できる。**Epic #716 は `mobile-scope.md` の突き合わせ（chat-main 手番）+ 狭幅の実機目視（ユーザー手番）**。#1300 / #1301 / #1211 / #1337 / #1335 は chat-main 手番のまま。#677 / #898 は凍結
- 気づき（未起票・ユーザー判断待ち）: **ノート削除だけ確認ダイアログが無い**（「その他の操作」→ ノートを削除で即ゴミ箱行き）。Todo 削除（`ScheduleTodoDetail.tsx:124`）と繰り返し削除（#1332 で ConfirmDialog に統一したばかり）は確認を挟むので作法が割れている。ゴミ箱から戻せる前提でわざとかもしれず、揃えるなら Issue 化する
