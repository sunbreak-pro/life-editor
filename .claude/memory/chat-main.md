...
- 次: **#831 の着手は「open PR が 0 件」の谷間まで待つ**（3,470 箇所に触るため並行レーンと必ず衝突する。着手条件は Issue にコメント済み）。それまでは各レーンの PR 回収と、**#829（main のテスト赤を直す PR）を他 3 本より先に merge** してもらうこと。判断キュー 2 件の回答待ちは継続 = `D-20260811-main-1`（#627 の子 Issue 起票）/ `D-20260811-main-2`（#321 の close 可否）
