...
- 390 幅で長いタイトルの行が横スクロールを生む問題を直した（#1749）✅（2026-09-19・**PR #1751 open**）
...
- **PR #1751（#1749）の merge 後に chat-main 側で 390 幅の実ブラウザ確認**: 長いタイトルのアイテムを持つタグを開き、横スクロールバーが消えて「…」が画面内に収まるか。jsdom はレイアウトを持たないのでここだけは実機が要る
