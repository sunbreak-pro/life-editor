...
- 前回: **#1038 の実測レポート（PR #1071 merged）と #1012 のテスト横展開（PR #1058 merged）**
- 現在: **#1101 = #1038 案 A-1 の実装（PR #1108 open）**。`useDomainLoad` に任意の `snapshotKey` を足し、成功した読みをモジュール store に置いて次の mount で layout effect から `apply` に流す stale-while-revalidate。ドメインフック 7 本が opt-in、キー無しの呼び出しは挙動据え置き
