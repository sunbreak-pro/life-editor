...
- ループ摩擦除去計画書 `2026-07-16-loop-friction-fixes.md` 新設（ユーザー要件 6 件 → 実測精査 + 決定 6 件確定。重要発見 = 手書き朝刊・夕刊は Daily 平文 textarea のため現状不成立 → F-1 Daily TipTap 化が Step 2 と並ぶループ開通の前提。briefing-loop 決定録 5 / tier-3 Analytics「破棄しない」追記・outbox 起票依頼 5 件 F-1〜F-5）→ PR #254 open・merge 待ち ✅（2026-07-16）
