...
- **🛑 こうだいさん手番 — merge 待ちが 7 本**（P-001）: 実装 5 本 = **#1305**（#1264 ツアーフッター）/ **#1315**（#1276 複数形）/ **#1321**（#1275 Trash の NoticePanel 化）/ **#1325**（#1278 text variant）/ **#1327**（#1283 ヘッダー中央化 + #1284 右パネルの閉じる導線）。docs 2 本 = **#1328**（#1279 の判断キュー起票）/ **#1246**（#1194 = Settings からのチュートリアル導線・前セッション分）。**2026-08-30 に全ブランチを現 main（7339bd2e）へ dry-run した実測**: **#1325 は取り込み済み**（#1306 と `web/tests/linkPanel.test.tsx` の末尾で衝突 → 両方保持・CI 緑・`CLEAN`）／ #1305 / #1315 / #1321 / #1327 は b31ee913 ベースのままだが **clean にマージできる**／ **#1246 だけ `en.json` / `ja.json` で衝突する**（未修正 — 直すかはこうだいさんの指示待ち）。#1328 は未確認
