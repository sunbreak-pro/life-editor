...
- **/goal 3 件一括（2026-08-27）**: #1114 / #1134 / #1122 のすべてに「origin/main から切ったブランチ + CI verify 15 ステップ緑 + PR」。**PR #1142 / #1146 / #1154 とも merged** ✅（2026-08-27）

## 予定

- **🛑 こうだいさん手番 — merge 待ちが 4 本**（P-001）: #1214（#1138 MCP の週開始）・#1217（#1192 ツアーのふきだし退避）・#1225（#1193 ツアーの再開）・#1228（#1201 Briefing 説明ステップ）
...
- **chat-main の手番（実ブラウザ検証）**: #1192 の DoD 後半 = ツールチップ表示中に `elementFromPoint` がタグの選択肢自身を返すこと。jsdom は「ポップオーバーが開いているあいだ描画されない」までしか保証できない（座標が全部 0 = §7.1）。#1193 も「step 3 で中断 → リロードでツアーが必ず出る」を実機で見るのが最終確認。ほかに #992 / #947 / #874 / #880 の実機確認が未消化
- **#1194 は Wave 2**: Settings からセクションを選んでツアーを始める導線。#1174（Settings の rightSidebar タブ化）の merge がゲートなので着手していない。`startSection` は #1193 の「前提が崩れたステップを巻き戻す」と設計が重なるので、着手時に #1225 の `giveUp` を読んでから設計すること
