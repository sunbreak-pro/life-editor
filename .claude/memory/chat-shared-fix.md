...
- **main がいま赤い（2026-09-22 実測・自分の PR とは無関係）**: `chartTheme.ts` の `fitAxisLabel` と `TodosTab.tsx` の `sessionsWithinRange` の 2 記号が merge の解決で落ちていて、`shared` の build / typecheck:tests / test と `web` build と `desktop` build の 5 ステップが main の時点で赤です。main 自身の CI（3d742107）も同じ理由で落ちています。直す PR は **#1905 / #1906 / #1907** の 3 本で、中身は 3 本とも同じ 2 ファイル 68 行です。同じ破損に 3 レーンが同時に着手したので、merge するのは 1 本で足ります。ローカルの verify は、この 5 ステップが同じ 2 記号だけを報告することと、`shared` の test が自分のブランチの有無にかかわらず同じ 6 ファイル / 17 件で落ちることを確かめて緑と判定しました
