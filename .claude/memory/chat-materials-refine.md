...
- **materials の 9 本を main 取り込みで緑に戻し、main の再破損を 1 本で修理** ✅（2026-09-22 — こうだいさんの指示「CI が赤いので main から取り込んで直してから再度 PR」。書いた時点の実測で **#1911 / #1915 / #1923 / #1928 / #1931 / #1934 / #1936 / #1939 / #1941 の 9 本と、main 修理の #1945 がすべて CI 緑・MERGEABLE**）
  - **9 本の赤はすべて赤い main から枝を切ったせい**（`fitAxisLabel` / `sessionsWithinRange` の欠落）。CI の `pull_request` は **base が直っても再実行されない**ので、赤は直った後も古い結果として残り続ける。**push し直すまで緑にならない**
  - **main がその日のうちに 2 回壊れた**。2 回目は briefing レーンの PR #1921 が `StreakDisplayLabels.days`（固定の文字列）を `formatDays`（件数を受け取る関数）へ改名してその時点の 7 本のテスト用データを直し、直後に入った PR #1926 が 8 本目を**古い形で**足した形。PR #1945 で 1 行直した
  - **main 自身の CI は赤を出力し切る前にキャンセルされる**: `ci.yml` の `concurrency` が `cancel-in-progress: true` なので、merge が立て込むと main の run は次の merge に潰される。**main の緑は「最後に完走した run」でしか読めない**
  - **同じ作業を別のセッションが並行していた**（Fable 5.1 / session 019XSFhXmKcf）。競合した 2 本（1837 / 1843）は向こうが先に push しており、解決の中身も同一だった。**force push せず向こうの commit を merge してから push する**
  - **eslint が「列挙した直後にファイルが消えた」で落ちることがある**（ENOENT）。同じ worktree でブランチを切り替える処理が重なると起きる。**もう一度静かな状態で回して判定する**
