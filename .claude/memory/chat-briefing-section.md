...
- Issue #413 — Briefing の rightSidebar に Schedule の `TodayTodoTray`（#298）を流用配線し、残タスクを今日の候補へ配置可能に ✅（2026-07-27・**PR #422 merge 済み**。ただし role-qa 監査の BLOCKING 修正 = 紙面の日付判定の UTC スライス（JST で終日タスクが常に前日扱い）は merge に間に合わず、**追随 PR #426 で別出し（open / merge 待ち）**。wide 限定 = narrow に詳細パネルの開閉導線が無いため。実ブラウザ確認は chat-main）
