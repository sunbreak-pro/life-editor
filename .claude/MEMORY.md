...
- Schedule 系 3 件のバグ修正（templateId 往復ロス / routineFrequency 暴走 / MCP dismiss 欠落）✅（2026-04-27）— ユーザーが triage したエラー報告 3 件（中・低〜中・低）の正当性を検証して全件修正。実装プランなしの bug fix。**P1 — templateId 往復ロス**: `src-tauri/src/db/schedule_item_repository.rs` の `ScheduleItem` struct + `from_row` に `template_id: Option<String>` を追加。INSERT 側は元々書いていたので、フェッチ時に `templateId: undefined` で消えるラウンドトリップロスが解消（`useRoleConversion.ts` の re-INSERT 経路で発生していた）。**P2 — routineFrequency default 暴走**: `frontend/src/utils/routineFrequency.ts` の switch default を `return true` → `return false` に変更。`frequencyType="group"` で group 未解決のとき全日マッチして `bulkCreateScheduleItems` が暴走するエッジケース（`OneDaySchedule.tsx::reconcileRoutineScheduleItems` で firstGroup undefined パス）を防止。他の呼び出し箇所はすべて group の frequencyType を渡すか `routineScheduleSync.ts::shouldCreateRoutineItem` 経由で "group" を独立処理するため副作用なし。新規テスト `routineFrequency.test.ts` 10 件で daily/weekdays/interval/group 全分岐をカバー（"group" デフォルト false の回帰ガード含む）。**P3 — MCP Server 機能差分**: `mcp-server/src/handlers/scheduleHandlers.ts` に `dismissScheduleItem` / `undismissScheduleItem` ハンドラ追加（version bump 付き、Tauri 側 `dismiss`/`undismiss` と等価）、ID 生成を `si-${Date.now()}` → `si-${randomUUID()}` で Tauri と統一、`formatItem` に `isDeleted` / `deletedAt` / `reminderEnabled` / `reminderOffset` 追加、list クエリに `is_deleted = 0` フィルタ追加。`mcp-server/src/tools.ts` に `dismiss_schedule_item` / `undismiss_schedule_item` ツール定義 + dispatcher case を 3 点同期で追加。**Verification**: cargo check 0 / mcp-server tsc 0 / frontend tsc -b 0 / vitest 42 files + 新規 1 file = 43 / 376 + 10 = 386 pass / cargo test --lib 25/25 / session-verifier 全 6 ゲート PASS
