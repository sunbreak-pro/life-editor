...
- Routine 削除のゴースト復活問題 + DayFlow 時間変更の Undo/Redo 全日付対応 ✅（2026-04-25）— ユーザー報告 2 件の Routine 関連バグを 1 セッションで対応。**症状 A — Untitled routine の削除不安定**: 削除後も他の日付で残ったり、削除ボタンを押していないのに突然消えたりする。**真の根本原因**: `src-tauri/src/db/routine_repository.rs::soft_delete` が schedule_items を**物理 DELETE** していたため、Cloud Sync の `is_deleted=1 + version+1 + updated_at` delta path に乗らず → 他端末から item が再 push され「ゴースト復活」。後から iOS が遅れて soft-delete を処理した際に「突然消える」現象も同源。修正は (1) `soft_delete` を物理 DELETE → soft-delete UPDATE (`is_deleted=1, version+1, deleted_at, updated_at, AND is_deleted=0` 条件付きで再実行時の version 二重 bump 防止) に書き換え、(2) defensive guard として `frontend/src/utils/routineScheduleSync.ts::shouldCreateRoutineItem` 冒頭に `if (routine.isDeleted) return false;` を追加、(3) `useScheduleItemsRoutineSync.ts::syncScheduleItemsWithRoutines` の routineMap lookup で `routine.isDeleted` も skip 条件化。**症状 B — DayFlow TimeGrid ドラッグ + 「ルーティンテンプレート更新」後の Undo/Redo が現在日付しか戻らない**: 「現在日 scheduleItem 更新」「routine 更新」「未来日 schedule_items 更新（IPC 直叩き）」の 3 アクションが独立しており、`UndoRedoManager.undoLatest()` が seq 最大の 1 ドメインしか pop しないため未来日更新が永久に戻らない構造。修正は (a) `useScheduleItemsCore::updateScheduleItem` と `useRoutines::updateRoutine` に `options?: { skipUndo?: boolean }` 引数追加（`ScheduleItemsContextValue` も型同期）、(b) `OneDaySchedule::handleUpdateScheduleItemTime` を routine item の場合のみ `skipUndo: true` で適用（dialog 選択待ち）、(c) `RoutineTimeChangeDialog::onApplyToRoutine` で適用前に `fetchScheduleItemsByRoutineId` で**未来日 items を before snapshot** 取得 → 3 アクションを skipUndo で実行 → **1 件の grouped undo entry** を `push("routine", { undo, redo })` に統合（undo は routine + 当日 item を local state revert + 未来日 items は IPC 直叩きで revert、次の loadItemsForDate / loadScheduleItemsForMonth で UI 反映）、(d) `onThisOnly` で deferred scheduleItem entry を後追い push、(e) `onCancel` で skipUndo revert（drag 自体「無かったこと」に）。**検証**: tsc -b 0 error / cargo check clean / vitest 35 files / 284 tests / 0 failed (新規 `routineScheduleSync.test.ts::soft-deleted routines never fire` 1 件) / cargo test 25/25 passed (新規 `routine_repository::tests::{soft_delete_marks_routine_and_uncompleted_schedule_items_without_physical_delete, soft_delete_preserves_completed_schedule_items}` 2 件) / 変更ファイル ESLint 新規エラー 0 (OneDaySchedule.tsx の `react-refresh/only-export-components` は git stash baseline で line shift のみと確認、本変更で発生せず) / session-verifier 全 6 ゲート PASS。**Rust 側変更ありのため `/Applications/Life Editor.app` 再ビルドが必要**（`cargo tauri dev` か `cargo tauri build` → 置換）。**残課題**: 既存 DB の "Untitled routine" 行は手動 trash 送り必要（生成経路を塞いだだけで既存データには触れない）/ 手動 UI 検証
- Calendar Events パネル UX 修正 + DayCell Routine アイコン整理 ✅（2026-04-25）— ユーザー報告 3 件の Calendar UI バグ・UX 改善。**Issue #1 — 終日トグル閉鎖防止**: `CalendarView.tsx::onUpdateAllDay` から `setScheduleItemPreview(null)` 撤去で、`updateScheduleItem(id, { isAllDay })` optimistic update + `liveScheduleItem` 経由 re-render のみとしパネルが開いたままに。**Issue #2 — TimeDropdown ポータルの click-outside 誤発火**: `frontend/src/components/shared/TimeDropdown.tsx` のドロップダウン (`createPortal(..., document.body)`) は React tree 上は親 popup 配下だが DOM tree 上は body 直下のため、`BasePreviewPopup` の document-level `useClickOutside` が option クリックで「外」と誤判定して親パネルを閉じていた。`isOpen=true` 時のみ `dropdownRef.current` に native `mousedown` listener を追加して `e.stopPropagation()` させ、document までバブルしないよう遮断。**Issue #3 — DayCell Routine アイコン削除 + `+`→Routine の再配線**: `DayCell.tsx` から `Repeat` import / `onOpenRoutineManagement` prop / セル右上 Routine ボタン撤去、`MonthlyView.tsx` の pass-through も削除、`CalendarView.tsx::onSelectRoutine` を `createRoutine("Untitled routine")` 直接呼びから `onOpenRoutineManagement?.()` 起動 (= `RoutineManagementOverlay` 表示) に変更。`onOpenRoutineManagement` 未提供時は `undefined` を返し `CreateItemPopover.BASE_ITEMS.filter(handlers[key] !== undefined)` で "Routine" 項目自体が非表示になる既存仕様で一貫。未使用化した `createRoutine` destructure も削除。**検証**: `tsc -b` 0 error / `vitest run` 35 files / 283 tests / 0 failed / 変更ファイル ESLint 新規エラー 0（TimeDropdown line 138 の `react-hooks/refs` は変更前から既存、stash で baseline 確認）/ session-verifier 全 6 ゲート PASS。**残課題**: 既存 DB の "Untitled routine" 行は手動 trash 送り必要（生成経路を塞いだだけ）/ 手動 UI 検証
- Cmd+K コマンドパレット統合（セクション動的アイテム + Sidebar Links + UI 拡大） ✅（2026-04-25）— ユーザー要件「現在の検索フィールドや Component / hooks を Cmd+K で開くコマンドパレットと統合したい」に対する実装。**UI 拡大**: `CommandPalette.tsx` を `max-w-[520px]` → `max-w-[680px]`、コマンドリスト `max-h-[320px]` → `max-h-[480px]`、起動位置 `pt-[15vh]` → `pt-[12vh]` で中央寄り。**Sidebar Links を Links カテゴリに動的注入**: `useAppCommands.ts` で `useSidebarLinksContext()` を購読し、`!isDeleted` リンクを `id=sidebar-link-{id}` / `title=emoji + name` / `icon=AppWindow|Link` / `action=openLink(link)` で Navigation の直後に挿入（既存 6 セクション + Settings deep links / Task / Timer / View はそのまま残置）。**セクション別動的アイテム**: 新規 `useSectionCommands.ts` で `activeSection + scheduleTab` から動的 Command[] 生成。`schedule + calendar/dayflow` → routines + non-routine events + scheduled tasks（最大 30 / グループ）、`schedule + tasks` → scheduledAt 降順 task、`schedule + events` → 非 routine の scheduleItems、`materials` → 最近のノート (updatedAt 降順) + Daily（date 降順）、選択時は `localStorage.MATERIALS_TAB` を書き換えて section/tab 遷移。`App.tsx` で `[...sectionCommands, ...baseCommands]` の順に集約しダイナミック群を上に置く。**RightSidebar 検索フィールド削除→アイコン化**: 新規 `shared/SearchTrigger.tsx`（Search アイコンボタンが `OPEN_COMMAND_PALETTE_EVENT` を `window.dispatchEvent` するだけの軽量 component）と新規イベント `events.ts::OPEN_COMMAND_PALETTE_EVENT`（App.tsx で listener 登録 → `setIsCommandPaletteOpen(true)`）。10 箇所の `<SearchBar>` 使用箇所（`ScheduleSidebarContent` / `DailySidebar` ×2 / `MaterialsSidebar` ×2 / `ConnectSidebar` / `PaperSidebar` ×2 / `WorkMusicContent` / `Settings` ×2）を `<SearchTrigger />` に置換、関連する `searchQuery` setter / suggestions useMemo / handleSuggestionSelect useCallback の dead code を削除（`useState` の値だけ残し setter を destructure 落としで `noUnusedLocals` 回避）。**ScheduleSection の整理**: `sidebarSearchQuery` state と searchPlaceholder / searchSuggestions / handleSearchSuggestionSelect を削除、`<ScheduleSidebarContent showSearchTrigger>` プロップ + 子コンポーネントには `searchQuery=""` / `sidebarSearchQuery=""` を固定で渡す（中身の filter が常に no-op になり実質的に Cmd+K に集約）。**Settings の整理**: `useSettingsSearch` 呼び出し / `settingsNavigators useMemo` / `handleSettingsSearchSelect` を削除（Cmd+K の Settings deep links が代替）。**検証**: `tsc -b` 0 error / `vitest --run` 283/283 pass / `npm run build` 成功。session-verifier は未実行（次回コミット時に走らせ予定）
