...
- ドキュメント整理: known-issues 統合（13→9）+ CLAUDE.md / rules コンパクト化 ✅（2026-04-25）— コンテキスト消費分析（自動ロード ~8.4K tok/turn）に基づき、known-issues を統合（旧 001/002/003 → 新 001 / 旧 013/014 → 新 013）。CLAUDE.md (350→267 行 / -23%) と `~/.claude/rules` (-22%) を圧縮。Stage A+B 完了、Stage C(HISTORY-archive 圧縮)は ROI 低で見送り。月間 ~342K tok 削減見込み

## 予定

### リファクタリング計画 Phase 2-3（Phase 0/1 完了済、IN_PROGRESS）

**対象**: `frontend/src/services/TauriDataService.ts` / `src-tauri/src/db/migrations.rs` / `src-tauri/src/db/*_repository.rs` 27 件 / 巨大コンポーネント 4 件 / Calendar Mobile-Desktop 統合 ほか
