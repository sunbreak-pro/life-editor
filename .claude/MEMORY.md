...
- 現在: **PR1 + 循環ガード + 406 A-1 + known-issue 020 完了 → pathspec commit/push 実施**。実ブラウザ評価で7問題判明→方針「バグ修正優先・UX 段階的」。PR1=①NoteTitleInput key から title 除去 ②初回 effect で loadDeletedNotes+softDelete subtree push ③セッション unlock 集合+本文 blur ゲート ④行ホバー削除+post-order subtree cascade soft-delete。実機 ①②③④ OK。+ ④collect に循環 parentId `seen` Set ガード(known-issues 016 同型 OOM 予防)。+ **406 "Cannot coerce to single JSON" = 楽観 create×version read `.single()` 0行レース**を案 A-1(`updateNote` を `.maybeSingle()`+0行skip、upsertDaily は元から maybeSingle 無変更、toggleBoolean/nextVersion は意図的現状維持)で修正、known-issue 020 起票(Status=Fixed)。役割分担: debug-strategy 原因特定 / role-engineer 実装 / role-qa 統合監査 PASS(Blocker0 Major0 循環ガード正当)。型/lint/build green・frontend 非破壊
- 次: **S4 Schedule 移植**(最大規模: routines/routine_groups/routine_group_assignments/schedule_items/calendars/calendar_tag_assignments + Schedule 3分割 Provider Routine→ScheduleItems→CalendarTags)。S3 同様ループ(migration→SupabaseDataService→context/hooks→web ミニ UI→検証)。S4 着手前に role-pm で要件分解(sync 区分判定/スキーマ依存順/Routine 生成仕様/曖昧点)。S4 以降も Option A 前提。PR2 UX(⑤行内アクション収束 ⑥drop indicator ⑦間隔)+⑧subtree restore は Backlog(計画書記録済)
