...
- Materials/iOS Notes アイテム名表示クリーンアップ ✅（2026-04-25）— ユーザー報告の単発バグ修正。**Desktop**: `Ideas/NoteTreeNode.tsx` の `extractFirstHeading(node.content) || node.title` を `node.title || "Untitled"` に統一（本文の最初の見出しと title が混在表示されていた問題を解消）+ 未使用 `extractFirstHeading` import 撤去。**iOS**: `Mobile/materials/MobileNoteTreeItem.tsx` で Pin (Heart) アイコンを Note アイコン位置に移動 (Desktop と同じ。pinned 時は StickyNote の代わりに Heart 表示) / Tag 名のピル (色付きラベル最大 2 件) を撤去し `+N` バッジのみタイトル後ろに残す / Lock アイコンは右端配置 / `renderExtra` prop 撤去。`MobileNoteTree.tsx` も同 prop 伝播を削除。`Mobile/MobileNoteView.tsx` の Favorites セクションから本文先頭 24 文字プレビュー (`renderExtra` 経由 `extractPlainText(note.content).slice(0, 24)`) を削除 + 未使用ローカル関数 `extractPlainText` 撤去。**検証**: tsc -b 0 error / vitest 268/268 pass / 自セッション変更分 ESLint 0 error (残 4 件は NoteTreeNode.tsx:293 の既知 react-hooks/refs baseline、私の変更外)。session-verifier 全 6 ゲート PASS
