...
- 前回: S4 Schedule 移植 コード完了（S4-0〜S4-6、子ブランチ `phase-2/schedule-migration`、全サブ role-qa PASS）
- 現在: **S4 完全クローズ**（`297ead6` push 済）。0006 本番 SQL Editor apply 成功・手動確認 OK（ユーザー実施、`cloud/db/migrations/0006_*` SQLite 版誤貼り → `supabase/migrations/0006_schedule_full_schema.sql` で再実行成功）。実ブラウザ 2 バグ修正完了: バグ1=Calendar 作成 FK 違反を folder-type task の `<select>` 選択化（schedule に TaskTreeProvider 追加）で構造的解消 / バグ2=Routine item 復活（Issue 017 震源地）を routine item の Delete/Trash 導線非表示+Dismiss 主導線化で構造的遮断（UI 非表示+DataService 重複ガードの二重防御を role-qa コードパス実証）。Routine 自体の削除は既存 softDeleteRoutine cascade で既存生成 item も消える。バグ3=RoutineGroup 空表示はデモにつき未修正・SSOT 既知制約 1 行記録。role-qa PASS(Blocker0 Major0 Minor0)、shared/frontend/src-tauri/cloud/supabase diff0、vitest 71/71 非回帰。S4 SSOT Status=COMPLETE
- 次: **`phase-2/schedule-migration` を `refactor/web-first-v2` へマージ判断（ブランチ戦略ユーザー確認待ち）→ S5 WikiTags**（`wiki_tags`/`wiki_tag_assignments`/`wiki_tag_connections` relation テーブル、tag アサイン move）。S8(Realtime/delta)申し送り 6 項は S4 SSOT `2026-05-17-s4-schedule-migration.md` 記録済＝S8 着手者必読
...
- Phase 2 S4 完全クローズ（0006 apply 成功 + 実ブラウザ 2 バグ修正）✅（2026-05-19）— 0006 本番 apply 成功・手動確認 OK。実ブラウザ判明の 2 バグ修正: バグ1=Calendar FK 違反を folder-type task select 化(TaskTreeProvider 追加)で構造解消 / バグ2=Routine item 復活(Issue 017)を Delete/Trash 導線非表示+Dismiss 主導線化で構造的遮断(UI 非表示+DataService 重複ガード二重防御、role-qa コードパス実証)。バグ3(RoutineGroup 空表示)はデモにつき SSOT 1 行記録のみ。role-qa PASS(Blocker0 Major0 Minor0)、保護領域 diff0、vitest 71/71 非回帰。`297ead6`。詳細 HISTORY 参照
- Phase 2 S4 Schedule 移植 コード完了 ✅（2026-05-17）— 子ブランチ `phase-2/schedule-migration` で S4-0〜S4-6 の 7 サブステップ実装。0006(7テーブル)+mapper7+DataService7+Routine/ScheduleItems/Calendar/CalendarTags Provider+Routine 生成器(論理 diff ゼロ移植)。各サブ role-qa PASS、security/sync-auditor PASS。vitest 71/71・非破壊。詳細 HISTORY 参照
- 移行 SSOT 復元 + MEMORY/CLAUDE ドキュメント陳腐化一掃 + orphan DB 削除 ✅（2026-05-17）— 移行 SSOT 誤削除(60f5f63)を git から完全復元。MEMORY 予定 11 項目+バグ温床 10 項目の陳腐化削除、クロス参照ドリフト修正。orphan DB 退避削除。詳細 HISTORY 参照
