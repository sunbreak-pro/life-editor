...
### Q2 機能パッチ Phase D / Phase A Cloud Sync の deploy & 手動 UI 検証

**対象**: D1 migration 適用 (`wrangler d1 execute`) + Worker deploy + macOS app launch / 既定ブラウザ選択 / iOS Drawer 表示の動作確認
**手順**:

1. `cd cloud && npx wrangler d1 execute life-editor-sync --remote --file=./db/migrations/0004_calendar_tags_v65.sql`
2. `cd cloud && npx wrangler d1 execute life-editor-sync --remote --file=./db/migrations/0005_sidebar_links.sql`
3. `npm run deploy` (cloud Worker)
4. Desktop V67 自動 apply 確認 → LeftSidebar に「Links」セクション表示 / `+` で URL/App リンク追加 / 既定ブラウザ切替で URL 起動先が変わる / `/Applications/*.app` 一覧から登録できる
5. iOS シミュレータ Drawer に `kind='app'` がグレーアウト + Toast 出ることを確認
6. Desktop ↔ iOS 双方向 sync で sidebar_links と calendar_tag_assignments が伝搬すること
