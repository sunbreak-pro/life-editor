...
- Calendar 表示整合性 A+B+C 完成 ✅（2026-05-13）— ユーザー報告「DB から消したのに Task/Event/Routine が Calendar に残る + Task が見えているのに RightSidebar が 0/0 表示」の根因 3 軸を順次修正。**(A) `useCalendar.ts` の `tasksByDate` filter に `!n.isDeleted` を 2 分岐追加** — soft-delete 後も Calendar に残るバグ解消、`useCalendar.test.ts` 2 ケース追加。**(B) `ScheduleSection::calendarCategoryProgress` を月単位集計に再設計** — Calendar (月表示) と Progress (旧日表示) の temporal scope ズレ解消、`monthlyScheduleItems` + month-wide `allTasksByDate` + 月内 `dailies`/`notes` 集計、`loadScheduleItemsForMonth` 発火、ProgressSection / DayFlowSidebarContent に `scope?: "day" | "month"` prop 追加 (Calendar=month / DayFlow=day)。A+B コミット ae365bb。**(C) Settings に Calendar データ一括削除 Dialog 追加** — `CalendarDataResetDialog.tsx` + テスト 7 件 / `bulkSoftDeleteCalendarData(kinds[])` IPC 4 点同期 (DataService / TauriDataService=data/misc.ts / Rust command / generate_handler!) / Rust 側は 2 phase (Phase 1: routines は既存 `routine_repository::soft_delete` cascade を再利用、Phase 2: tasks / events / dailies / notes を 1 transaction で UPDATE)、全て version+1 + updated_at bump で Cloud Sync delta path 対応。bulk_soft_delete unit test 5 件 pass。**Verification**: tsc -b 0 / eslint 0 / Frontend 44 files 394 tests pass / Rust 30 + 5 = 35 tests pass / IPC 4 点同期確認済。C は role-engineer worktree で実装、stalled 失敗だったが成果物が残っており本セッションで取り込み完成。**ブランチ**: refactor/web-first-v2。計画書 archive 済 (`.claude/archive/2026-05-12-calendar-display-integrity.md`)
