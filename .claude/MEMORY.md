...
- Routine Tag 廃止 + Group 中心の再設計 ✅（2026-04-25）— **計画書**: `archive/2026-04-25-routine-group-migration.md`。Routine の Tag 機能（`routine_tag_definitions` / `routine_tag_assignments` / `routine_group_tag_assignments`）を完全廃止、Routine ↔ RoutineGroup を直接 junction で結ぶ新モデルに移行。`FrequencyType` に `"group"` 追加、Group を選んだ Routine は所属 Group の frequency 設定（daily/weekdays/interval）を OR で継承。**8 Phase 完了**: V69 migration（`routine_group_assignments(id PK, routine_id FK, group_id FK, updated_at, soft-delete, UNIQUE(routine_id, group_id))` 新設 + 旧 3 テーブル DROP）+ D1 0007 / Backend DB layer (`routine_group_assignment_repository.rs` + `set_groups_for_routine` トランザクション差分 upsert + parent routine version+1) / Commands (`routine_group_assignment_commands.rs` + lib.rs handler IPC 4 点同期) / Cloud Sync (`RELATION_TABLES_WITH_UPDATED_AT` に追加、`RELATION_PARENT_JOINS` 空配列化) / Types & Service (`RoutineNode.groupIds?` + `RoutineGroupAssignment` interface + `fetchAllRoutineGroupAssignments` / `setGroupsForRoutine`) / Hook & Context (`useRoutineGroupAssignments` 新規、3 旧 hook 廃止、`useRoutineGroupComputed` を直接メンバーシップ参照に書き換え) / UI (`RoutineEditDialog` 全面書き換え: frequencyType=group 選択時に inline で既存 Group 多重選択 + 「+ 新規 Group 作成」フォーム展開、`RoutineGroupEditDialog` から tag picker 撤去、`RoutineManagementOverlay` から Tag 管理セクション撤去、`FrequencySelector` に "group" タイプ追加、`routineScheduleSync.shouldCreateRoutineItem` を frequencyType="group" は Group 群 OR / それ以外は routine 自身のセマンティクスに書き換え + `tagAssignments` 引数を全シグネチャから除去) / i18n (en+ja 両方更新)。**検証**: cargo test 23/23 pass（V69 統合テスト 2 件含む）/ vitest 283/283 pass（新規 routineScheduleSync.test.ts 7 件 + useRoutineGroupComputed.test.ts 追従）/ tsc -b + cargo check + cloud tsc 全 clean / 変更ファイル ESLint 0 error。session-verifier 全 6 ゲート PASS。**Rollout (重要)**: D1 migration を Worker deploy より先に適用（`wrangler d1 execute --remote --file=./db/migrations/0007_drop_routine_tags_add_group_assignments.sql` → `npm run deploy`、逆順だと 500）。**コミット範囲分離**: ユーザーの並行作業（CommandPalette / SearchTrigger refactor で `sidebarSearchQuery` undefined / TS6133 が App.tsx / ScheduleSection.tsx / Settings.tsx / \*Sidebar.tsx 等に残存）は別コミット
