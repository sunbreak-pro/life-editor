...
- リファクタリング Phase 0 完了（@deprecated 整理 / formatTime 統合 / tiptap XSS / MEMORY.md dedup）✅（2026-04-25）— 中粒度の subsystem 並列分析（Frontend / Rust / Cloud）から 4-Phase 計画を策定し Phase 0 を実行。`@deprecated` 4→0 件 / `ScheduleContextValue.ts` 削除 + `GroupFrame.onDoubleClick` / `UndoRedoButtons.domain` 削除 / `ScheduleItemEditPopup.tsx` の真の重複 formatTime を `utils/timeGridUtils` から import に統一（agent 推定 18+ 箇所は 4 シグネチャ責務違い並存で過剰検出と判明）/ `tiptapText.ts::getContentPreview` の innerHTML を `DOMParser.parseFromString` 化で `<img onerror>` 系 XSS 経路 inert 化 / §バグの温床 17→16 ユニーク項目。-58 行 / 7 ファイル変更 / 5 commits（76c6591 docs / 991b5bf @deprecated / 36b2de7 formatTime / cd8d59e tiptap / 4f2d552 MEMORY dedup、push 未実施）。tsc -b 通過 / Vitest 255 passed (31 files) / `@deprecated` grep 0 件。新規 doc: `.claude/2026-04-25-refactoring-plan.md` (IN_PROGRESS) + `docs/code-inventory.md`。Phase 1-3 は別セッション継続
- ドキュメント整理: known-issues 統合（13→9）+ CLAUDE.md / rules コンパクト化 ✅（2026-04-25）— コンテキスト消費分析（自動ロード ~8.4K tok/turn）に基づき、known-issues を統合（旧 001/002/003 → 新 001 / 旧 013/014 → 新 013）。CLAUDE.md (350→267 行 / -23%) と `~/.claude/rules` (-22%) を圧縮。Stage A+B 完了、Stage C(HISTORY-archive 圧縮)は ROI 低で見送り。月間 ~342K tok 削減見込み。session-verifier Gate 5 で INDEX 参照と実ファイル完全一致 / 旧 ID 参照ゼロを確認
...
### リファクタリング計画 Phase 1-3（Phase 0 完了済、IN_PROGRESS）

**対象**: `cloud/src/routes/sync.ts` / `frontend/src/main.tsx` / `frontend/src/services/TauriDataService.ts` / `src-tauri/src/db/migrations.rs` / `src-tauri/src/db/*_repository.rs` 27 件 / 巨大コンポーネント 4 件 / Calendar Mobile-Desktop 統合 ほか
**計画書**: `.claude/2026-04-25-refactoring-plan.md` / `.claude/docs/code-inventory.md`
**Phase 1（2-3 セッション、推定 -500〜-800 行）**: Cloud sync.ts 責務分割 (versioned/relations/shared) + Bearer token timing-safe 化 + zod validation / Provider tree 共通化 (`<DesktopProviders>` / `<MobileProviders>`) / Rust `row_to_json` 重複統合 (`db/row_converter.rs` 新設) / SQL injection whitelist の SAFETY コメント明示化
**Phase 2（4-6 セッション、推定 -1500〜-2500 行）**: `migrations.rs` 2328 行を V1-V30 / V31-V60 / V61-V64 の 3 分割 / `TauriDataService.ts` 1453 行を domain ごとに分割 / 巨大コンポーネント 4 件（ScheduleTimeGrid 1220 / OneDaySchedule 1165 / TagGraphView 1443 / TaskDetailPanel 947）を index + サブコンポーネント構造に / Calendar Mobile-Desktop 統合（`useCalendarViewLogic` + `components/Calendar/shared/` 新設）
