...
- sync_engine V65 follow-up fix（calendar_tag_assignments delta query 修正）✅（2026-04-25）— /session-verifier Gate 3 で発見した sync_engine.rs テスト 2 件失敗を解消。Q2 patch (1847e4c) の V65 migration が `calendar_tag_assignments` を `(entity_type, entity_id, tag_id)` + 自身の `updated_at` 持ちに再構築したが、`sync_engine.rs::collect_local_changes` が旧スキーマの `cta.schedule_item_id` JOIN を保持していた。CTA 自身の `updated_at` を delta cursor とする query に書き換え（task-typed CTA も同時に拾えるよう JOIN 撤去）。`cargo test --lib sync::sync_engine` 2/2 pass、全体 11/13 → 13/13 pass。1 commit (`58609b3`)。**Cloud 側の同種修正は別セッション**: `cloud/src/config/syncTables.ts:97-100` も同じ stale `schedule_item_id` JOIN を持つため、CalendarTags Cloud Sync (D1 migration 0004) 着地時に併修必要
- リファクタリング Phase 2-1 migrations.rs 6 分割完了 ✅（2026-04-25）— 2431 行のモノリシック `src-tauri/src/db/migrations.rs` を `migrations/{mod, full_schema, util, v2_v30, v31_v60, v61_plus}.rs` の 6 ファイル directory に分割（commit `e36845b`）。`mod.rs`(341) は `run_migrations` orchestrator + dispatch + 末尾 defensive backfill + 既存 7 tests 保持、`full_schema.rs`(592) は V60 final state CREATE TABLE batch、`util.rs`(32) は `exec_ignore` / `has_column` / `has_table` を `pub(super)` で集約、`v2_v30.rs`(688) / `v31_v60.rs`(536) / `v61_plus.rs`(311) は各バージョンレンジを `pub(super) fn apply(conn, current_version)` でラップ。各 SQL ブロックは byte-identical、公開 API 不変。**副次改善**: `LATEST_USER_VERSION = 66` 定数を導入し `assert_eq!(user_version, 64)` 5 箇所のハードコードを置換 → Q2 patch (V65/V66 追加) で陳腐化していた 5 件のテスト失敗を解消、`cargo test --lib db::migrations` が 2/7 → **7/7 pass** に復活。`cargo check --lib` clean / `+2500/-2431 行`
- Q2 機能パッチ Phase A/B/C（CalendarTags 1:1+Task / Pomodoro Free / WikiTag 未登録 + Events ソート）✅（2026-04-25）— ユーザー要件 4 件のうち 3 件を実装。**Phase A**: V65 migration で `calendar_tag_assignments` を `(entity_type, entity_id, tag_id)` + `UNIQUE(entity_type, entity_id)` に再構築（旧複合 PK の multi-tag を `MIN(tag_id)` で 1:1 に collapse）/ `calendar_tag_definitions` に created_at/updated_at/version/is_deleted カラム追加 / `db_calendar_tags_set_tag_for_entity` 新コマンド + `setTagForEntity` DataService API / `useCalendarTagAssignments` を `Map<entityKey, number>` 1:1 化、後方互換 shim `setTagsForScheduleItem` 維持 / `useCalendarTagFilter` 新規（localStorage 永続化、`null | number | "untagged"`）/ `CalendarTagsPanel`（Schedule rightSidebar 全タブ常時表示、タグ管理 + フィルタチップ）/ `CalendarTagSelector`（単一選択 dropdown）を `ScheduleItemEditPopup` の event/task 両モードに統合 / Optional Provider バリアント（`useCalendarTagsContextOptional`）で Provider 外でも null 安全。**Phase B**: V66 migration `timer_sessions.label` 追加 / SessionType に `"FREE"` / TimerContext に `startFreeSession` + `pendingFreeSave` + `saveFreeSession` + `discardFreeSession` / `FreeSessionSaveDialog`（Task/Event/破棄選択 + 親 Task autocomplete + CalendarTag 選択 + 「次回から表示しない」localStorage） / Settings に「Pomodoro 有効」「保存ダイアログ表示」トグル追加 / `pomodoroSettings.ts` 新規（react-refresh 制約回避で関数 export を分離）。**Phase C**: `TagFilterOverlay` に `(Untagged)` エントリ + `UNTAGGED_FILTER_ID` sentinel / DailySidebar / MaterialsSidebar の filter ロジックに「未登録」分岐 / `EventList` 全面再実装で排他的ソート 4 軸（date-desc/date-asc/title-asc/tag）+ CalendarTag フィルタ連動 + 行内タグドット表示。**ドキュメント整備**: `tier-1-core.md` / `tier-2-supporting.md` の Tasks-WikiTag 誤記を全削除（V60 で撤去済 `task_tags` 言及含む）し「Tasks は CalendarTags 担当、WikiTags 対象外」を明記。**Phase D（Sidebar Links）と Phase A の Cloud Sync は次セッション以降**。tsc -b 0 errors / Vitest 257/257 / 変更ファイル lint clean / cargo check pass。28 modified + 6 new files

## 予定

### Q2 機能パッチ Phase D — Sidebar Links + Browser/App settings

**対象**: `src-tauri/src/db/migrations.rs` (V67) / `src-tauri/src/commands/{sidebar_link,system}_commands.rs` 新規 / `frontend/src/context/SidebarLinksContext.tsx` 新規 / `frontend/src/components/Layout/{LeftSidebar,SidebarLinkItem,SidebarLinkAddDialog,MobileLayout}.tsx` / `frontend/src/components/Settings/BrowserSettings.tsx`
**計画書**: `.claude/2026-04-25-sidebar-tags-free-pomodoro.md` Phase D
**手順**: V67 で `sidebar_links` テーブル追加 → repository + commands → DataService bridge → SidebarLinksContext (Pattern A) → LeftSidebar 統合（Analytics と Settings の間に表示、右クリック CRUD + 絵文字選択）→ Settings に検出済みブラウザ選択（Chrome/Safari/Firefox/Edge を `/Applications` 存在チェックで列挙）→ /Applications 一覧から Mac アプリ登録 → iOS Drawer に表示（kind='app' はグレーアウト）→ Cloud Sync 対応 (D1 migration 0005)

### Q2 機能パッチ Phase A 残 — CalendarTags Cloud Sync

**対象**: `cloud/db/migrations/0004_calendar_tags.sql` 新規 / `cloud/src/config/syncTables.ts` の VERSIONED_TABLES + RELATION_TABLES_WITH_UPDATED_AT
**手順**: D1 用テーブル追加（`server_updated_at` 列含む）→ Workers の sync 対象に `calendar_tags` / `calendar_tag_assignments` 追加 → wrangler deploy + migration 同時適用
