...
- **重複防止**: 全イベントの summary を `[life-editor] <タスク>` で始める。create 前に
  必ず `list_events`(fullText=`[life-editor]`, その日) で既存を確認 → 無ければ create、
  あれば差分のみ update。タグを消すと重複するので消さない。
...
- 1 日 1 ブロック。割り当てきれない分は「来週候補」へ残す。
...
### 5. Google Calendar 反映
1. `mcp__Google_Calendar__list_calendars` で反映先を決定（「開発」/「Dev」相当の
   カレンダーがあれば採用、無ければ primary）。決めた calendarId は台帳ヘッダに記録。
2. その週の初回は、生成した週次案をユーザーに提示して**承認を取る**。
3. 日毎に `mcp__Google_Calendar__list_events`(fullText=`[life-editor]`, その日の時間窓)
   で既存を確認:
   - 無ければ `mcp__Google_Calendar__create_event`
   - あって内容差分があれば `mcp__Google_Calendar__update_event`（差分なしはスキップ）
4. 返ってきた eventId を台帳の GCal 列へ記録。
