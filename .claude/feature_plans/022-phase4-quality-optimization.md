...
Status: PLANNED
Created: 2026-02-22
Depends: 021-phase3-architecture-improvement
---
   ...
   function migrateV26(db: Database.Database): void {
     const migrate = db.transaction(() => {
       ...
       if (fs.existsSync(metaPath)) {
         ...
         const insert = db.prepare(`INSERT OR IGNORE INTO custom_sounds (...) VALUES (...)`);
...
5. カスタムサウンドの入力検証を追加（Phase 1 で未対応だった部分）:
   ...
   const ALLOWED_MIME_TYPES = ['audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/mp4', 'audio/webm'];

   function validateCustomSound(data: Buffer, meta: Partial<CustomSoundMeta>): void {
