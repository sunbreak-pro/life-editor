...
Status: PLANNED
Created: 2026-02-22
Depends: 020-phase2-data-integrity
---
...
1. `loggedHandler` にエラーサニタイズ層を追加:
   ...
   function sanitizeError(e: unknown): Error {
     if (e instanceof Error) {
       // SQL エラー
       if (e.message.includes('SQLITE') || e.message.includes('UNIQUE constraint')) {
       ...
       if (e.message.startsWith('Invalid') || e.message.startsWith('Unsupported')) {
         return e;
       }
...
1. ベースリポジトリファクトリを作成:
   ...
   function createBaseRepository<T>(db: Database.Database, config: RepoConfig<T>) {
...
1. Undo/Redo ヘルパー関数群を作成:
   ...
   function undoableOperation<T>(options: UndoableOptions<T>): void {
     const { domain, label, push, setState, apply, revert, persist, unpersist } = options;
...
3. Phase 1 の `optimisticUpdate` と統合（エラー時ロールバック付き）:
   ...
   async function undoableOperation<T>(options: UndoableOptions<T>): Promise<void> {
     ...
     options.push(options.domain, { /* undo/redo */ });
   }
   ```
