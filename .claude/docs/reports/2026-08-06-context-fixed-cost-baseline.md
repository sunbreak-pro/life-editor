...
削減方針の裁定（親計画 Step 3 = 🛑 人手）に要るもの。**ここでは何も決めていない。**

### 5-1. グローバル資産が Scope に入っていない

固定費の 66% が `~/.claude/`（実体 = 別リポジトリ `claude-dotfiles`）にある。親計画の Scope は `.claude/**` と `.mcp.json` だけなので、**このままでは支配項に手が届かない**。別リポジトリなので PR も別になる。Scope に入れるかはユーザー判断（decision キュー D-20260806-main-2）。

### 5-2. 移送先の当たり（順不同・実行は Phase 3）

| 対象                                       | 概算 tok | 移送先の候補                                                                                  |
| ------------------------------------------ | -------: | --------------------------------------------------------------------------------------------- |
| 口調定義の三重化（§3.2）                   |    4,472 | output style を正本に一本化 → 保険と詳細版を薄くする                                          |
| `.claude/CLAUDE.md` §7 の手順部分（§3.1）  |  〜1,600 | `worktree-policy` / `git-workflow` / `docs-workflow` へ（検証コマンドと落とし穴は航法に残す） |
| `~/.claude/CLAUDE.md` の docs 構造テンプレ |      781 | `project-setter` スキル側（既に参照が書かれている）                                           |
| エージェント description の起動条件列挙    |  〜1,500 | 各エージェント本体（description は判断用の 1〜2 行に）                                        |
...
**ハーネスはタダではない**（測る道具にも案内板にも場所代がかかる）。Phase 3 の再測定は 19,463 を起点に比較する。
