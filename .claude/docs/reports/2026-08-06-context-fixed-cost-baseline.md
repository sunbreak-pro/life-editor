...
## 7-2. グローバル側の移送（-1,945 tok・[`claude-dotfiles#13`](https://github.com/sunbreak-pro/claude-dotfiles/pull/13)）

D-20260806-main-2 = A を受けて 2 ラウンド実施。**merge 前の数値**なので、merge 後にこの表を実測で確定させる。

| 対象                              | 変更前 | 変更後 |       差分 | ラウンド |
| --------------------------------- | -----: | -----: | ---------: | -------- |
| `~/.claude/CLAUDE.md`（常時）     |  1,991 |    641 | **-1,350** | 1        |
| `~/.claude/rules/tone.md`（常時） |  2,184 |  1,458 |   **-726** | 2        |
| `project-setter` の一覧行（新規） |      0 |    131 |       +131 | 1        |
| **計**                            |        |        | **-1,945** |          |

- ラウンド 1 = 口調の「保険」1 層 + docs 構造テンプレ。**正本（output style 1,338）は無傷**。`project-setter` は**参照が 2 か所あるのに実在しなかった**ので新規作成し、死んだポインタも解消した
- ラウンド 2 = `/loop-prune` の 1 件目（→ §8）
- **見込み固定費: 19,463 → 約 17,520**。Phase 3 の再測定はここを起点にする
- 訂正: 初版は `~/.claude/CLAUDE.md` の変更後を 678 tok（-1,313）と書いたが、その後もう一段圧縮したため確定値は **641（-1,350）**

---

## 8. `/loop-prune` 第 1 回の記録（2026-08-07）

**対象: `~/.claude/rules/tone.md`（2,184 tok）** — 一覧で常時ロードの単独 2 位。

### 8-1. 実測で決まった前提（この棚卸しの分岐点）

プローブ用サブエージェントに「自分の指示文に次の文字列が含まれるか」を引用付きで答えさせた。

| 文字列                             | 出所                 | 結果   |
| ---------------------------------- | -------------------- | ------ |
| 「3 つ以上あるときだけ使う」       | output style のみ    | **No** |
| 「ユーザーの口調（噛み合わせ用）」 | `rules/tone.md` のみ | Yes    |
| 「使われた用語＝理解済み」         | `rules/tone.md` のみ | Yes    |
| 「親しい若手の部下」として振る舞う | 旧 global CLAUDE.md  | Yes    |

**output style はメイン会話にしか届かず、`rules/` と global CLAUDE.md は届く。** よって `tone.md` は**サブエージェントにとって唯一の口調定義**であり、「output style と重複するから落とす」は成立しない（サブエージェントが口調規定を全部失う）。**ハード削り案を棄却し、圧縮に切り替えた。**

この実測がなければ -1,500 の案を選んで劣化させていた。**棚卸しは削る前に「誰に届いているか」を測る。**

### 8-2. 判定（5 反復 = 5 節）

| 節               | tok | 判定 | 根拠                                                                   |
| ---------------- | --: | ---- | ---------------------------------------------------------------------- |
| 基本トーン       | 276 | 畳む | output style に同等（移送先実在）。サブエージェントには芯 1 行で足りる |
| 助詞を省略しない |  99 | 畳む | output style に例文まで同文                                            |
| 呼びかけ         |  24 | 畳む | output style に同等。実際の呼び名は global CLAUDE.md 側に残存          |
| 説明の順序・体裁 | 357 | 圧縮 | 体裁規定は重複。**サブエージェント向け要点 3 つは固有なので残す**      |
| 応答前の自己確認 | 196 | 畳む | 本文の再掲 + output style にも同じ項目                                 |

**残した記述と理由**（1 行ずつ）:

- 比喩の具体例 3 つ — ラウンド 1 で global CLAUDE.md 側を消したため、**現在ここが唯一の所在**
- 「使われた用語＝理解済み、とみなさない」 — output style に無い固有の判断規定
- ユーザーの口調（噛み合わせ用） — output style に無い
- 良い例 / 悪い例 — output style が「詳細は `rules/tone.md` 参照」と**明示委譲している先**

### 8-3. トレードオフ（記録しておく）

サブエージェントは丁寧語・助詞・呼びかけの詳細規定を受け取らなくなる。サブエージェントの出力はユーザーに直接は届かず、要点は簡潔さ側にあるため許容と判断した（芯として丁寧語 1 行は残した）。**報告文が素っ気なくなったら差し戻す。**

### 8-4. ループ運用の実測

- 反復 **5**（上限 5・ちょうど使い切り）/ 所要 **約 25 分**（上限 30 分）
- **1 周目は `Skill` ツールから起動できなかった** — 新規スキルは merge してセッションを開き直すまでスキル一覧に載らない。定義に沿って手で回した
