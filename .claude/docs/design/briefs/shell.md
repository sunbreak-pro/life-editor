---
Status: Generated # Draft → Ready（ClaudeDesign 投入可）→ Generated（デザイン生成済み）。2026-07-05 Turn 2（rightSidebar + ハンバーガー）まで反映
Created: 2026-07-05
Section: shell
...
>
> **この brief は他の全 brief の基準**になる。2026-07-05 決定の目標 IA（サイドバー = 本流 5 + ユーティリティ枠 2 / Mobile 固定 4 タブ）を初めて視覚化し、**header タブの標準意匠（形状・アクティブ表現・件数バッジ）を定義**する。各画面 brief はここで定めたタブ・シェルの意匠を参照する。
...
- **新規に必要な部品候補**（部品層 `shared/src/components/` への追加候補として列挙するだけ。実装しない）:
  ...
  - `RightSidebar`（詳細パネル枠 + リサイズハンドル + 開閉トグル。2026-07-05 Turn 2 追加）
  - `MobileDrawer`（左スライドの drawer。BottomSheet とは別物。2026-07-05 Turn 2 追加）
...
- **メインエリア**（この 1 枚では Materials セクションを表示中というダミー）:
  - 最上部に **header タブ標準**を置く。この brief の核心なので丁寧に描く:
    ...
    - タブ列の右端（`margin-left: auto`）に **rightSidebar 開閉トグル**（PanelRight 型アイコン・28×28・角丸 6px。この基本形では closed = `text-secondary`、hover で `hover` 背景）
...
5. **ローディング**: サイドバー・header タブは即時表示のまま、タブ下の本文だけがスケルトン（Kanban 列の位置にグレーの角丸カードを数枚）。外枠は常設で中身だけが遅延することを示す
6. **（任意）header タブの標準見本**: Materials（タスク/ノート/デイリー/タグ・タスクにバッジ）だけでなく、Schedule（カレンダー/ルーティン・バッジなし）と Analytics（概要/タスク/集中/予定・バッジなし）のタブ列を並べた「タブ標準カタログ」1 枚。他 brief がコピーする基準として
7. **rightSidebar open**（2026-07-05 Turn 2 で追加・生成済み: フレーム 2a/2b）: メイン右端に幅 320px の押し込み式パネル（`bg-subsidebar` + 左 `border`・左端 6px リサイズハンドル、hover で accent 30%）。上部 48px に「詳細」+ 閉じる X。中身は選択中タスクの詳細（タイトル input「週次レビューの下書きを仕上げる」・ステータス チップ「◐ 進行中」・内容エディタはスケルトン）。タブ行右端のトグルは accent 文字 + `accent-subtle` 地の活性表示。light / dark 両方
...
- **メインエリア**（この 1 枚では Materials を表示中というダミー）:
  - 最上部の行は、左端の**ハンバーガーボタン**（Menu アイコン・36×36・`border` 付き・角丸 8px。タップで詳細パネルの drawer が開く — 状態バリエーション 5）+ **header タブのセグメントコントロール**（Mobile での header タブ標準・残り全幅）: `bg-secondary` の角丸トラック（角丸 8px、内側 2px パディング）に「タスク / ノート / デイリー / タグ」の 4 セグメントを等分割。**アクティブ（タスク）だけ `bg-primary` + 影 sm で押し出し + `text-primary` + `font-medium`**、他は `text-secondary`。件数は Mobile では省く（幅が狭いため。タブ名のみ）
...
4. **ローディング**: メインエリア本文の位置にスケルトン行（3〜4 本のグレーの角丸バー）。外枠（タブバー・セグメント）は即時表示で、中身だけがロード中という「シェルは常設・セクション本文だけが遅延」を示す
5. **ハンバーガー drawer open**（2026-07-05 Turn 2 で追加・生成済み: フレーム 2c）: 左から幅 320px の drawer（`bg-subsidebar`・右端 `border` + 影）+ 黒 30% スクリム。中身は Desktop の rightSidebar と同一（「詳細」ヘッダー + 閉じる X + タスク詳細カード）。ステータスバー / ホームインジケータの safe-area を drawer 内部で確保する
...
- [x] 共通前提ブロック（**v3 / 2026-07-05**）が全プロンプトの冒頭に**全文**埋まっている（要約・改変なし）
- [x] Desktop / Mobile 両方のプロンプトがあり、フレーム仕様（1440×900 / 390×844・light / dark）が明記されている
- [x] 通常（データあり）/ 空 / ローディング状態の指示がある（該当すればエラーも）。シェルは「空状態」= 該当なし（外枠は常にデータあり）のため対象外とし、代わりに 通常 / サイドバー折畳 / ⌘K open / offline / loading（Desktop・Mobile とも本文スケルトンのフレームを明記）で状態を網羅する
...
- [x] （shell 固有 AC・2026-07-05 Turn 2）rightSidebar（詳細パネル）標準（トグル位置・パネル意匠・Mobile ハンバーガー → drawer 継承）を定義し、共通前提 v3 として全 brief へ同期した

## 6. 生成後の運用メモ

- 分業: 生成 = claude.ai/design 側（ユーザーがプロンプト投入）/ Claude Code 側 DesignSync は同期専用 / 出荷 UI 化は `shared/src/components/` への移植（別計画）
- 移植先候補: `shared/src/components/AppShell.tsx` / `SidebarNav.tsx` / `NavItem.tsx` / `BottomTabBar.tsx` / `BottomSheet.tsx` + 新規候補 `HeaderTabs` / `SegmentedControl` / `SidebarGroup` / `Badge` / `RightSidebar` / `MobileDrawer`（生成結果を見てから確定。Turn 1 分は PR #160 で実装済み — HeaderTabs / SegmentedControl 新設、SidebarGroup / Badge は SidebarNav / HeaderTabs 内蔵で完結）
...
- **履歴**:
  - 2026-07-05: Turn 1 生成（project `c73cdbf4` / `App Shell.dc.html`・フレーム 1a-1l）→ Status: Generated。PR #160（shell-impl）は Turn 1 版準拠
  - 2026-07-05: Turn 2（ユーザーフィードバック）で **rightSidebar（詳細パネル・2a/2b）+ Mobile ハンバーガー → 左 drawer（2c）** を追加。§3 に標準を追記し、共通前提を v3 化して全 brief へ同期。ハンバーガーは Turn 1 の Mobile フレーム（1h-1l）にも遡及追加済み。**残課題（実装で補完）**: aria 属性なし / タスク未選択の空状態未デザイン / タブなし単画面のトグル置き場は未例示（補完標準 = 画面最上部右端）/ drawer の safe-area 未処理 / パレット外 hex 2 色（`#bfdbfe` = 2c スケルトンバー・`#25252b` = 2b dark カード地 → 実装では lumen トークンへ丸める）/ drawer open の dark フレーム無し
