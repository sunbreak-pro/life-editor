---
Status: Ready # Draft → Ready（ClaudeDesign 投入可）→ Generated（デザイン生成済み）。本 brief は _COMMON-CONTEXT v2 を最初から埋め込んで作成（2026-07-05 に v3 へ同期済み）
Created: 2026-07-05
Section: trash
...
> 各プロンプトの冒頭に `_COMMON-CONTEXT.md` の水平線以降（v3 / 2026-07-05）を全文コピー済み（改変・要約なし）。
...
- [x] `_COMMON-CONTEXT.md` の共通前提ブロック（**v3 / 2026-07-05**）が全プロンプトの冒頭に全文埋まっている（要約・改変なし）
