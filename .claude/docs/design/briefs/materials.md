...
>
> ⚠️ **既知の同期ずれ（brief 側では改変しない）**: `_COMMON-CONTEXT.md` の accent 系の値（accent light `#1f4fff` / dark `#5b82ff`・accent-hover・accent-subtle light `#e1e6fb`）が、正本 `shared/src/styles/tokens.css:48-56`（2026-07-05 に blue-700 `#1d4ed8` / dark `#5b8cff` / subtle `#dbeafe` へ更新）と食い違っている。共通ブロックは規約（要約・改変禁止）どおり原文のまま全プロンプトに埋め込んだ。修正は `_COMMON-CONTEXT.md` 側で行い、全 brief に一斉反映すること（→ §6）。
...
- [x] 通常（データあり）/ 空 / ローディング状態の指示がある（Kanban はエラー相当の空カラムも）
...
- **要対応（本 brief のスコープ外・別 PR）**: `_COMMON-CONTEXT.md` の accent 系 4 値が `tokens.css`（2026-07-05 更新）と乖離。tokens.css → _COMMON-CONTEXT.md → 各 brief の順で同期し、全 brief の埋め込みブロックを一括更新すること
