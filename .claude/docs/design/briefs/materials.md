...
Created: 2026-07-05
Section: materials
Owner-chat: design-materials
Branch: claude/design-brief-materials
---

# Design Brief: Materials クラスタ（Tasks Kanban / Notes / Daily / Tags）

> 目的: **この 1 ファイルだけで「ClaudeDesign に貼るプロンプト」とその根拠が完結する**こと。
> Materials クラスタは 4 サブ画面（Tasks Kanban / Notes / Daily / Tags）を持つため、§1〜§4 を 4 サブ画面構成に拡張している。§4 の各プロンプトは Desktop / Mobile のペア構造を維持する。
>
> ⚠️ **既知の同期ずれ（brief 側では改変しない）**: `_COMMON-CONTEXT.md` のパレットのうち accent 系（accent light `#1f4fff` / dark `#5b82ff`・accent-hover・accent-subtle light `#e1e6fb`）に加え、task チップ面色（bg `#e3e7ff` fg `#2330b0`）とそれらの dark 値も、正本 `shared/src/styles/tokens.css:48-56`・`86-88`（dark 側 `146-153`・`174-176`）と食い違っている（2026-07-05 に blue-700 `#1d4ed8` / dark `#5b8cff` / subtle `#dbeafe` / task チップ `#dbeafe`・`#1e40af` へ更新済み）。共通ブロックは規約（要約・改変禁止）どおり原文のまま全プロンプトに埋め込んだ。修正は `_COMMON-CONTEXT.md` 側で行い、全 brief に一斉反映すること（→ §6）。

## 1. 画面要件ダイジェスト

**クラスタ共通**: 4 画面は同じ「Materials（素材・記録）」クラスタとして、リスト密度・見出し・空状態・ローディングの意匠を統一する（§3 共通方針）。Mobile はいずれも Consumption（閲覧）+ Quick capture（最短記録）に責務を絞る。
...
### クラスタ共通方針（4 画面で統一 — プロンプトにも同文を埋め込む）

- **一覧の行**: 高さ 36px 前後・角丸 8px・1px border・地は bg-secondary。選択行 = accent 枠 + hover 地。行内アイコン 14px / text-secondary
...
- **残す意匠**: Kanban のカード / カラム語彙（4px バンド・チップ・ウォッシュ）は完成度が高いのでクラスタの基準にする。Notes のツリー行（hover で grip / 削除出現、アイコン入替）も維持
...
> 各プロンプトの冒頭に `_COMMON-CONTEXT.md` の水平線以降を全文コピー済み（要約・改変なし）。
...
## この画面: Tasks — Kanban ボード（Desktop 1440×900・light / dark 両方）

Tasks セクションの正面画面。タスクをカンバン（カラム + カード）で運用する。この画面は中央寄せ max-width を使わず、サイドバー右側の全幅を使う。カラム群が収まるときは中央寄せ、あふれたら横スクロール。

### Materials クラスタ共通の意匠（Tasks / Notes / Daily / Tags の 4 画面で統一）
...
1. ツールバー（上部 1 行）: 左に 3 択セグメンテッドコントロール「フォルダ / ステータス / タグ」— 角丸 12px の枠（bg-secondary 地）に入った 3 ボタン。各ボタンに lucide アイコン（Folder / CircleDot / Tag）+ ラベル。選択中は bg-primary + 小さな影で浮く。右端に「+ タスクを追加」ボタン（accent 塗り・Plus アイコン付き）
2. ボード: 横並びのカラム列。カラムは幅 316px・角丸 16px・1px border + md 相当の影・最大高さ 560px。カラム間 16px
3. カラムヘッダー（地は bg-secondary）: 上端いっぱいに高さ 4px のアクセント色バンド。中身は色ドット（フォルダ列 = 角丸四角 / タグ列 = 丸）またはステータスアイコン（ステータス列: Circle / CircleDashed / CheckCircle2）+ カラム名（15px bold・1 行省略）+ 右端に件数バッジ（丸ピル・bg-primary 地にアクセント色の数字）
4. カラム本体: 縦スクロールのカードスタック（内側余白 10px・カード間 10px）
5. フォルダ表示のとき: カラムパネル背景にフォルダ色を 10% 混ぜた薄いウォッシュ、ヘッダーは 18%。カード自体は不透明な bg-primary のまま浮かせる
...
## この画面: Tasks — Kanban（Mobile 390×844・light / dark 両方・下部タブバー込み・safe-area 考慮）

Mobile の責務 = 閲覧 + ステータス変更 + 最短のタスク追加。Desktop から落とすもの: カードのドラッグ&ドロップ / カラムの横並び（ステータスチップフィルタ + 縦 1 カラムに置換）/ カラム色編集 / タグ別・フォルダ別ビュー。
...
1. ヘッダー: 画面名「Tasks」（18px bold）+ 右端に「+」アイコンボタン（accent 色）
...
## この画面: Notes — 階層ノート（Desktop 1440×900・light / dark 両方）

長文ナレッジベース。左 = 階層ツリー（一覧）、右 = 選択ノートの詳細（リッチテキストエディタ）の 2 枚組（MasterDetail）。一覧 : 詳細 = 1 : 1.4 の 2 カラム。2 枚組のためこの画面は中央寄せ 768px より広め（コンテンツ幅 1080px 目安）で組む。
...
2. ツリー: フォルダ / ノートの階層リスト（行はクラスタ共通の意匠）。階層は縦のインデントガイド線で表現
...
## この画面: Notes（Mobile 390×844・light / dark 両方・下部タブバー込み・safe-area 考慮）

Mobile の責務 = 閲覧 + 最短のノート追加。Desktop から落とすもの: ドラッグ&ドロップ並び替え / フォルダ作成・リネーム / パスワードの設定・変更（閲覧のための解除だけ残す）/ タグ・リンクの編集（表示のみ）/ ゴミ箱。
...
1. ヘッダー: 「Notes」（18px bold）+ 検索アイコン + 「+」アイコンボタン（最短追加）
...
- ピン留め: 「今月の目標」
- フォルダ「レシピ（2）」（折りたたみ）／フォルダ「開発メモ（2）」展開中: 「Supabase 移行の設計メモ — 認証フローを Session pooler に…」「同期の落とし穴まとめ — updated_at を LWW cursor に…」
- ルート: 「パスワード管理方針」（Lock アイコン・抜粋なし）

### 状態バリエーション

- 通常 / 空状態（「ノートがまだありません」+ 追加ボタン）/ ローディング（2 行リスト形スケルトン）
- シートを開いた状態を 1 枚（閲覧ビュー）
```
...
## この画面: Daily — 日次ジャーナル（Desktop 1440×900・light / dark 両方）

1 日 = 1 エントリの思考ログ。日付が主役のジャーナル画面。中央寄せ max-width 768px の 1 カラム。
...
1. 日付ナビ（上部）: ◀（前日）/ 日付見出し「2026年7月5日（日）」（20px bold・主役）/ ▶（翌日）を中央寄せで並べ、右側に「今日」ボタン（border）と小さなカレンダーアイコン（日付ジャンプ）。今日を表示中は日付の脇に小さな「今日」チップ（accent-subtle 地）
2. エディタカード: その日の本文を書くリッチテキスト領域（角丸 12px・1px border・最低 10 行分の高さ）。プレースホルダ「今日の出来事や考えたことを書く…」。カード右上に保存状態の小さな表示（「保存済み」text-tertiary）とピン留めトグル（Pin アイコン）
3. メタ行: タグチップの列（+ タグ追加）とリンクの行（コンパクトに 1 行ずつ）
4. 過去の記録: 見出し「これまでの記録（14）」+ 日付リスト。各行 = 日付（「7月4日（土）」）+ 本文冒頭の抜粋 1 行（text-secondary・省略）+ ピンは Pin アイコン（accent 色）/ ロックは Lock アイコン。表示中の日は accent 枠
5. 最下部: 「ゴミ箱（1）」の折りたたみ行

### 表示データ

- 2026年7月5日（日）のエントリ: 日記調の本文 3〜4 文（「朝のうちに移行作業の残件を片付けた。午後は雨だったので家で読書。夕方に週次レビューを書いて、来週の段取りを整理した。」）+ 箇条書き 2 点（「買い出し: 洗剤・コーヒー豆」「明日: 粗大ゴミ回収の予約」）
- 過去リスト: 7月4日（土）「食洗機の修理業者に連絡。見積もりは…」／7月3日（金）「仕事の後にジムへ。体調は…」／7月1日（水・ピン留め）「7月の目標を書き出した。今月は…」／6月30日（火・ロック）／6月29日（月）／6月28日（日）の 6 件

### 状態バリエーション（各テーマで揃える）

- 通常: 上記
- 未記入の日: エディタが空で、内側に淡い空状態（「この日の記録はまだありません。書き始めましょう」+ ペン先アイコン）。過去リストは残る
- 全体空状態: 記録が 1 件も無い（CalendarDays アイコン + 「最初の 1 日を書いてみましょう」+ 「今日を書く」ボタン）
- ローディング: エディタカード + 日付リストのスケルトン
```
...
## この画面: Daily（Mobile 390×844・light / dark 両方・下部タブバー込み・safe-area 考慮）

Mobile の責務 = 今日をすぐ書く（Quick capture）+ 過去を読む（Consumption）。Desktop から落とすもの: ピン・削除・ゴミ箱の管理 / タグ・リンクの編集（表示のみ）/ カレンダーでの自由な日付ジャンプ（直近の日付ストリップで代替）。
...
1. 日付ストリップ: 横スクロールの日付チップ（「今日」「昨日」「7/3」「7/2」「7/1」…）。選択中は accent 塗り + on-accent 文字、他は border チップ
2. 今日のカード: 選択日の本文ビュー（角丸 12px カード）。今日を選択中は、カード下部に固定のクイック追記バー — 1 行入力（プレースホルダ「いま思ったことをメモ…」）+ 送信ボタン（accent の丸ボタン）。送信で本文の末尾に追記される。本文タップで全画面編集に入れる
3. 過去の日を選択中: 読み取り専用の本文ビュー（追記バーは出さない）。タグチップは下部に表示のみ
4. ピンやロックは表示のみ: 日付チップやカード見出しに小さな Pin / Lock アイコンを添える

### 表示データ

- 「今日」（7月5日・日）選択中: 本文 2〜3 文（「朝のうちに移行作業の残件を片付けた。午後は雨だったので家で読書。」）+ 追記バーに入力途中のテキスト（「夕方に週次レビューを書い」）
- 日付ストリップ: 今日 / 昨日 / 7/3 / 7/2 / 7/1（Pin アイコン付き）/ 6/30（Lock アイコン付き）

### 状態バリエーション

- 通常: 上記
- 今日が未記入: 空状態（「今日はまだ書いていません」+ 大きめの「今日を書き始める」ボタン）+ 追記バー
- ローディング: カード + ストリップのスケルトン
```
...
## この画面: Tags — タグ管理（Desktop 1440×900・light / dark 両方）

ノート / 日次 / 予定を横断するタグ体系のマスタ管理画面。左 = タグ一覧、右 = グループの 2 カラム。中央寄せ・コンテンツ幅 900px 目安。
...
1. 作成フォーム（1 行）: 「新しいタグ名」入力 + 「追加」ボタン（Plus アイコン）
2. タグ行のリスト（クラスタ共通の行意匠）: 各行 = タグピル（タグ色の点 + 名前の薄塗りピル。ピルの塗りはタグ色を 13% 混ぜた地・文字はタグ色寄り）+ 使用件数（「8 件」text-tertiary）+ 行 hover で右端に 色スウォッチ（小さな丸。クリックでカラーピッカー: グラフカテゴリ 10 色のスウォッチグリッド + クリア）/ リネーム（Pencil）/ 削除（Trash2）が現れる

### 右カラム: グループ（3）

1. 作成フォーム（同型）
2. グループカード（角丸 8px・bg-secondary）: グループ名（14px semibold）+ hover でリネーム / 削除アイコン。カード内にメンバータグのチップ列（各チップは タグピル + × で除外）+ 「タグを追加…」の検索入力 + 候補リスト（最大 8 件・タグ色の点 + 名前）

### 表示データ

- タグ: 「デザイン」（#8b5cf6・8 件）「経理」（#f59e0b・4 件）「健康」（#22c55e・6 件）「読書」（#ec4899・3 件）「Supabase」（#2563eb・9 件）「振り返り」（#06b6d4・5 件）「家事」（#84cc16・7 件）「急ぎ」（#ef4444・2 件）
- グループ: 「開発」（Supabase / デザイン）／「暮らし」（健康 / 家事）／「インプット」（読書 — 1 件だけ。「タグを追加…」の候補リストが開いた状態で描く）

### 状態バリエーション（各テーマで揃える）

- 通常: 上記
- 空状態: タグ 0 件（Tag アイコン + 「タグがまだありません。最初のタグを作りましょう」+ 作成フォームを指す）とグループ 0 件（「グループはまだありません」）
- グループにタグ未割当: カード内に「タグが割り当てられていません」の 1 行
- ローディング: 両カラムとも行スケルトン
```
...
## この画面: Tags（Mobile 390×844・light / dark 両方・下部タブバー込み・safe-area 考慮）

Mobile の責務 = タグとグループの閲覧 + 最短のタグ追加。Desktop から落とすもの: リネーム / 削除 / 色変更 / グループの編集（メンバー追加・除外）/ タグのマージ。
...
1. ヘッダー: 「Tags」（18px bold）+ 「+」アイコンボタン → タグ名 1 行入力のミニシート（入力 + 「追加」のみ）
2. セグメント: 「タグ / グループ」の 2 タブ（角丸 12px 枠のセグメンテッドコントロール）
3. タグタブ: タグ行のリスト（クラスタ共通の行意匠）— タグピル（色の点 + 名前）+ 使用件数 + 右端に ChevronRight（そのタグが付いたアイテム一覧へ進める想定）。編集アイコンは出さない
4. グループタブ: グループごとの見出し（14px semibold + 件数）+ メンバータグチップの読み取り専用の折り返し列。× ボタンや追加入力は出さない

### 表示データ

- タグタブ: 「Supabase」（#2563eb・9 件）「デザイン」（#8b5cf6・8 件）「家事」（#84cc16・7 件）「健康」（#22c55e・6 件）「振り返り」（#06b6d4・5 件）「経理」（#f59e0b・4 件）「読書」（#ec4899・3 件）「急ぎ」（#ef4444・2 件）
- グループタブ: 「開発（2）」Supabase / デザイン ／「暮らし（2）」健康 / 家事 ／「インプット（1）」読書

### 状態バリエーション

- 通常: タグタブ選択状態（グループタブは余力があれば別フレーム）
- 空状態: タグ 0 件（Tag アイコン + 「タグがまだありません」+ 「+」で追加できる旨）
- ローディング: 行スケルトン
```
...
- [x] Desktop / Mobile 両方のプロンプトがあり、フレーム仕様（1440×900 / 390×844・light / dark）が明記されている（4 サブ画面 × ペア = 8 プロンプト）
- [x] 通常（データあり）/ 空 / ローディング状態の指示がある（Kanban は移動拒否エラーの Toast 変形も指定）
...
- 移植先候補: `shared/src/components/`（EmptyState / SkeletonList / StatusFilterChips / QuickAddSheet / DateStrip / ListItemWithExcerpt — §3 の新規部品候補。生成結果を見てから確定）
- **要対応（本 brief のスコープ外・別 PR）**: `_COMMON-CONTEXT.md` の accent 系（accent / hover / subtle）・task チップ面色・それらの dark 値が `tokens.css`（2026-07-05 更新）と乖離（詳細は冒頭の注記）。tokens.css → _COMMON-CONTEXT.md → 各 brief の順で同期し、全 brief の埋め込みブロックを一括更新すること
- 要件ドリフト: `tier-2-supporting.md:168`（WikiTags = Desktop only）は旧 Tauri 期の記述。現行 CLAUDE.md §2 では WikiTag は Mobile でも有効 — 本 brief は CLAUDE.md 準拠
