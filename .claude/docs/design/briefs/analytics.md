...
- ⚠️ **\_COMMON-CONTEXT.md のパレット表が tokens.css より古い**: accent 系が旧値（light `#1f4fff` / dark `#5b82ff` 等）のままで、2026-07-05 の Lumen blue 深化（light `#1d4ed8` / dark `#5b8cff`、`shared/src/styles/tokens.css:48-56` / `shared/design-system/PRINCIPLES.md §3.3`）が未反映。本 brief は規約どおり全文を**無改変で**埋め込んだ。**投入前に \_COMMON-CONTEXT.md 側を同期 → 全 brief の埋め込みブロックを更新**する必要がある（_COMMON-CONTEXT.md の変更は本ストリームのスコープ外のため未修正）
