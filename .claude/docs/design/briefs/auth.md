...
> ⚠️ **本文中の「6 文字以上」は 2026-07-05 時点の仕様で、現在の下限は 12 文字**（#956 / PR #967 で引き上げ・裁定 = D-20260816-shared-fix-4 = A）。ClaudeDesign へ渡した時点のブリーフなので**歴史として据え置き**、本文は書き換えていない。実装の正本は `shared/src/constants/password.ts` の `PASSWORD_MIN_LENGTH` で、文言は `{{min}}` 差し込みなので数値を持たない。ここを grep して古い下限を拾わないこと。
