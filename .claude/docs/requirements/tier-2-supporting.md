...
Tasks / Notes / Memos / Schedule など異種エンティティを横断して単一のタグ体系で束ねる「wiki-style」タグシステム。インラインタグ（`#tag` 記法）による自動認識と、タグの有向接続 / グルーピングで知識の構造化を支援する。
...
- [ ] AC1: Task / Note / Memo / Schedule Item のいずれのエンティティにも同じ WikiTag を付与でき、`search_by_tag` で 4 ドメイン横断結果が返る
...
- 他機能: Tasks / Notes / Memo / Schedule / Database（entity_type を拡張すれば対応可）
...
- 保留 S-5: `useServiceErrorHandler` 共通ヘルパ未実装（各 Provider が try/catch + showToast を重複実装）→ **Phase C Verdict: Keep**、`.claude/feature_plans/2026-04-18-service-error-handler-hook.md` で実装着手

### Related Plans

- PLANNED: `.claude/feature_plans/2026-04-18-service-error-handler-hook.md`（S-5 実装）
