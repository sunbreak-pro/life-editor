...
**Platform**: Desktop only（Mobile では AudioProvider 省略 — CLAUDE.md §2）

### Purpose

集中作業中の環境音ミキサー。6 種プリセット（Rain / Thunder / Wind / Ocean / Birds / Fire）+ カスタムサウンドを個別にミックスし、Pomodoro Timer / Playlist と連動して「没入型ワークフロー」を形成する。Tier 1 機能には寄与しないが、V2 Target User の特性（「環境音 + ポモドーロタイマーの没入型」志向）を直接満たす補助機能。
...
- `AudioContext` state suspended → ユーザー操作後 `resume()` 必要（CLAUDE.md §3 gotcha）
...
- やる:
  ...
  - ~~タグのマージ（`merge` コマンドで重複統合）~~ → **2026-09-23 #1886 で機能ごと削除**: Connect の「別のタグへ統合」（#1644）を撤去した。統合は「付け替え → 元タグをソフトデリート」で動いており専用の DDL は無かったため、スキーマは変えていない。一括の「移動」（`moveItemsToTag`）は残る
...
- やらない:
  ...
  - Memo テンプレート（現状未対応、Future で検討）
  - テンプレートのバージョニング / 差分管理
  - テンプレート自体へのタグ / リンク付与（#1047 — テンプレートはスタンプであってグラフ上のアイテムではない。タグとリンクは、そこから作られたノート側に付ける）
  - 削除したテンプレートの TrashView 復元（#1047 — ソフトデリートはするが、Trash はテンプレート行も除外するので UI からは戻せない）

### Note テンプレート（2026-08-18 #1047 で追加・上の Todo ツリー版とは別系統）

Tauri 時代の Todo ツリー版（本節の本文）は `frontend/` + `src-tauri/` ごと削除済みで、再利用できる資産は残っていない。#1047 の Note テンプレートは新スタック上に別途組んだもので、**専用テーブルを持たず `notes_payload.note_type='template'` の notes 行**として表現する（migration `0024_notes_template_type.sql`）。

- 入口 = ノート詳細の 3 点メニュー「テンプレートを作成する」→ パネル（Mobile は全画面シート = 画面遷移）
- UI はノート詳細とほぼ同じ（タイトル部分がテンプレート名 + TipTap 本文）。タグ行・リンク行は無く、本文エディタにも `[[` の loader を渡さない（機能ごと不在）
- 「このテンプレートからノートを作成」で、名前と本文を引き継いだ**通常のノート**を作る。以降はタグもリンクも普通に付く
- テンプレート行は一覧 / 検索 / タブのバッジ件数 / ゴミ箱から除外され、`listNoteTemplatesUnified` だけが返す
...
ダーク / ライトモードとフォントサイズ（12-25px の 10 段階スライダー）を Settings から切替可能にする。Tailwind デザイントークン（`lumen-*`）経由で全 UI コンポーネントに一貫適用される。
...
- ライブラリ: Tailwind CSS v4 デザイントークン（`lumen-*`）
...
- やる:
  ...
  - 共有コンポーネントは props で i18n テキストを受け取り、自身では `useTranslation` を呼ばない（CLAUDE.md §6 Coding Standards / rules/frontend.md）
- やらない:
  - 他言語追加（必要時に検討）
...
- [ ] AC2: 新規 UI テキストは `en.json` / `ja.json` 両方にキー追加必須（CLAUDE.md §6 Coding Standards 準拠）
...
**MCP Coverage**: —
**Platform**: Desktop only（Mobile では ShortcutConfigProvider 省略 — CLAUDE.md §2）
...
**2026-09-16（#1643）でタグマスタの編集所を兼ねる**（[`D-20260912-main-1`](../../decisions/D-20260912-main-1.md) Q1-A）。同じタグ一覧が本セクションのレールとタグ編集モーダルの 2 画面にあり、行の形まで揃えてあるのに編集できるのはモーダル側だけ、という二重保守を畳んだもの。モーダル（`shared/src/components/tagEdit/`）と、それを開いていた `web/src/tags/TagEditorHost.tsx` は削除済み。Mobile「その他」シートの「タグを編集」は本セクションへの遷移に差し替わった。wide 側のサイドバー行も同じ遷移にしたが、nav の「つながり」と行き先が同じで重複するため後に削除した。
