...
**Tier**: 1
**Status**: ✗RETIRED (2026-07-05) — 機能ごと廃止をユーザー決定。以下の本文は履歴として保持。⚠️ 退役するのは Terminal（Claude Code の起動導線）のみで、**MCP Server（別 Feature・存続）ではない**。`SectionId` の `terminal` / `TerminalPanel` / `portable-pty` 系コマンドのコード除去は別 Issue で追跡（→ CLAUDE.md §3.2 / §8）
