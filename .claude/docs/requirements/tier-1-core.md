...
- やる:
  - **紙面表示（Step 1 出荷済み）**: 今日の約束（= ScheduleItem）・今日のタスク（= TaskNode）・持ち越し・フォーカス・AI 講評の各ブロック（列挙の出典 = briefing-loop §1。実装上の全ブロック構成は `BriefingView.tsx` が正 — Analytics のウィジェット群を移入した視覚ゾーンを含む）
  - **朝刊の保存規約（`extractBriefing`）**: TipTap JSON の `heading` ノード（レベル不問）でテキストが「朝刊」/「Briefing」（大文字小文字不問・trim 後完全一致）のセクションを抽出。セクション内の最初の非空テキストブロック = フォーカス行・以降 = AI 講評（paragraph 限定ではなくリスト等も 1 ブロック扱い）。次の heading（テキスト不問）でセクション終了。content が欠落・パース不能・セクション無しは `null` → 紙面は空状態表示
  ...
  - 約束行の完了トグル（現行 = 行頭チェックアイコンのボタン。名称タップは現状 no-op）。名称タップへの拡張・タスク行への同型追加・各行からの移動ボタンは loop-friction-fixes F-2
- やらない:
...
- [ ] AC2: 当日 DailyNode content に「朝刊」（or Briefing）見出し + 本文があると、最初のテキストブロックがフォーカス行・以降が AI 講評として紙面に出る。セクションが無い / パース不能な日は空状態が出てクラッシュしない
- [ ] AC3: 約束行のチェックアイコンで完了トグル（取り消し線）が働き、Schedule 側と同じ完了状態を共有する（名称タップへの拡張は F-2）
