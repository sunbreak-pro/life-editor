...
> **Status (2026-07-07 consistency-cleanup で実態化)**: **G 系のみ Done** — G-1 / G-2 / G-3 / G-5 は 2026-04-24 実装完了済（G-4 は △ 部分着手）。後続セクション（M 系 / C 系など）は未着手が残り、Done 表記の項目も AC 未チェックがある。全体が ALL DONE ではない。本ファイルは Tier 1/2 の Mobile 補完要件の SSOT として **archive せず保持**。Capacitor 移行 (移行 SSOT Phase 4) で再評価する。
>
...
- やらない:
  ...
  - Mobile 省略 Provider 依存の UI（省略 4 種 = Audio / ScreenLock / FileExplorer / ShortcutConfig。CalendarTags は撤去済・WikiTag は Mobile 有効で対象外 — CLAUDE.md §2）は Optional hook で `null` ガードして除外（CLAUDE.md §6 / rules/frontend.md）
...
- 設計制約: CLAUDE.md §6 / rules/frontend.md Optional hook（Mobile 省略 Provider 必須）
...
**Depends on Feature**: Section Routing（CLAUDE.md §3.2 — `App.tsx` の `activeSection`）
**Platform**: iOS only
...
- 既存資産: `MobileScheduleView.tsx` スワイプパターン / `MobileActionSheet` / ソフトデリート機構（CLAUDE.md §4 Data Model）
...
- [ ] AC3: IME 変換中はヒントが表示されない（CLAUDE.md §6 / rules/frontend.md IME 対応）
...
- やる:
  - Task Picker のラッパ背景の opacity / alpha を除去し、`bg-lumen-bg-primary` 等の solid 色に
...
- 既存資産: `MobileWorkView.tsx` / lumen-bg デザイントークン
...
- 関連デバッグ: `add-ipc-channel` スキルの IPC 未登録トラブルシュートと同じ文化で root cause を `known-issues/` に記録
