...
| 4   | schedule カレンダー          | Consumption                                               | Phase 1（現状維持）+ #467          | #467 で narrow は **アンカー日のリスト + FAB の単画面**（Month / Timeline の切替は switcher ごと退役 — Epic #290 Step 5-c）。日移動は前後 1 日ステップ。#411 の Calendar / Todo の SegmentedControl（`MainScreen.tsx` `scheduleMobileSwitcher`）はそのまま                                                                                                                   |
