...
Claude Code が notion-timer の SQLite データベースにアクセスできる MCP Server を構築する。
...
```bash
node mcp-server/dist/index.js /path/to/sonic-flow.db
```
...
**重要**: notion-timer の既存 Repository パターン（`electron/database/` 内）を参考にする。
...
```
node /path/to/notion-timer/mcp-server/dist/index.js /path/to/sonic-flow.db
```

### タスク ID の形式

notion-timer のタスク ID は `"task-{timestamp}"` または `"folder-{timestamp}"` 形式。
