...
**⚠️ MCP life-editor サーバーは旧 SQLite（`.mcp.json` の `DB_PATH` = `~/Library/Application Support/life-editor/life-editor.db`）を読んでおり本番と別データ**（folder 1 件 / tags 3 件と、本番の folder 9 件 / tags 4 件が不一致）。実測は Supabase 側を正とした。
...
| mapper / hooks   | `services/taskMapper.ts`（`task_type`/`folder_type`）/ `notesUnifiedMapper.ts`（`note_type`）/ `hooks/useTaskTreeCRUD.ts`（Complete-folder 自動生成:43-72,294-341）/ `useTaskTreeMovement.ts` / `useNotesUnifiedAPI.ts`（`createFolder`:446-475）/ `utils/sortTaskNodes.ts` / `getDescendantTasks.ts` / `folderTag.ts` |
