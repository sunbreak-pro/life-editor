...
### W7 — セクション内部深化 第2弾（Tasks 詳細パネル / Master-Detail 採用）★子計画書 `2026-06-18-web-parity-w7-task-detail.md`

- W6 の共有 `MasterDetail` を**第2採用先＝Tasks** に展開。Notes と違い Tasks は選択状態を持たないため、`useTaskTreeAPI` への**選択基盤追加**（`selectedTaskId` / `setSelectedTaskId` / `selectedTask`）+ 新設 `TaskDetailPanel`（純粋表示・TipTap は host slot 注入）が要点。`MasterDetail` は無改変で再利用。Schedule カレンダー充実 → **W8**。
- Gate: 🤖 実装 → 👀 目視 → 🛑 PR merge
