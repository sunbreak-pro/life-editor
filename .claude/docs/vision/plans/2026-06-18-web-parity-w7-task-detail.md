...
- **制約**:
  ...
  - 既存の不変式を維持: DataService 境界（§3.1/§6.4・注入のみ）、Provider ネスト順（§6.2）、section ルーティングは `useState`（§3.2・React Router 不使用）、新規 UI は `shared/src/components/` 集約（§6）、`ink-*` トークン厳守・主要 UI 背景に透明度禁止、i18n は props 注入（shared 内 `useTranslation()` 直呼び禁止）。
...
| 2 ✅ | `TaskDetailPanel` 新設（title 編集 / status トグル / content スロット注入・`ink-*`・props 注入）                | 🤖 自律 | `cd shared && npm run build` exit 0                 |
...
- frontend 規約: `../../../rules/frontend.md`（Provider 順序 / Pattern A / `ink-*` / i18n props 注入）
