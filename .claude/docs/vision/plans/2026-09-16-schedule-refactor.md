...
G-01〜G-04 は [`mobile-scope.md`](../../requirements/mobile-scope.md) の「Consumption + Quick capture」と整合する可能性がある。**意図した省略なのか穴なのかはユーザー裁定が要る**（判断キュー Q1 → W12）。
...
| 繰り返しの「範囲」を全入口で必ず聞く                     | ✗    | 現状の「聞かない」5 経路には個別の理由がコメントで書かれており、仕様の是非はユーザー判断 | 判断キュー Q2 の回答が B / C なら W6 で実施                |
| narrow のグリッド書き込みを本計画で足す                  | ✗    | 機能追加で「挙動変更ゼロ」から外れる。mobile-scope.md の意図的な省略かも未確定           | 判断キュー Q1 が「穴」だった場合、別 Issue で実施          |
...
| W6  | 繰り返しの範囲確認の境界を実装に落とす（E-01〜E-09）                 | 裁定 Q2 | 🛑 人手 | 判断キュー回答後に着手。入口ごとの差が表から消える                          |
...
| W13 | narrow の書き込みの穴（G-01〜G-04）の扱いを裁定に従って処理          | 裁定 Q1 | 🛑 人手 | 「意図的」なら本書へ追記のみ。「穴」なら別 Issue を chat-main へ依頼        |
...
**着手順の要**: W0 を必ず最初に置く（以降の回帰検知がすべてここに乗る）。W1 は独立でファイルが重ならないため並行してよい。W3 で原因が B-01 と確定した場合、`UndoRedoContext` は Scope 外なので**触らず、Schedule 側の回避で済ませるか別 Issue に出すかを PR 本文で明示する**。W6 と W13 は裁定待ちで、回答が来るまで着手しない。
...
- **2026-09-16**: 工程 1。4 領域を並列調査し、主要な主張 16 件をメインが全数 spot check した（結果は §Context の表・棄却ゼロ）。棚卸し 60 件・書き込み経路 20 操作・作業単位 16 本を確定。コードは 1 行も変えていない。
  - 判断キューへ 2 件積んだ。Q1 = narrow にグリッド書き込みが無いのは意図的な省略か穴か（→ W13）。Q2 = 繰り返しの範囲をどこまで聞くか（→ W6）。どちらも回答が来るまで該当作業単位に着手しない。
