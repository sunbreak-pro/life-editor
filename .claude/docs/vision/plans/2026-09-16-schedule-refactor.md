...
**2026-09-21 時点の消化状況**（§1-K の 16 件のうち、着地したものと Issue になったもの）:

| ID                   | 状態                                                                            |
| -------------------- | ------------------------------------------------------------------------------- |
| K-01 / K-02          | #1801（D-20260919-sched-2 = B）で Undo に載せた。タグが戻らないことはダイアログに明記 |
| K-08                 | #1770 = PR #1787 で survivor 全員へ渡す形に広げた（merge 済み）                 |
| K-12 / K-14          | #1771 = PR #1788 で戻り値を読む形にし、両方落ちたときだけ報告する（merge 済み） |
| K-15（D-05 の一部）  | #1769 = PR #1785 で `landed` にぶら下げた巻き戻しを入れた（merge 済み）         |
| B-14（K の隣）       | #1772 = PR #1776 / #1767 = PR #1778 で undo の失敗を manager へ返す（merge 済み） |
| **K-07 / K-10**      | **本書 W16。複製のリマインダー引き継ぎと、落ちた複製の巻き戻し + 報告**        |
| K-03 / K-05 / K-09 / K-11 / K-13 / K-16 | 未着手。W14 の後に Issue 化するかを決める（起票は chat-main）      |

**K-06（複製がタグを引き継がない）は W16 に入れない。** タグを写すには複製がタグの書き込み経路を持つ必要があり、`shared/src/hooks/useWikiTagsUnifiedAPI.ts` は本書の Scope 外である。加えて #1800 がその層を Undo に載せたため、複製が n 個の付与を撃つと **1 押下 = 1 エントリ**という複製の約束（`useScheduleMutations.ts` の #223 コメント）が n+1 エントリに割れる。どう畳むかは仕様判断なので、実装せず記録にとどめる（P-008）。
...
| W16  | 複製が元の予定を写し、落ちたら黙らない（K-07 / K-10）               | —       | 🤖 自律 | 複製がリマインダーを引き継ぎ、失敗した INSERT の楽観行が消えてトーストが出る・CI 緑 |
...
- **2026-09-21**: W16。複製の 2 件（K-07 / K-10）を直した。`handleDuplicate` が `reminderOffset` を渡すようにし、`onSaved(null)` で楽観行と選択を戻してホストへ報告する。リマインダーは画面に顔を持たないので、既定値にすり替わっても**間違った通知が来るまで気付けない**のが K-07 の性質だった。K-10 は「他のすべての書き込みは報告するか reload で戻るのに、複製だけがどちらもしない」非対称で、サーバが見たことのない id にエディタが書き続ける形になっていた。**K-06（タグの引き継ぎ）は入れていない** — 理由は §1-K の表の下に書いた。
  - 同じ日に #1801 / #1802 / #1800 を別ブランチで PR 化した。#1800 は調査の結果**やること 1・2 が #1667（PR #1697）で着地済み**と分かったため、残っていた「5 Context と形を揃える」だけを扱い、ついでに **ambient 経路に守りが 1 本も無かった**（既存スイートは全部 `undoRedo` prop を明示注入していた）ことを塞いだ。
