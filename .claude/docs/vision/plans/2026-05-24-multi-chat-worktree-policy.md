...
- [x] `grep -n worktree .gitignore` で 1 行以上ヒット
- [ ] `grep -n "7.4" .claude/CLAUDE.md` で §7.4 ヘッダが存在
- [ ] `grep -n "session-branch" .claude/hooks/session-start-check.sh` で検査 F の実装が存在
- [ ] `bash .claude/hooks/session-start-check.sh; echo $?` で exit 0
- [ ] `grep -n -i worktree ~/dev/Claude/skill-lib/global/lead-pipeline/SKILL.md` で本規約節が存在
- [ ] `grep -n -i worktree ~/dev/Claude/agents-lib/global/session-manager.md` で worktree 検査記述が存在
- [ ] `grep -n -i worktree ~/dev/Claude/agents-lib/global/git-orchestrator.md` で main 専有節が存在
- [ ] `cd frontend && npm run build` exit 0（型崩れがないこと、frontend には触らないので自明）
