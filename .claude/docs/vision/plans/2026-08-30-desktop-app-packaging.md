---
Status: IN PROGRESS — Step 1-5 / 8-10 は着地済み（PR #1348 / #1360）。Step 3 は 2026-09-05 に `workflow_dispatch` を実走させて win / mac 両ジョブ success を実測（run 33958069275）。Step 7 の Windows 実機受け入れも同日、その run の artifact を入れて 6 項目中 5 項目まで通した。**Step 8（Mac 実機）は 2026-09-07 に全項目通過し、移行 SSOT の Phase 3 完了判定が埋まった**。**Step 6 も 2026-09-09 に消化** — こうだいさんが tag `desktop-v0.1.0` を push し、run 34342012501 が 3 ジョブとも success、draft Release に `.exe` + arm64 `.dmg` の 2 点が乗った（Worklog の同日エントリ）。**残りは Step 7 の最後の 1 項目（Windows 実機での実アカウントログイン + Todo CRUD）・Step 11（merge）と、draft の公開判断**
Created: 2026-08-30
...
**2026-09-09 回答 = A（arm64 のみ）** → 台帳 [D-20260830-main-3](../../../decisions/D-20260830-main-3.md)（キューの ID `-1` は台帳で衝突していたため `-3` へ振り直し）。`electron-builder.yml` から x64 を消し、workflow の `unverified-*` artifact も撤去。
...
| 6   | tag `desktop-v<version>` を打って Release 発行                | 🤖 自律 | `gh release view desktop-v<version> --json assets` に `.dmg` と `.exe` ✅（2026-09-09 / 2 点）  |
| 7   | Windows 実機の受け入れ（#1300）                               | 👀 目視 | インストール → 起動 → ログイン → Todo 追加 / 編集 / 削除（6 項目中 5 通過・残り = 実アカウント） |
...
- **2026-09-09 (chat-main) — Step 6 実走**: こうだいさんが tag `desktop-v0.1.0` を push（run 34342012501）。build 2 OS + `draft GitHub Release` の 3 ジョブとも success。draft Release に `Life.Editor-0.1.0-arm64.dmg`（103,608,244 B）と `Life.Editor-0.1.0-x64-setup.exe`（84,351,933 B）の 2 点。release ジョブのガード 2 本（版一致 / 資産集合）は初回で通過。同日 D-20260830-main-3 = A の回答を受け、x64 dmg の宣言を落とした（この run は回答前なので `unverified-desktop-macos-x64` artifact を持つが Release には載っていない）。公開（draft 解除）はユーザー手番。
...
- **2026-09-12 (refactor-core / #1300) — Step 10 の追随**: Release 発行後の状態を `gh` で再実測し、本計画書の Status 行と移行 SSOT を事実に合わせた。直した中身は 2 つ。(1) Status 行が「残りは Step 6」と言っていたが、同じファイルの Worklog には 2026-09-09 の実走が書かれていた — **1 つのファイルの中で前後が食い違っている**状態で、先頭だけ読む人が「まだタグを打っていない」と受け取る。(2) 移行 SSOT は 2026-08-31 のエントリで「tag はまだ打っておらず `gh release list` は空のまま」と書いたまま止まっていた。日付つきの進捗は書き換えずに後継を足す規約（`rules/records.md` §3）なので、旧文には解消済みの注記だけ足し、実測は同日エントリとして下に置いた。実測値: draft Release `desktop-v0.1.0` に `.exe` 84,351,933 B / arm64 `.dmg` 103,608,244 B、run 34342012501 は 3 ジョブ success、Release は**まだ draft**。コードは 1 行も触っていない（配布経路の実装は 2026-08-31 / 09-05 で着地済み）。
