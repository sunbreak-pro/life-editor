...
shared/src/hooks/index.ts                   ← barrel export 追加（既存があれば）
...
| 7   | レスポンシブ/操作感の目視（広幅サイドバー↔狭幅ボトムタブ切替・全セクション遷移・Cmd+K・折りたたみ）     | 👀 目視 | 主要動線を手で1周（広幅/狭幅とも）                          |
...
- [ ] `cd shared && npm run test`（vitest）全 pass（`useMediaQuery` / `AppShell` の最小 test 含む）
- [ ] `cd web && npm run build`（tsc -b --force && vite build）exit 0
- [ ] `cd frontend && npm run build`（旧 Tauri 非破壊の担保・並立期間中）exit 0
- [ ] `shared/src/i18n/locales/en.json` と `ja.json` で新規ナビラベルキーが**両方**に存在
- [ ] PR diff が ±500 行以内（機能追加目安。scope creep ガード）
