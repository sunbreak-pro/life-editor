...
| R9  | **v3-rev3 追加 (DU-B-2 role-qa Minor-1)**: 将来 TaskNode に `startAt` / `dueAt` を追加した瞬間、mapper の `start_at: null` / `due_at: null` 既定書き込みで silent loss が発生する (現状 TaskNode に対応フィールド無し、reservation only) | 低     | TaskNode フィールド追加 PR の責務として `taskMapper.ts` L228-230 / L324-327 と同時編集を必須化。子計画書ではここで 1 行記録のみ。追加 PR レビュー時に「将来予約列の silent loss」を check してもらう                                      |
