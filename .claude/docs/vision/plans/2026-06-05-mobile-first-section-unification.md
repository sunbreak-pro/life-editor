...
| 2     | **Schedule**        | `MobileCalendarView` + `Mobile/schedule/*`     | `Schedule` 系（`Tasks/Schedule/*`） | 未設計（着手時に詳細化）  |
...
## Phase 2 — Schedule（未設計・着手時に詳細化）

- **正**: `MobileCalendarView` + `Mobile/schedule/*`（`MobileDaySheet` / `MobileMonthlyCalendar` / `MobileDayflowGrid` / `MobileEventChip`）
- **Desktop**: `Tasks/Schedule/*`（3 分割 Provider: Routine / ScheduleItems / CalendarTags）
- **着手時にやること**: プレイブック §1 棚卸し（Desktop の月送り/ビュー切替/リサイズ等の扱い）。Schedule は状態が大きいので、`Schedule/shared/` 既存資産との整合を要確認。
- Gate/Acceptance/Scope は着手時に本節へ追記（または子計画書 `2026-NN-NN-schedule-mobile-unify.md` へ分割）。
