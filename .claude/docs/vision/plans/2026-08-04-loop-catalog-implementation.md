...
- **カタログが増えて選べなくなる** — 棚卸し（`loop-prune`）は姉妹計画の管轄。**2026-08-06 に配置され 5 本目として加わった**ので、「4 本上限」の条件（= `loop-prune` が来るまで）は解けている。以後の上限管理は `loop-prune` 自身が持つ（同スキルの `## 環境の事実` に「1 本増やす前に 1 本畳めないか確認する」を継承済み — カタログ自身も棚卸しの対象）
