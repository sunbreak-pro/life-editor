...
- [ ] 5-3. `requirements/ios-additions.md` が**保持され**冒頭に `Status: ALL DONE` マークと各 Tier への統合リンクが追加されている (Phase 3-4 方針変更後)。
...
- [ ] **iOS 要件の保持と ALL DONE マーク**: `requirements/ios-additions.md` が**存在し** (`test -e .claude/docs/requirements/ios-additions.md`)、冒頭に `Status: ALL DONE` マークが入っている (`grep -q "Status.*ALL DONE" .claude/docs/requirements/ios-additions.md`)。Phase 3-4 で方針変更（archive → 保持 + ALL DONE マーク）。
...
2. ~~**Phase 2-6 の `coding-principles.md` 縮減**~~: **解消** — Phase 2-6 で「§1 Tauri IPC のみマーク追加・§2-6 保持」にスコープ縮小し、機械的削除を行わない方針に変更したため Risk 消滅。
3. ~~**Phase 3-4 の iOS 統合**~~: **解消** — Phase 3-4 で「archive せず保持 + ALL DONE マーク + 各 Tier への統合リンク」方針に変更したため分散統合自体を行わず、Mobile Support 節の肥大化リスクが消滅。
...
2. 本計画書を `archive/2026-05-23-cleanup-and-consolidation.md` に移動（archive/plans/ ではなく直下が慣習。Phase 1-1 の commit 77afff6 で確定済）。Status を COMPLETED に。
3. `deletion-targets.md` は移行 SSOT Phase 5 実削除完了まで `vision/plans/` に残置 → 実削除完了後に archive へ追従（移行 SSOT 側で消化されるまで現役）。
4. `archive/SUMMARY.md` に本計画の総括を追加。
5. 残る課題（Phase 5 = 物理削除）は移行 SSOT 側のチェックリストに移譲。
