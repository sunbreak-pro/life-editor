---
Status: IN-PROGRESS — W8-1（週グリッド描画 + 週ナビ・read-only）を web-first で実装・機械検証 green（下記 Worklog 2026-06-20 参照。shared `WeekTimeGrid` 化 + 狭幅アジェンダ + DnD は未）
Created: 2026-06-19
...
- 2026-06-20（app-dev-roadmap チャット・W8-3 ドラッグ移動/リサイズ）: ネイティブ pointer イベントでイベント本体ドラッグ=移動（縦=時刻 / 横=曜日列）、下端ハンドルドラッグ=終了時刻リサイズ（共に 30 分 snap）。移動中は weekItems を楽観 re-layout で追従、pointer-up で `updateScheduleItem` 永続。閾値 4px 未満は click 扱い（選択）。`touchAction:none` + ドラッグ中 `select-none`。検証 = web build / web eslint exit 0（shared テストは 17 のまま）。これで W8（描画→作成/編集→DnD）コア3スライス完了。残課題 = shared `WeekTimeGrid` 化（2層 responsive・狭幅アジェンダ）/ i18n / 週キャッシュ自動再整合。
- 2026-06-20（app-dev-roadmap チャット・W8-2 クリック作成/編集）: `WeekGrid` に編集系を追加 — 空きスロット click で snap した時刻に 1h イベントを楽観生成（`createScheduleItem` + 即時ローカル反映）、イベント click でグリッド下の編集パネル（title / start / end / all-day / done / delete or routine は dismiss）を開く。全ミューテーションは weekItems を楽観更新 + ScheduleItems DataService 経由で永続（refetch レースを回避、Refresh で再整合）。純関数 `minutesToTime` / `snapMinutes`（最終スロットへグリッド整合クランプ）を shared に追加・4 tests 追加（計 17）。検証 = shared build / 17 tests / web build / web eslint いずれも exit 0。残 = W8-3 DnD 移動/リサイズ。
