...
**Status**: Consumed (Phase A-2 完了 — 内容は `.claude/CLAUDE.md` に統合済み)
**Date**: 2026-04-17
**Consumed**: 2026-04-18
**Related**: ADR-0005 (Claude Cognitive Architecture)

> **NOTE**: ビジョン・機能マップ・ギャップ分析・統合アーキテクチャ・実装フェーズ・設計原則は `.claude/CLAUDE.md` の §1-5/7.4/8 に吸収されました。Google Calendar / NotebookLM 連携の検討内容も §11 Tier 3 と §8.4 シナリオに反映。本ファイルは履歴保存のため残置。
...
| フェーズ             | 内容                                       | 日常生活ハブとの関係                                       |
| -------------------- | ------------------------------------------ | ---------------------------------------------------------- |
| Phase 1: 記憶基盤    | claude\_\* テーブル + mcp-server-cognitive | 「こうだいは午前中に集中作業が得意」等のパターン学習       |
