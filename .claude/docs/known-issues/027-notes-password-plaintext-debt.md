# 027: Notes password が plaintext で保存される（SECURITY DEBT）

**Status**: Active (Backlog)
**Category**: Security
**Severity**: Important (N=1 では実害なし、N>1 で即 HIGH 化)
**Discovered**: 2026-05-24 (DU-G G1 security-reviewer 監査で明文化)
...
Legacy `useNotesAPI.ts` (Tauri 時代) が plaintext を `password_hash` 列に保存する設計だった。DU-G G1 で Unified 経路に移植する際、UX 完全パリティを優先して **legacy mandate** として継承した。

## Impact

放置 / 再発した場合:

- **N=1（現状、作者単独 Cloud Sync）**: 実害なし。RLS policy `notes_payload_select_own` (migration 0008) が `auth.uid() = user_id` で他ユーザ row を SELECT 段階で遮断。`NOTES_PAYLOAD_COLUMNS` (notesUnifiedMapper.ts:158-161) からも `password_hash` は除外され、`verifyNotePasswordUnified` のみが SELECT する
- **N>1（友達 MVP 拡張時に即 HIGH 化）**:
  - Supabase DB バックアップ / PITR dump に plaintext password が残る
...
**現状の Mitigation**（DU-G G1 で適用済）:

- `NOTES_PAYLOAD_COLUMNS` SELECT shape から `password_hash` 確実に除外 + 回帰テスト (`SupabaseNotesUnifiedService.test.ts` L728-745) で固定
- `verifyNotePasswordUnified` は `.select("password_hash")` 単一列 SELECT に限定
- 全 JSDoc に SECURITY DEBT を明記
...
- 関連ファイル:
  - `shared/src/services/SupabaseNotesUnifiedService.ts:495-591` (set / remove / verify password)
  - `shared/src/services/notesUnifiedMapper.ts:158-161` (`NOTES_PAYLOAD_COLUMNS` から password_hash 除外)
  - `supabase/migrations/0008_data_unification_schema.sql` (RLS policy `notes_payload_select_own`)
- 関連 commit: DU-G G1 commit (本 PR で SECURITY DEBT を明文化)
