---
Status: IN_PROGRESS (Phase 0 ✅ / Phase 1-3 pending)
Created: 2026-04-25
Updated: 2026-04-25 (Phase 0 完了)
Task: MEMORY.md §予定（リファクタリング計画 Phase 1-3）
...
## Phase 1: Critical Cleanup（2-3 セッション、中リスク）

> 構造的負債で「次のバグ温床」が確実な部分を先回り。

### Steps

- [ ] **1-1**: `cloud/src/routes/sync.ts` 責務分割（既知 Issue 012 の前提地ならし）
...
- [ ] **1-2**: `frontend/src/main.tsx` の Provider tree 共通化
  - `<DesktopProviders>` / `<MobileProviders>` コンポーネント抽出
  - 共通 Provider（ErrorBoundary / Theme / Toast / UndoRedo / TaskTree / Calendar / Template / Daily / Note / Routine / ScheduleItems / Timer）を内部コンポーネントへ
  - `main.tsx` 行数を 97 → 30 行程度に
- [ ] **1-3**: Rust `helpers.rs` / `sync_engine.rs:177` の `row_to_json` 重複統合
  - `db/row_converter.rs` 新設し `row_to_json` / `row_to_string_map` / `row_value_to_serde` を集約
  - `helpers.rs:80-91` を re-export 化
- [ ] **1-4**: SQL injection 防御の明示化
  - `sync_engine.rs:94,100` / `db/helpers.rs:150` の `format!("...{table}...")` 周辺に **`// SAFETY: table is whitelisted via VERSIONED_TABLES`** コメント追加
  - 関数内で `assert!(is_known_table(table))` の `debug_assert!` を 1 行追加（panic 防御）
...
- [ ] **2-1**: `src-tauri/src/db/migrations.rs` (2328 行) を 3 分割
  - `migrations/v1_v30.rs` / `migrations/v31_v60.rs` / `migrations/v61_plus.rs`
  - `migrations/mod.rs` で `apply_all(&mut conn)` を組み立て
  - `create_full_schema()` は v60 状態で確定し、v61+ のみ段階適用に
- [ ] **2-2**: `services/TauriDataService.ts` (1453 行) を domain ごとに分割
  - `services/data/{tasks,schedules,notes,daily,database,sync,wikitags,timer,audio,paper}.ts` で IPC bridge 分離
  - `TauriDataService.ts` は composition root として 200 行以下に
- [ ] **2-3**: 巨大コンポーネント分割（DataService 構造を真似た subcomponent 化）
  - `ScheduleTimeGrid.tsx` (1220) → `ScheduleTimeGrid/{index,GridLayer,EventLayer,DragHandlers,Hooks}.tsx`
  - `OneDaySchedule.tsx` (1165) → `OneDaySchedule/{index,Header,EventColumn,RoutineColumn,Hooks}.tsx`
  - `TagGraphView.tsx` (1443) → `TagGraphView/{index,ForceLayout,Renderer,Interactions,Hooks}.tsx`
  - `TaskDetailPanel.tsx` (947) → `TaskDetailPanel/{index,Header,Body,Tabs,Hooks}.tsx`
  - 各分割は **1 セッション 1 ファイル** で独立 commit
