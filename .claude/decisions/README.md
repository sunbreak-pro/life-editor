...
| 層 | 持つもの | 持たないもの |
| --- | --- | --- |
| ここ（D ファイル） | 問い・選択肢・採否理由・却下案・supersede 連鎖 | 手順・実装詳細 |
| `comm/decisions/POLICY.md` | 「聞かなくていい」恒久裁定 1 行（P-NNN） | 理由の詳細（→ `promoted-to` 元の D ファイル） |
| plans の決定録 / Worklog | **計画スコープ内**の設計判断 | 計画を跨いで効く判断（→ ここへ昇格し plan からは ID 参照） |
| CLAUDE.md インライン注記 | 「消したら Claude が間違う」結論 1 行 | 理由・経緯（新規注記には `(D-… )` を付ける） |

書き込み先の 1 分判定 → [`rules/records.md`](../rules/records.md)。
