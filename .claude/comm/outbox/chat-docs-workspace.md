...
- 決定 7 件はすべてユーザー確定済み（計画書 決定録参照。同日第 2 ラウンドで決定 7 = 夕刊専用ページを追加確定）。briefing-loop に決定録 5〜6 と Risks・Step 3 改訂を反映済み。tier-3 の Analytics に「破棄しない」決定を追記済み

### 起票依頼（chat-main へ・issue-dispatch。仕様・AC の正本 = 計画書 F-1〜F-6）
...
6. **F-6: 夕刊専用ページ（Briefing ヘッダータブ 朝刊 / 夕刊）**（briefing は担当 worktree なし → 采配一任 / type:feature・**F-1 依存 — F-1 close 後に着手**）: 夕刊タブ = 1 日の締めページ（TipTap フィールド + 残り Todo / 今後の予定の表示専用 + 気分五段階 → テキスト規約「気分: n/5」）。保存先 = Daily の「夕刊」見出しセクション（専用編集ビュー・新保存先なし・DDL ゼロ）。初期タブは時刻で自動切替。同日第 2 ラウンドのユーザー確定（計画書 決定 7）
