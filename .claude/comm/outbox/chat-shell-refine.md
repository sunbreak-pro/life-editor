...
- 2026-07-26 (3): **Epic #304 close 完了**。子 PR 1（#316）/ 子 PR 2（#380）が両方 MERGED 済みだったため、main の実コードで DoD 全項目を実測確認（ヘッダー配置 / 履歴空 disabled / エディタ非干渉 `activeInInput: false` / Toast + i18n en·ja / vitest 3 ファイル 20 件緑）→ body チェックボックス消し込み + 完了コメント + close（reason: completed）。実装の新規差分はゼロ（per-chat meta のみの docs PR を提出）。
  - **【chat-main への依頼】実ブラウザ実測**（§7.4 どおり merge 済みコードで）: (1) ヘッダー Undo / Redo ボタン（履歴空 disabled → 操作後 enabled） (2) ⌘Z / ⌘⇧Z のアプリレベル undo と、Materials 本文編集中は TipTap 側に流れること (3) Undo / Redo 時の Toast 文言（en / ja） (4) 対象 4 ドメイン（taskTree / scheduleItem / daily / note）の代表操作の往復。既知挙動（Daily 暗黙作成の undo で当日エントリが soft-delete / moveNote はリストスナップショット復元 / 履歴はセクション切替でリセット）は Issue #304 コメント参照 — バグではなく記録済み仕様。
  - 見送り = Routine（RoutineScheduleSync との undo 連鎖が未設計）。必要になったら chat-main で別 Issue 起票を判断してほしい。
