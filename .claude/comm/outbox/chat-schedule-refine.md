...
ラベルは `section:schedule` + `type:bug` が妥当です。3 件とも 390×844 の同じ画面で連続して見られるので、分ける理由はありません。

### 確認してほしいこと

| 対象                         | 見る場所                                      | 合否の決め方                                                                 |
| ---------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------- |
| #1516 タイトルのフェード     | 月グリッドの、タイトルが長い予定がある日      | **目視のみ**。末尾が透明に溶けて見えるか                                     |
| #1515 + #1558 行の当たり判定 | ドロワー「本日の Todo」タブ + 月の前へ・次へ  | `getBoundingClientRect()` が 44×44 以上、容器が `scrollWidth <= clientWidth` |
| #1517 繰り返しタブ           | 予定行をタップ → `EventEditorPane` の繰り返し | 4 タブが 1 行に収まり、高さが揃っているか                                    |

**機械で測れないのは #1516 だけです。** `mask-image` のグラデーションなので、jsdom ではクラスが付いているかしか見られません。#1515 / #1558 / #1517 は `getBoundingClientRect()` と `document.elementFromPoint()` で数字が出るので、実ブラウザなら断定できます。

### 見るときに知っておいてほしい 3 点

**#1516 は Issue 本文どおりに直していません。** Issue は `text-overflow: ellipsis` を求めていましたが、**#1401 point 4 のユーザー指定「「…」のような省略記号は付けない」と正面から衝突します**。同 Issue の DoD が併記している「フェード等の意図した省略」を採り、末尾 0.5rem を `mask-image` で透明化しました（`shared/src/components/schedule/MonthGrid.tsx:118` の `TITLE_FADE`）。**「…」が出ていないことは不具合ではないので、同じ所見を再起票しないでください。**

**#1517 は Desktop の見た目も 1 段変えています。** `singleLineLabels` は文字サイズと左右パディングを 1 段下げるフラグなので、Desktop の予定エディタでも繰り返しタブ 4 つが少し小さくなります。狭幅だけに効かせる仕組みがフラグ側に無いため、意図して全幅に適用しました。**Desktop で見て許容できないなら、フラグに幅条件を足す別 Issue が要ります。**

**#1558 の当たり判定は `::after` ではなく箱を広げています。** 行末の 2 ボタンは `gap-2` しか離れていないので、当たり判定だけを箱の外へ広げると隣と重なって別のボタンが反応します。そのため `max-md:min-h-11 max-md:min-w-11` で箱ごと広げました。**見た目の四角（`size-6`）は変えていないので、「押せる範囲が四角より広い」状態が正解です。** 指で押して隣が反応しないかを併せて見てください。
