...
## 2026-09-01 → @chat-main（#1400 / #1402 の実機確認依頼）

**#1400（PR #1416）と #1402（PR #1420）を open しました。** どちらも Mobile ドロワーで、このレーンは実ブラウザを起動できない（CLAUDE.md §7.4）ため、merge 後に 390px 幅で次を見てもらえると助かります。

1. **#1400 = 幅の固定**: 設定 → 外観の文字サイズを 小 / 中 / 大 と切り替えて、**ドロワーの幅が動かない**こと（従来は 280 / 360 / 440px と動いていた）。あわせて 360px 幅の端末で**右側に scrim のタップ余地が残る**こと — `max-w-[85vw]` を足したのはこのためで、**issue 本文には無い追加**です。不要なら外します
2. **#1402 = swipe 判定**: 左端から 40px ほど内側に指を置いて右へ引いても**開く**こと（帯を 24 → 44px に拡張）。開いた状態で**パネルの外（scrim）から左へスワイプして閉じる**こと。#1050 のスライドイン + scrim フェードに退行が無いことも一緒に見てほしいです

- どちらも `origin/main` 直上の独立ブランチで、CI `verify` + `docs-lint` はローカル全緑です

## 2026-08-30 → @chat-main（#1290 の実機確認依頼 + #1289 との関係）

**#1290（Mobile のタグ編集導線）を PR #1308 で open しました。** このレーンは playwright / dev server を起動できない（CLAUDE.md §7.4）ので、merge 後に実機（390px 幅）で次の 2 点を見てもらえると助かります。

1. **導線が通ること**: bottom bar「その他」→ Quick actions「タグを編集」→ 一覧 → タグをタップ → 名前 / アイコン / 色を編集 → 保存。パネルは wide と同じ `TagEditModal` で、narrow は #740 の 2 ステップ（戻るリンク付き）になる想定です
2. **ソフトキーボードとの相性**: パネルは `Modal`（中央固定・`h-[560px] max-h-[80vh]`）なので、名前入力にフォーカスしたときに入力欄がキーボードの裏に回らないか。回るなら**パネル側の調整は別 Issue**にしたいです（#1289 = アイコンピッカーの背景透過が同じファイル群を直している最中なので、着地後に）

- **エディタ内部は 1 行も触っていません**（`shared/src/components/tagEdit/*` / `web/src/tags/TagEditorHost.tsx` は差分ゼロ）。#1289 と同時に merge しても衝突しない範囲です
- `mobile-scope.md` #9 行は **Full** に上書きしました（ユーザー指示 2026-08-30）。`D-20260810-mobile-1` の「+ 色」は経緯として残しています
- **積み残し（別 PR）**: 同ファイル #16 行が「Undo/Redo の導線 = その他シート」のまま（#1035 でヘッダー右端にも出るようになった件の docs 追随）。このレーンで拾えますが、#1290 のスコープ外なので分けました
