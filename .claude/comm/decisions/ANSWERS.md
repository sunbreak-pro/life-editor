...
- D-20260815-briefing-1: **A**（週・月・年の目標は専用ノート 1 枚の本文に `## 週目標` / `## 月目標` / `## 年目標` として持つ。DDL ゼロで「宣言」の実装をそのまま写せる → #872（PR #914 着地済み）。2026-08-16 チャットで回答・chat-briefing-refine が転記を受任）
- D-20260815-briefing-2: **A**（目標ノートは予約 id `note-goals` で固定する。`daily-YYYY-MM-DD` と同じ決定論的 id の流儀で、改名・重複タイトルに強い。同上）
- D-20260815-briefing-3: **B**（期間キー付きで保存し、週・月・年が変わると欄が空になり前の期間はノート本文に履歴として残る。**推奨の A ではなく B を採用**したため #872 の着地形を作り替える → 実装 Issue #957。同上）
- D-20260815-briefing-4: **A**（目標ブロックは「宣言」の直下に置く。今日 → 週 → 月 → 年 と時間軸が広がる並びにする。同上）
- D-20260815-briefing-5: **A**（目標は朝刊のみ。夕刊（`EveningView.tsx`）には出さない。同上）
- D-20260815-briefing-6: **A**（目標は幅共通で編集可 = 宣言・気分と同じ扱い。「Consumption なのに書ける」矛盾を 1 件広げるので、`docs/requirements/mobile-scope.md` に本件も D-20260810-mobile-2 の対象である旨を追記する。同上）
- D-20260815-briefing-7: **B**（保存失敗を Toast で拾う仕組みを紙面の書き込み 3 経路（宣言 / 夕刊 / 目標）にまとめて入れる。穴の本体は「失敗が無音」であってキャプションの不在ではないため、目標だけにキャプションを足す A は却下 → 実装 Issue #955。同上）
