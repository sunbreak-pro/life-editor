...
2. **AC 免除**: **免除ゼロ、超過の明示が 1 件**。「PR diff ±200 行以内」を超過（実装 約 200 行 + テスト 約 380 行 + 記録類）。免除ではなく PR 本文に内訳を記載した。**未達が 1 件残る** = Step 5 の 👀 実機目視 4 点（本文タップでシートが閉じないか / タブバーの出戻り / タブバー非表示時の safe-area 下端 / 「その他」シートがキーボードで消える件）。merge 後 deploy 待ちのため、memory の「👀 ユーザー実機目視待ち」へ移送して追跡を継続する。**iOS は未検証**
