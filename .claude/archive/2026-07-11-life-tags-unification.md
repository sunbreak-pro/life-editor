...
| クラスタ         | 主なファイル                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 型               | `shared/src/types/taskTree.ts`（`NodeType`:9・`folderType`:36・`originalParentId`:37・`FOLDER_PAD_TOP`:12 — **S3 #225 で対応済**）/ `types/note.ts`（`NoteNodeType` — **#375 で対応済**）/ `shared/src/index.ts` re-export                                                                                                                                                            |
...
| mapper / hooks   | `services/taskMapper.ts`（`task_type`/`folder_type`）/ `notesUnifiedMapper.ts`（`note_type`）/ `hooks/useTaskTreeCRUD.ts`（Complete-folder 自動生成:133-159・`completeFolderWithChildren`/`uncompleteFolder`:294-341）/ `useTaskTreeMovement.ts` / `useNotesUnifiedAPI.ts`（`createFolder` — **#375 で撤去済**）/ `utils/sortTaskNodes.ts` / `getDescendantTasks.ts` / `folderTag.ts` |
...
横断の後継対応（各セクション adoption・Step 4 後段）: ~~`utils/analyticsAggregation.ts` `aggregateByFolder` → タグ集計（analytics）~~ **完了 = #334**（`aggregateWorkTimeByTag` へ置換・チャートは「タグ別作業時間」に改称）/ ~~`Connect/graph/buildGraphModel.ts` の folder→"project" ノード → タグ起点（connect）~~ **完了 = #375**（`project` ノード種別ごと退役し tag ノードが後継。Notes 側 `NoteNodeType` / `createFolder` の撤去も同 PR）/ i18n の folder 系キー（`newFolder`・`folderCompleteConfirm`・`defaultTaskFolder*` 等 en/ja 両 catalog）/ ~~mcp-server の `folder_id` パラメータ~~ **完了 = #419**（`list_tasks` の `folder_id` → `parent_id` にリネーム。実体は最初から `tasks_payload.parent_item_id` で、`create_task` が同じ概念に使っている名前へ揃えた。旧名は残していない）。
...
- 2026-07-27: **Notes 側 folder 退役の後段完了（#375・chat-analytics-refine）**。S3 が「意図的な過渡期非対称」として温存していた Notes 側を撤去 — `NoteNodeType = "note"` 単一化・`useNotesUnifiedAPI.createFolder` 撤去（undo ラベル i18n も en/ja lockstep で削除）・**legacy `note_type='folder'` 行の fetch 時除外を新設**（`isLegacyNoteFolderRow` — list / Trash / search / MCP `fetchLiveNotes` の 4 経路。NULL note_type 生存・孤児許容は Tasks 側と同型）・buildTagGroups / NotesView / useItemLinkTargets の folder ガード撤去。**Connect グラフの `project` ノード種別も退役**（ユーザー確定 = 「project 種別ごと退役」）: まとまりを表すノードは `wiki_tags` + `wiki_tag_assignments` 由来の tag ノードが後継で、凡例・型フィルタ・`connect.graph.typeProject` を撤去。shared build + 1168 tests + web build + mcp-server build green。DDL 変更なし（データ側は 0020 で完了済み）
- 2026-07-11 (3): **S3 実装完了**（chat-materials-refine）。`NodeType = "task"` 単一化・`folderType`/`originalParentId`/`FOLDER_PAD_TOP` 除去・`folderTag.ts`/`getDescendantTasks()` 削除・sortTaskNodes / useTaskTreeAPI / useTaskTreeCRUD / TaskDetailPanel / KanbanView / useTaskTreeDnd の folder 分岐撤去・**legacy folder 行の fetch 時除外を新設**（`isLegacyFolderRow` — tree / Trash に幽霊 folder を出さない。NULL task_type 生存・孤児許容維持）・i18n folder 系 orphan キー削除（en/ja lockstep・FileExplorer / Notes 系は温存）・docs sweep（tier-1 Tasks/Notes・tier-2 WikiTags→life-tags 昇格）・新規テスト 2 本（applyStatusChange DONE 沈み / legacy filter）。shared build + 855 tests + web build + lint green。監査 = role-qa PASS（Blocking 0）/ sync-auditor Blocking 0（Nit 1: 再 root 化タスクの `original_parent_id` が syncTaskTree で null 上書きされ得るが rollback の SSOT は `life_tags_migration_log` のため実害なし）。**意図的な残置**: analytics `aggregateByFolder` は `[]` 返却（Project work time チャート空表示 = tag 後継集計は analytics レーン）/ Notes folder（`NoteNodeType`）は Connect グラフ後継と併せ別レーン — 両方 chat-main へ起票依頼済み。**残ゲート**: 🛑 実データ変換 = ユーザー `supabase db push`（0020 + 0021）+ verify.sql（Step 5）
