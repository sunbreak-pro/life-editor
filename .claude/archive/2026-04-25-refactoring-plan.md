---
Status: COMPLETED (Phase 0-3 全完了 2026-04-26)
Created: 2026-04-25
Updated: 2026-04-26 (Phase 2-4 / 3-1 / 3-4 完了で全 Phase 完遂、archive 移動)
Task: MEMORY.md §予定（リファクタリング計画 Phase 2 残）
Project path: /Users/newlife/dev/apps/life-editor
Related: [.claude/docs/code-inventory.md](./docs/code-inventory.md) — アクティブ / 凍結 / 重複 / 削除候補の棚卸し / [.claude/2026-04-26-refactoring-verification-plan.md](./2026-04-26-refactoring-verification-plan.md) — 検証手順
---
...
- [ ] **2-3**: 巨大コンポーネント分割（DataService 構造を真似た subcomponent 化）
  ...
  - [x] **2-3b ScheduleTimeGrid** (1221→926 行) ✅ 2026-04-26 — 純粋ロジック (`layoutAllItems` / `rangesOverlap` / `computeGroupFrames` / `detectRoutineTaskSplit` / `adjustItemsForRoutineSplit` / 型 / 定数) を `scheduleTimeGridLayout.ts` (326 行) に抽出。当初計画の JSX サブディレクトリ分割 (GridLayer/EventLayer/DragHandlers) は手動 UI 検証必須のため見送り、純粋関数のみ抽出する保守的アプローチ。`tsc -b` clean、288/288 tests pass
  - [x] **2-3c OneDaySchedule** (1276→1172 行) ✅ 2026-04-26 — `useDayFlowFilters` (97 行) + `useDayFlowDialogs` (223 行) 抽出、`DAY_FLOW_FILTER_TABS` / `DayFlowFilterTab` を `dayFlowFilters.ts` に分離 (re-export で後方互換維持)。`tsc -b` clean、288/288 tests pass
  - [x] **2-3d TagGraphView** (1443→1414 行) ✅ 2026-04-26 — localStorage helpers (`loadPositions` / `savePositions` / `loadViewport` / `saveViewport` / `isSpecialFilterId` / `VIRTUAL_LINK_EDGES_HIDDEN_ID` 定数) を `tagGraphStorage.ts` (45 行) に抽出。React Flow 内部と密結合の JSX サブディレクトリ分割 (ForceLayout/Renderer/Interactions/Hooks) は手動 UI 検証必須のため見送り。`tsc -b` clean、288/288 tests pass
  - 各分割は **1 セッション 1 ファイル** で独立 commit + UI 手動検証必須（自動テスト無し）
- [x] **2-4** ✅ 2026-04-26: Calendar 共通ロジック抽出 (保守的アプローチ — 完全 UI 統合は見送り)
  - `frontend/src/utils/calendarGrid.ts` 新設 (59 行): `buildCalendarGrid({year, month, weekStartsOn: 0|1, fixedRows?})` で月グリッド計算統一 (Sunday/Monday 両モード)、`addDays` / `getMondayOf` / `getWeekDates` も追加 (3-4 で利用)
  - `frontend/src/utils/calendarGrid.test.ts` (8 tests): 両モード / fixedRows 有無 / 月境界 / うるう年 / addDays 月跨ぎ
  - Desktop `useCalendar.ts:265` 30 行 useMemo を `buildCalendarGrid({weekStartsOn: 0, fixedRows: 6})` 1 行に
  - Mobile `MobileCalendarView.tsx` 12 行 inline calendarDays を `buildCalendarGrid({weekStartsOn: 1})` に + destructure rename `{date, isCurrentMonth: inMonth}` で内部 prop 互換維持
  - **完全 UI 統合 (`useCalendarViewLogic` + `Calendar/shared/*.tsx`) は見送り**: Context vs Service 層差 + Popover vs BottomSheet で regression リスク高、Phase 2-3b/d と同方針 (純粋ロジックのみ抽出)。等価性 dow=0/1/6 全モード一致確認済。`tsc -b` clean / 332/332 tests pass
...
- [x] **3-1** ✅ 2026-04-26: Rust 全 repository の row_to_model 統一
  - `db/row_converter.rs` に `FromRow` trait + `query_all<T: FromRow, P: Params>` + `query_one<T: FromRow, P: Params>` ヘルパを追加 (`row_to_json` は column-agnostic JSON converter として既存維持)
  - 24 repository (+ sidebar_link、合計 25 ファイル) で 33+ の `fn row_to_X(&Row) -> Result<X>` を `impl FromRow for X { fn from_row(...) -> Result<Self> { ... 既存 body 完全保持 ... } }` に移行
  - callers の `prepare → query_map → collect` を `query_all(conn, sql, params)` に / `prepare → query_row` を `query_one(conn, sql, params)` に / Option 返却の match 付きパターンを `match query_one::<T, _>(conn, sql, params) { ... }` に変換
  - エッジケース: `note_link_repository::fetch_backlinks` の query_map 内 `BacklinkHit` 組立カスタムロジックは closure 内で `NoteLink::from_row(row)?` 置換のみ (closure pattern 4) / transaction 内 (`tx.query_map`) は対象外で保持 / SQL 文字列・パラメータ・ロジックは全件無変更
  - 4 並列 sub-agent で機械的書き換えを高速実行。実績 LOC: Plan 想定 -1500 に対し +175 行 (impl block boilerplate と call-site 短縮の差し引き)。trait 抽象化のメリットは確保 (将来の generic helper 追加可能)
  - 検証: `cargo build --lib` warnings=0 / `cargo test --lib` 25/25 pass / `grep -rnE "fn row_to_" src/db/` 出力は `row_to_json` のみ
- [x] **3-2** ✅ 2026-04-26: 既知 Issue 012 cursor pagination の本実装
  - Server (`cloud/src/routes/sync/versioned.ts`): `pullVersionedDelta` が batch 内の最大 `server_updated_at` を `nextSince` として返却。`/sync/changes` のレスポンスに同梱
  - Client (`src-tauri/src/commands/sync_commands.rs::sync_trigger`): `loop { fetch_changes(cursor); apply; if !has_more break; cursor = next_since; }` ループ化。`sync_last_synced_at` は全 page 完了後にのみ永続化 (中断時の再開保証)
  - `SyncPayload` に `next_since: String` 追加 (旧サーバ互換: `#[serde(default)]` で空文字 fallback、空時は break して安全に終了)
  - 多重 break ガード: `has_more=false` / `next_since` 空 / cursor 進行なし のいずれかで終了 (無限ループ防止)
  - LIMIT=5000 (`SYNC_PAGE_SIZE`) は既に `config/syncTables.ts` に切り出し済 (Phase 1-1 で実施)
  - `cargo build` clean、`cargo test --lib sync` 2/2 pass、`tsc -b` clean、288/288 frontend tests pass
- [x] **3-3** ✅ 2026-04-26: Schedule 命名混乱の解消
  - `frontend/src/components/Schedule/` → `frontend/src/components/ScheduleList/` に rename (`git mv`)
  - 外部 import 4 箇所更新: `App.tsx` (×2) / `useTaskDetailHandlers.ts` / `useSectionCommands.ts` / `Work/FreeSessionSaveDialog.tsx`
  - `Tasks/TaskDetail/*` の `../Schedule/shared/` import は `Tasks/Schedule/` を参照しており別ディレクトリのため変更不要
  - Plan の "alias re-export 1 週間維持" は省略 — 端末 / claude history 等の外部参照は調査の結果なし。`tsc -b` clean、288/288 tests pass
- [x] **3-4** ✅ 2026-04-26: Schedule 共通 hook 抽出 (保守的アプローチ — 完全 UI 統合は見送り)
  - `getMondayOf(date)` (Mon = 月曜、Sun → 6 日前) / `getWeekDates(monday)` (7 日 array) を `utils/calendarGrid.ts` に追加 (Phase 2-4 のファイルを拡張)
  - `MobileCalendarStrip.tsx`: local `getMonday` / `formatDateStr` / `addDays` / `getWeekDates` 4 関数 (約 24 行) を削除、共有版 import に置換
  - `MobileScheduleView.tsx`: `loadWeekItems` の inline week-range 計算 (15 行) を `const monday = getMondayOf(...); const sunday = addDays(monday, 6)` 2 行に圧縮、local `todayStr()` 削除して `useState(getTodayKey)` lazy init pattern に
  ...
  - **完全 UI 統合 (`useScheduleViewLogic` + shared component) は見送り**: ScheduleSection (613 行) と MobileScheduleView (489 行) は機能差大 (Routine management / Task↔Event変換 / Undo/Redo / Portal sidebar が Desktop のみ)、Phase 2-4 と同方針
- [x] **3-5** ✅ 2026-04-26: 論理キー UNIQUE 制約の網羅的追加 — **AUDIT ONLY (no migration needed)**
  - 主要テーブルの audit 結果:
    - `dailies.date` ✅ UNIQUE 既存 / `wiki_tags.name` / `routine_tag_definitions.name` / `sound_tag_definitions.name` / `calendar_tag_definitions.name` ✅
    - `sound_tag_assignments` / `routine_tag_assignments` / `wiki_tag_assignments` / `wiki_tag_group_members` / `routine_group_tag_assignments` / `calendar_tag_assignments` ✅ すべて PRIMARY KEY 複合キーで重複防止済
    - `wiki_tag_connections (source_tag_id, target_tag_id)` ✅ UNIQUE / `note_connections (source_note_id, target_note_id)` ✅ UNIQUE
    - `time_memos (date, hour)` ✅ UNIQUE / `database_cells (row_id, property_id)` ✅ UNIQUE / `note_aliases.alias` ✅ UNIQUE
    - `tasks` / `notes` / `routines` の論理キーは PK (id) のみ — タイトル重複は仕様上許容 (N=1 ユーザーがフォルダ内に同名アイテム作成可)
  - **唯一未制約な箇所**: `note_links` (source/target/heading/block_id/link_type). ただし「同一ノート間に異なる heading を指す複数リンク」が正当ユースケースのため UNIQUE 化は要件 vague で見送り (V63 で schedule_items 重複対応した時のように具体的に問題発見されたら別途対処)
