...
- 残した恒久知見: 各台帳の共通仕様の正本は親計画 `archive/2026-07-11-layout-standard-v2.md`（COMPLETED・#474 で archive 移動）側にあり、台帳自体は転記を持たない設計だったため失われる知見はない。担当の一次情報は常に `gh issue list -R sunbreak-pro/life-editor --label section:<id>`。
