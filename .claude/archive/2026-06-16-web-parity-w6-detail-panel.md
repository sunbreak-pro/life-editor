---
Status: COMPLETED — Steps 1–5 + 7 完了・#89 main merged（archive 2026-07-04）。残 Step 6 👀 実機目視のみ
Created: 2026-06-16
...
| 7 ✅ | Draft PR → レビュー → main merge（#89 squash-merge 済）                                             | 🛑 人手 | PR レビュー & merge ボタン                   |
...
- 2026-06-17（実装・merge）: Steps 1–5 実装（`shared/src/components/MasterDetail.tsx` 新設・barrel export・en/ja に `notesView.detailEmpty` / `detailTitle` / `closeDetail`・`shared/tests/masterDetail.test.tsx`・`web/src/notes/NotesView.tsx` を `MasterDetail` で組み替え）。`AppShell` 無改変・選択は `NotesUnifiedProvider.selectedNote` 保持・`RichTextEditor` の `key={selected.id}` remount 戦略維持。検証: shared build / web build / frontend build いずれも exit 0、shared test 40 files / 436 tests 全 pass。diff 374 insertions / 19 deletions（Scope 内）。PR #89 を Draft 作成 → squash-merge で main（`1b35d6b`）へ統合済。残 = Step 6 👀 目視（広幅3ペイン同時表示 / 狭幅シート開閉 / ノート切替で editor remount 維持）。
