...
Created: 2026-02-22
Depends: 019-phase1-security-critical-fixes (also DROPPED)
Drop reason: Electron 前提の Plan（`electron/database/migrations.ts` / `electron/ipc/dataIOHandlers.ts` 参照）。Tauri 2.0 migration (2026-04-15) で対象ファイルが消失。3 ヶ月放置 (commit 1 件)。Tauri 版の類似課題は 2026-04-18 のコードレビューで Blocking 3 / Important 5 を修正済。
---
...
1. ファイルサイズ制限の追加:
   ...
   if (stats.size > MAX_IMPORT_SIZE) {
     throw new Error(
       `Import file too large: ${stats.size} bytes (max: ${MAX_IMPORT_SIZE})`,
     );
   }
   ```
...
3. `runMigrations` 自体にもエラーハンドリングを追加:
   ...
   try {
     ...
     throw new Error(
       `Database migration V${N} failed. Please contact support.`,
     );
   }
   ```
