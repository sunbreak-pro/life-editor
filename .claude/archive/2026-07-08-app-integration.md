...
- 2026-07-09: **PR 前提の訂正** — PR #176 は計画書のみの状態で先行 squash merge 済みだった（main `731c3faf`。QA レビュー反映 commit `dd778eb3` は未収載）。origin/main（#177 含む）を本ブランチへ merge し、計画書は QA 反映版（ours）を採用。Step 7 のゴールは「実装を積んで**新規 PR** を Ready 化」に読み替え。
- 2026-07-09: Step 1+2 実装・commit — `shared/src/sections.ts` registry 新設（`SectionId` は registry 派生 7 種）・MainScreen 5 リスト導出化・i18n terminal 掃除（Issue #146 DoD の case-insensitive grep 0 まで充足）・死画面 5 + 旧部品 3 削除。検証: shared build/test・web build 緑。
- 2026-07-09: Step 3 は**選択肢 b** で解決 — registry に `rightSidebar` フラグを追加し「トグル表示 ⟺ portal 供給」を SSOT 化（Analytics / Trash は false でトグル非表示・空パネル解消。担保テスト = `sections.test.ts` の registry contract）。Connect は無変更。
- 2026-07-09: Step 4 — new-task create-and-focus は「shell が pending フラグ → Kanban が mount/遷移時に消費して add dialog を開く」方式で配線（新規 DataService API なし・`set-state-in-effect` lint は adjust-during-render パターンで設計回避）。Materials タブ件数バッジは headless `MaterialsCountsBridge`（SyncProvider 内・syncVersion で再取得）で配線。**undo・redo は見送り** — web に UndoRedo 基盤（provider / command stack）が存在せず、サブシステム新設は本統合パスのスコープ外（新機能追加 = Non-goals）。`MainScreen.tsx` の GlobalShortcuts 隣に DEFERRED コメントで明示。
- 2026-07-09: Step 6 検証時の note — shared test で `monthGrid` / `weekTimeGrid` の 2 件が timeout 失敗したが、worker fork 起動失敗（マシン負荷）起因のフレークで、単独再実行で全緑を確認（変更ファイルとも無関係）。
- 2026-07-26: 追記 — Step 4 の「undo・redo は見送り」は Issue #304 で解消済み（子 PR 1 = PR #316 が UndoRedo 基盤 + taskTree、子 PR 2 が schedule / daily / note を配線）。当時の DEFERRED コメントも同 Epic で撤去。
