...
- **到達基準**: 操作感・情報密度の同等（`ink-*` で再構成。ピクセル一致は求めない。親書 Context 方針 2）。
- **制約**:
  ...
  - 新規 UI は `shared/src/components/` に集約（§6・W0 案 A）。`ink-*` トークン厳守・主要 UI 背景に透明度禁止。i18n は props 注入（shared 内で `useTranslation()` 直呼び禁止）。
...
| 2 ✅ | `NavItem` + `SidebarNav`（広幅・折りたたみ/アクティブ/フッタ。props 注入・`ink-*`）                | 🤖 自律 | `cd shared && npm run build` exit 0                          |
...
- frontend 規約: `../../../rules/frontend.md`（Provider 順序 / Pattern A / 配置表 / `ink-*` / i18n）
