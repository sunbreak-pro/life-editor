...
> **2026-09-09 進捗 (#1300 / #1301 — 配れる形が実在した)**: こうだいさんが tag `desktop-v0.1.0` を push し、**run 34342012501 が build 2 OS + release の 3 ジョブとも success**。draft Release `desktop-v0.1.0` に `Life.Editor-0.1.0-x64-setup.exe`（84,351,933 B）と `Life.Editor-0.1.0-arm64.dmg`（103,608,244 B）が乗った（2026-09-12 に `gh release view` で再実測。**まだ draft** で、公開 = `--draft=false` はユーザー手番 — P-001 と同じ考え方で workflow が draft で作る）。アセット名のスペースが `.` になるのは GitHub 側の置換で、`artifactName` の `${productName}` が「Life Editor」だから起きる（release ジョブの glob は `*-x64-setup.exe` / `*-arm64.dmg` なので影響しない）。同日 D-20260830-main-3 = A を受けて **x64 dmg の宣言を落とした**（`electron-builder.yml` の mac arch は arm64 のみ・workflow の macos 行も arm64 だけ）。**残るのは Windows 実機での実アカウントログイン + Todo CRUD だけ**（Step 7 の 6 項目中 5 は 2026-09-05 に通過済み）。
