...
- **データ管理**: SQLite永続化（rusqlite）、JSON Export/Import、バックアップ付きインポート、全データリセット（自動バックアップ付き）
...
- **自動アップデート**: tauri-plugin-updater + GitHub Releases、ユーザー確認型ダウンロード・インストール
- **構造化ログ**: ファイル出力、Settings画面でログ閲覧・フィルタ・エクスポート
- **アプリ内ターミナル**: portable-pty (Rust) + xterm.js による統合ターミナル。Ctrl+`` ` ``で開閉、ドラッグで高さ調整、Catppuccin Mocha テーマ。全セクション共通の下部パネルとして表示
...
- **Desktop**: Tauri 2.0 + rusqlite + portable-pty + tauri-plugin-\*
...
```bash
npm install    # postinstallでfrontend依存を自動インストール
```
