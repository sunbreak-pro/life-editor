...
- **キーボードショートカット**: Space（タイマー）、n（新規タスク）、Escape（モーダル閉じ）、Cmd/Ctrl+.（左サイドバー開閉）、Cmd/Ctrl+,（Settings遷移）、Cmd/Ctrl+1〜4（セクション切替: Tasks/Work/Analytics/Settings）、↑/↓（タスク移動）、Tab/Shift+Tab（インデント）、r（タイマーリセット）、Cmd/Ctrl+Shift+T（モーダル）、j/k/t/m（カレンダー操作）
- **ゴミ箱**: サイドバーからアクセス可能なトップレベルセクション、削除済みタスク・ノート・カスタムサウンドの復元・完全削除
- **Settings画面**: 右サイドナビ5タブ構成（General/Notifications/AI/Data/Advanced）、外観設定、言語切替、通知設定
