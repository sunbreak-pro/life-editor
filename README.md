...
- **タスクツリー（メインコンテンツ）**: Tasksセクションのメインエリアにタスクツリー全体を表示、ヘッダーにフォルダディレクトリドロップダウン+Undo/Redoボタン
- **タスク詳細サイドバー**: 右サイドバーでタスク/フォルダの詳細表示（タイトル編集、フォルダ移動、カラーピッカー、再生ボタン、作業時間設定、ミニカレンダー、テキストメモ）
...
- **Work画面**: LeftSidebarの「Work」セクションで常時アクセス可能、ヘッダーにセッション完了・タスク完了ボタン配置（確認ダイアログ付き）、WorkSidebarにPomodoro設定/Playlistタブ、SoundMixerはsessionTypeに応じて自動切替
- **サイドバータイマー表示**: タイマー実行中はWork項目下にタスク名・残り時間・編集ボタンを表示
- **TaskTreeタイマー表示**: 実行中のタスク行に残り時間テキスト+ミニプログレスバーを表示
- **Music管理画面**: MusicSidebarにSounds/Playlistsタブ（検索・タグフィルタ・サウンド一覧・プレイリスト管理）、メインエリアにタグ管理パネル+プレイリスト詳細、各サウンドにボリューム・トグル・シーク操作、「タイマーに追加」ボタン、カスタムサウンド追加
...
- **スケジュール**: Calendar + Routine を統合した Schedule セクション。月/週/1日の3ビュー切替。1日ビューは左にタイムグリッド（0:00〜24:00、ScheduleItem + TaskNode 表示、クリックで新規作成、完了チェック→半透明+次ハイライト、現在時刻インジケーター）、右にタブパネル（Today Flow 縦フローチャート / Routines タブ）。ルーティンテンプレート（毎日/カスタム曜日）によるスケジュール自動挿入。ルーティンCRUD + 完了率統計。月/週ビューはタスク日付別表示・フィルタリング（incomplete/completed）・フォルダ別ビュー対応。Tasks/Memoモード切替（サイドバータブ）、MemoモードはDaily memo+Notes を月表示で統合表示
