...
#[tauri::command]
pub fn db_routine_groups_fetch_all_tag_assignments(
    state: State<'_, DbState>,
) -> Result<Value, String> {
    let conn = state.conn.lock().map_err(|e| e.to_string())?;
    routine_group_repository::fetch_all_tag_assignments(&conn)
        .map_err(|e| e.to_string())
        .and_then(|v| serde_json::to_value(v).map_err(|e| e.to_string()))
}

#[tauri::command]
pub fn db_routine_groups_set_tags_for_group(
    state: State<'_, DbState>,
    group_id: String,
    tag_ids: Vec<i64>,
) -> Result<(), String> {
    let conn = state.conn.lock().map_err(|e| e.to_string())?;
    routine_group_repository::set_tags_for_group(&conn, &group_id, &tag_ids)
        .map_err(|e| e.to_string())
}
